...
public:
	static TSharedPtr<FModulateAlpha> CreateFromLayerInfo(ULandscapeLayerInfoObject* InLayerInfo, int32 InLandscapeMinX, int32 InLandscapeMinY)
	{
		if (CVarLandscapeSplineFalloffModulation.GetValueOnAnyThread() == 0
			|| (InLayerInfo == nullptr)
			|| (InLayerInfo->GetSplineFalloffModulationTexture() == nullptr))
		{
			return nullptr;
		}
	...
	float Modulate(float InValue, int32 InX, int32 InY) const
	{
		...
		const float TextureWidth = Image.GetWidth();
		const float TextureHeight = Image.GetHeight();
		...
		int32 X1 = FMath::Min(FMath::CeilToInt(X), TextureWidth - 1);
		int32 Y0 = FMath::FloorToInt(Y);
		int32 Y1 = FMath::Min(FMath::CeilToInt(Y), TextureHeight - 1);

		float SampleX0Y0 = Image.GetOnePixelLinear(X0, Y0).Component(ChannelIndex);
		float SampleX1Y0 = Image.GetOnePixelLinear(X1, Y0).Component(ChannelIndex); 
		float SampleX0Y1 = Image.GetOnePixelLinear(X0, Y1).Component(ChannelIndex);
		float SampleX1Y1 = Image.GetOnePixelLinear(X1, Y1).Component(ChannelIndex);

		float LerpY0 = FMath::Lerp(SampleX0Y0, SampleX1Y0, X - X0);
		float LerpY1 = FMath::Lerp(SampleX0Y1, SampleX1Y1, X - X0);
		float FinalLerp = FMath::Lerp(LerpY0, LerpY1, Y - Y0);

		InValue = InValue + ((FinalLerp - Bias) * Scale);
...
private:
	...
	{
		check(InLayerInfo && InLayerInfo->GetSplineFalloffModulationTexture());

		bIsValid = Initialize(InLayerInfo, InLandscapeMinX, InLandscapeMinY);
	}

	bool Initialize(ULandscapeLayerInfoObject* InLayerInfo, int32 InLandscapeMinX, int32 InLandscapeMinY)
	{
		FTextureSource& TextureSource = InLayerInfo->GetSplineFalloffModulationTexture()->Source;
		if (!TextureSource.IsValid() || !TextureSource.GetMipImage(Image, /*MipIndex = */0))
		{
			UE_LOG(LogLandscape, Error, TEXT("Invalid source data for spline falloff modulation texture (%s) of layer %s. Alpha modulation will be disabled."),
				*InLayerInfo->GetSplineFalloffModulationTexture()->GetPathName(), *InLayerInfo->GetLayerName().ToString());
			return false;
		}

		if ((TextureSource.GetSizeX() * TextureSource.GetSizeY()) <= 0)
		{
			UE_LOG(LogLandscape, Error, TEXT("Invalid source data for spline falloff modulation texture (%s) of layer %s: incorrect texture size (%i x %i). Alpha modulation will be disabled."),
				*InLayerInfo->GetSplineFalloffModulationTexture()->GetPathName(), *InLayerInfo->GetLayerName().ToString(), TextureSource.GetSizeX(), TextureSource.GetSizeY());
			return false;
		}
		
		const ETextureSourceFormat Format = TextureSource.GetFormat();
		const FTextureSourceFormatInfo& FormatInfo = GTextureSourceFormats[Format];
		if ((int32)InLayerInfo->GetSplineFalloffModulationColorMask() >= FormatInfo.NumComponents)
		{
			UE_LOG(LogLandscape, Error, TEXT("Invalid color mask (%s) for spline falloff modulation texture (%s) of layer %s: the texture only has %i channel(s) available. Alpha modulation will be disabled."),
				*UEnum::GetValueAsString(InLayerInfo->GetSplineFalloffModulationColorMask()), *InLayerInfo->GetSplineFalloffModulationTexture()->GetPathName(), *InLayerInfo->GetLayerName().ToString(), FormatInfo.NumComponents);
			return false;
		}
		ChannelIndex = (int32)InLayerInfo->GetSplineFalloffModulationColorMask();

		if (FMath::Abs(InLayerInfo->GetSplineFalloffModulationTiling()) > UE_SMALL_NUMBER)
		{
			Tiling = 1.0f / InLayerInfo->GetSplineFalloffModulationTiling();
		}
		else
		{
			UE_LOG(LogLandscape, Warning, TEXT("Invalid tiling for spline falloff modulation texture of layer %s. Alpha modulation will be disabled."),
				*InLayerInfo->GetLayerName().ToString(), Tiling);
			return false;
		}

		Bias = InLayerInfo->GetSplineFalloffModulationBias();
		Scale = InLayerInfo->GetSplineFalloffModulationScale();
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	LandscapeEdit.SetAlphaData(LayerInfo, MinX, MinY, MaxX, MaxY, Data.GetData(), /*Stride = */0, ELandscapeLayerPaintingRestriction::None, 
		/*bWeightAdjust = */(LayerInfo->GetBlendMethod() == ELandscapeTargetLayerBlendMethod::FinalWeightBlending), /*bTotalWeightAdjust = */false);
...
{
	return ApplySplines(OutModifiedComponents, bMarkPackageDirty);
}

bool ULandscapeInfo::ApplySplines(TSet<TObjectPtr<ULandscapeComponent>>* OutModifiedComponents, bool bMarkPackageDirty)
{
