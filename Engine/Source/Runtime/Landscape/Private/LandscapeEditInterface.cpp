											   ...
											   bool InUpdateBounds, bool InUpdateCollision, bool InGenerateMips)
{
	...
	if (InCalcNormals)
	{
		...
		for (int32 Y = 0; Y < NumVertsY - 1; Y++)
		{
			...
			{
				FVector Vert00 = FVector(0.0f, 0.0f, LandscapeDataAccess::GetLocalHeight(InData[(X + 0) + InStride*(Y + 0)])) * DrawScale;
				FVector Vert01 = FVector(0.0f, 1.0f, LandscapeDataAccess::GetLocalHeight(InData[(X + 0) + InStride*(Y + 1)])) * DrawScale;
				FVector Vert10 = FVector(1.0f, 0.0f, LandscapeDataAccess::GetLocalHeight(InData[(X + 1) + InStride*(Y + 0)])) * DrawScale;
				FVector Vert11 = FVector(1.0f, 1.0f, LandscapeDataAccess::GetLocalHeight(InData[(X + 1) + InStride*(Y + 1)])) * DrawScale;
	...
	for (int32 ComponentIndexY = ComponentIndexY1; ComponentIndexY <= ComponentIndexY2; ComponentIndexY++)
	{
		for (int32 ComponentIndexX = ComponentIndexX1; ComponentIndexX <= ComponentIndexX2; ComponentIndexX++)
		{
			...
			// In Edit Layer based landscapes, cumulate dirty collision region (will be used next time UpdateCollisionHeightData is called)
...
void ULandscapeComponent::DeleteLayerInternal(ULandscapeLayerInfoObject* LayerInfo, FLandscapeEditDataInterface& LandscapeEdit, const FGuid& InEditLayerGuid)
{
	...
	if ((LayerInfo == nullptr) || (LayerInfo->GetBlendMethod() == ELandscapeTargetLayerBlendMethod::FinalWeightBlending))
	{
		...
		for (int32 LayerIdx = 0; LayerIdx < ComponentWeightmapLayerAllocations.Num(); LayerIdx++)
		{
			...
			// Find the layer info and record if it is a "no weight blend" layer.
			const bool bPerformLegacyWeightBalancing = (Allocation.LayerInfo == nullptr) || (Allocation.LayerInfo->GetBlendMethod() == ELandscapeTargetLayerBlendMethod::FinalWeightBlending);
			LayerNoWeightBlends.Add(!bPerformLegacyWeightBalancing);
		}
...
void ULandscapeComponent::FillLayer(ULandscapeLayerInfoObject* LayerInfo, FLandscapeEditDataInterface& LandscapeEdit)
{
	...
	// TODO [jonathan.bard] : This should always be false now, as we should always use this function in the context of edit layers. Check that it's the case
	//  and look for all places where we use LandscapeEdit.GetEditLayer() (and check that the result !IsValid(), these should all go. We might even be able to 
	//  get rid of FScopedSetLandscapeEditingLayer at long last!)
	...
	// TODO [jonathan.bard] : remove all this once we get rid of non-edit layers, weight balancing at the data level is not reliable and weight blending with edit layers active 
	//  is providing the same service in a non-destructive way
	const bool bPerformLegacyWeightBalancing = (LayerInfo->GetBlendMethod() == ELandscapeTargetLayerBlendMethod::FinalWeightBlending);
	bool bClearOtherWeightBlendLayers = bPerformLegacyWeightBalancing;
	...
	if (FillLayerIdx == INDEX_NONE && bPerformLegacyWeightBalancing)
	{
		FillLayerIdx = ComponentWeightmapLayerAllocations.IndexOfByPredicate(
			[](const FWeightmapLayerAllocationInfo& Allocation) 
			{ 
				return (Allocation.LayerInfo == nullptr) || (Allocation.LayerInfo->GetBlendMethod() == ELandscapeTargetLayerBlendMethod::FinalWeightBlending);
			});
	...
	if (bClearOtherWeightBlendLayers)
	{
		for (int32 LayerIdx = 0; LayerIdx < ComponentWeightmapLayerAllocations.Num(); ++LayerIdx)
		{
			...
			if ((Allocation.LayerInfo == nullptr) || (Allocation.LayerInfo->GetBlendMethod() != ELandscapeTargetLayerBlendMethod::FinalWeightBlending))
			{
				continue;
			}
...
void ULandscapeComponent::ReplaceLayerInternal(ULandscapeLayerInfoObject* FromLayerInfo, ULandscapeLayerInfoObject* ToLayerInfo, FLandscapeEditDataInterface& LandscapeEdit, const FGuid& InEditLayerGUID)
{
	...
	// See if the deleted layer is a NoWeightBlend layer - if not, we don't have to worry about normalization
	const bool bFromLayerIsNoWeightBlend = (FromLayerInfo && (FromLayerInfo->GetBlendMethod() != ELandscapeTargetLayerBlendMethod::FinalWeightBlending));
	const bool bToLayerIsNoWeightBlend = (ToLayerInfo && (ToLayerInfo->GetBlendMethod() != ELandscapeTargetLayerBlendMethod::FinalWeightBlending));
...
void FLandscapeEditDataInterface::SetAlphaData(ULandscapeLayerInfoObject* LayerInfo, const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, const uint8* Data, int32 Stride, ELandscapeLayerPaintingRestriction PaintingRestriction /*= None*/, bool bWeightAdjust /*= true*/, bool bTotalWeightAdjust /*= false*/)
{
		...
		for (int32 ComponentIndexX = ComponentIndexX1; ComponentIndexX <= ComponentIndexX2; ComponentIndexX++)
		{
			...
			for (int32 LayerIdx = 0; LayerIdx < ComponentWeightmapLayerAllocations.Num(); LayerIdx++)
			{
				...
				if (Allocation.LayerInfo != nullptr) // only take into account valid layer
				{
					...
					LayerNoWeightBlends[LayerIdx] = (Allocation.LayerInfo->GetBlendMethod() != ELandscapeTargetLayerBlendMethod::FinalWeightBlending);
					...
					for (int32 SubY = SubY1; SubY <= SubY2; SubY++)
					{
						for (int32 SubX = SubX1; SubX <= SubX2; SubX++)
						{
							...
							if (bWeightAdjust)
							{
								...
								// TODO [jonathan.bard] : this isn't used. TODO: Remove
...
void FLandscapeEditDataInterface::SetAlphaData(ULandscapeLayerInfoObject* LayerInfo, const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, const uint8* Data, int32 Stride, ELandscapeLayerPaintingRestriction PaintingRestriction)
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	SetAlphaData(LayerInfo, X1, Y1, X2, Y2, Data, Stride, PaintingRestriction, /*bWeightAdjust = */true, /*bTotalWeightAdjust = */false);
PRAGMA_ENABLE_DEPRECATION_WARNINGS
}
