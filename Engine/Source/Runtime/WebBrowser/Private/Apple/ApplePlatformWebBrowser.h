...
DECLARE_LOG_CATEGORY_EXTERN(LogAppleWebBrowser, Log, All);

class SAppleWebBrowserWidget;
...
@interface AppleWebViewWrapper : NSObject <WKUIDelegate, WKNavigationDelegate, WKScriptMessageHandler>
#else
@interface AppleWebViewWrapper : NSObject
#endif
{
	SAppleWebBrowserWidget* WebBrowserWidget;
	FTextureRHIRef VideoTexture;
	TWeakPtr<SWindow> Current3DWindow;
...
#if PLATFORM_MAC
@property(strong) NSView* WebViewContainer;
@property(strong) NSView* GameWindowView;
#else
...
enableFloatingCloseButton : (bool)bEnableFloatingCloseButton;
-(void)didRotate;
-(void)close;
...
private:
	...
	*/
	FWebBrowserWindow(FString InUrl, TOptional<FString> InContentsToLoad, bool ShowErrorMessage, bool bThumbMouseButtonNavigation, bool bUseTransparency, bool bInJSBindingToLoweringEnabled, FString InUserAgentApplication, bool bMobileJSReturnInDict);
...
public:
	...
	virtual void SetViewportSize(FIntPoint ViewportWindowSize, FIntPoint WindowPos) override;
	...
	virtual void SetParentDockTab(TSharedPtr<SDockTab> DockTab) override;
...
public:
	...
	*/
	...
	/**
	* Webview floating close button was pressed
	*/
	...
	TWeakPtr<SDockTab> GetParentDockTab();

private:

	TOptional<FSlateRenderTransform> GetWebBrowserRenderTransform() const;
	void RedirectNSEvent();
	
	TSharedPtr<SAppleWebBrowserWidget> BrowserWidget;
	...
	TSharedPtr<FWebView3DInputHandler> WebView3DInput;
	...
	FDelegateHandle ParentDockTabDelegate;
