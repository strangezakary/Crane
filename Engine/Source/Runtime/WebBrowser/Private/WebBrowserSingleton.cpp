	...
	, DefaultTranslucentMaterial(nullptr)
{
	...
	if (bAllowCEF)
	{
		...
		for (int NumRetries = 0; NumRetries <= CEF3_INIT_MAX_RETRIES; ++NumRetries)
		{
			...
			{
#if PLATFORM_WINDOWS
				UE_LOG(LogWebBrowser, Warning, TEXT("Detected concurrent CEF initialization for cache dir %s! Retrying... (%d of %d)"), *CachePath, NumRetries + 1, CEF3_INIT_MAX_RETRIES);
				...
				if (!__FUnloadDelayLoadedDLL2(TCHAR_TO_UTF8(*FPaths::GetCleanFilename(CEFModuleFileName))))
				{
					...
					UE_LOG(LogWebBrowser, Error, TEXT("Unable to cleanly reload CEF DLL, can't retry initialization!"));
					break;
				}
				...
				UE_LOG(LogWebBrowser, Warning, TEXT("Another CEF instance is already running with cache dir %s! WebBrowser module won't be functional."), *CachePath);
				break;
#endif // PLATFORM_WINDOWS
			}
			UE_LOG(LogWebBrowser, Error, TEXT("CEF initialization for cache dir %s failed with exit code %d!"), *CachePath, CefExitCode);
#endif // CEF3_DEFAULT_CACHE
			break;
		}
...
FString FWebBrowserSingleton::GenerateWebCacheFolderName(const FString& InputPath)
{
	...
	for (uint32 CacheNumber = 0; CacheNumber < 32; ++CacheNumber) {
		...
		if (IsCEFLockFileCreated(*LockFile))
		{
			...
			{
				UE_LOG(LogWebBrowser, Warning, TEXT("Found stale CEF lockfile at %s, overwriting"), *LockFile);
				return ConcurrentCachePath;
			}
		}
		else
