...
bool FCEFWebBrowserWindow::CreateInitialTextures()
{
	if (FSlateApplication::IsInitialized())
	{
		if (FSlateRenderer* Renderer = FSlateApplication::Get().GetRenderer())
		{
			...
			{
				if (FCEFWebBrowserWindowRHIHelper::BUseSupportedRHIRenderer() && !RHIRenderHelper)
				{
					RHIRenderHelper.Reset(new FCEFWebBrowserWindowRHIHelper);
...
void FCEFWebBrowserWindow::OnPaint(CefRenderHandler::PaintElementType Type, const CefRenderHandler::RectList& DirtyRects, const void* Buffer, int Width, int Height)
{
	...
	const FIntPoint PaintedBufferSize(Width / ViewportDPIScaleFactor, Height / ViewportDPIScaleFactor);
	bHasCorrectNativeCefBuffer = (PaintedBufferSize == ViewportSize);
	if (CapturedCefBuffer.IsValid())
	{
		// We attempt to capture a good CEF buffer, especially when it is correct. The CEF buffer size does not actually need to match the viewport
		// size to end up being captured, but also may not be captured if it fails another test (which we skip if we know we have the correct CEF
		// buffer.) See code.

		CapturedCefBuffer->SetBufferAsB8G8R8A8(Buffer, Width, Height, ViewportDPIScaleFactor, bHasCorrectNativeCefBuffer);
	}
#endif
...
#if UE_CEF_HAS_RESIZE_BUG
			if (!CapturedCefBuffer.IsValid() && Type == PET_VIEW && !bHasCorrectNativeCefBuffer)
			{
...
#  endif
	...
	bHasCorrectNativeCefBuffer = (DirtyRect == ViewportRect);
	...
	// Fortunately, the DirtyRect for these textures (and only these) has a non-zero X/Y depending on the black bars orientation, so we can detect them without inspecting the content.
	if (DirtyRect.Min.X > 0 || DirtyRect.Min.Y > 0)
	{
		return;
	}
...
bool FCEFWebBrowserWindow::CanSupportAcceleratedPaint()
{
	...
	return FCEFWebBrowserWindowRHIHelper::BUseSupportedRHIRenderer();
#elif PLATFORM_MAC
...
bool FCEFWebBrowserWindow::HasCorrectNativeCefBuffer() const
{
#if UE_CEF_HAS_RESIZE_BUG
	return bHasCorrectNativeCefBuffer;
#else
	return true;
#endif
}

void FCEFWebBrowserWindow::SetUseCapturedCefBuffer(const bool bValue)
{
#if UE_CEF_HAS_RESIZE_BUG && UE_WITH_CAPTURED_CEF_BUFFER
	if (bValue)
	{
		if (!CapturedCefBuffer.IsValid())
		{
			CapturedCefBuffer = MakeShared<FCapturedCefBuffer>();
		}
	}
	else
	{
		CapturedCefBuffer.Reset();
	}
#else
	CapturedCefBuffer.Reset();
#endif
...
bool FCEFWebBrowserWindow::HasResizeBug() const
{
#if UE_CEF_HAS_RESIZE_BUG
	return true;
#else
...
TSharedPtr<FCapturedCefBuffer> FCEFWebBrowserWindow::GetCapturedCefBuffer()
{
	return CapturedCefBuffer; // ..may not be set
}

void FCEFWebBrowserWindow::SetNeedsResize(const bool bInValue)
{
	bNeedsResize = bInValue;
}
