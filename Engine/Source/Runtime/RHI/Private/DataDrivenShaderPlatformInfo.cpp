...
{
	TMap<FName,FText> DeviceProfileToFriendlyName;
...
#define GET_SECTION_STATIC_SHADER_BINDING_LAYOUT_SUPPORT_HELPER(SettingName) \
	...
	GET_SECTION_BOOL_HELPER(bSupportsRayTracingClusterOps);
	...
	GET_SECTION_BOOL_HELPER(bSupportsAdaptiveGBuffer);
...
#undef ADD_PROPERTY_TO_SHADERPLATFORM_FUNCTIONMAP
	...
	FText FriendlyName;
	FTextStringHelper::ReadFromBuffer(*GetSectionString(Section, FName("FriendlyName")), FriendlyName);
	EditorInfo.DeviceProfileToFriendlyName.FindOrAdd(NAME_None) = FriendlyName;
	...
	for (int32 Index = 0; Index < NumDDInfoFiles; Index++)
	{
		...
		for (const TPair<FString, FConfigSection>& Section : AsConst(IniFile))
		{
			if (Section.Key.StartsWith(TEXT("ShaderPlatform ")))
			{
				...
				{
					const TArray<FPreviewPlatformMenuItem>* Items = FDataDrivenPlatformInfoRegistry::GetMenuItemForShaderPlatformName(Infos[ShaderPlatform].Name);
					if (!Items)
					{
						continue;
					}
					...
					for (bool bFirst = true; const FPreviewPlatformMenuItem& Item : *Items)
					{
						if (bFirst)
						{
						...
						{
							PreviewEditorInfo.DeviceProfileToFriendlyName.FindOrAdd(Item.DeviceProfileName) = Item.OptionalFriendlyNameOverride;
						}
						else
						{
							PreviewEditorInfo.DeviceProfileToFriendlyName.FindOrAdd(Item.DeviceProfileName) = GetFriendlyName(PreviewShaderPlatform);
						}
					}
				}
#endif
			...
			else if (Section.Key.StartsWith(TEXT("PreviewShaderPlatform ")))
			{
				...
				const TArray<FPreviewPlatformMenuItem>* Items = FDataDrivenPlatformInfoRegistry::GetMenuItemForShaderPlatformName(Info.Name);
				if (!Items)
				{
					continue;
				}

				for (const FPreviewPlatformMenuItem & Item : *Items)
				{
					...
					{
						PreviewEditorInfo.DeviceProfileToFriendlyName.FindOrAdd(Item.DeviceProfileName) = GetFriendlyName(ShaderPlatform);
					}
				}
			}
#endif
...
#define PREVIEW_FORCE_DISABLE(SettingName) \
	PREVIEW_FORCE_SETTING(SettingName, false)
				...
				PREVIEW_USE_RUNTIME_VALUE(bSupportsBindless);
				PREVIEW_USE_RUNTIME_VALUE(StaticShaderBindingLayoutSupport);
...
FText FGenericDataDrivenShaderPlatformInfo::GetFriendlyName(const FStaticShaderPlatform Platform, FName DeviceProfileName)
{
	...
	TMap<FName, FText>& DeviceProfileToFriendlyName = DataDrivenShaderPlatformInfoEditorOnlyInfos[Platform].DeviceProfileToFriendlyName;
	if (FText* FriendlyName = DeviceProfileToFriendlyName.Find(DeviceProfileName))
	{
		return *FriendlyName;
	}

	return FText::FromName(DeviceProfileName);
}
...
FName FGenericDataDrivenShaderPlatformInfo::GetPreferredShaderCodeFormat(const FStaticShaderPlatform Platform)
{
	return FDataDrivenShaderPlatformInfo::GetIsPreviewPlatform(Platform) ? GRHIGlobals.PreferredPreviewShaderCodeFormat : NAME_None;
}
