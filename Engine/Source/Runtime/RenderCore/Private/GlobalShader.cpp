...
void FGlobalShaderMapId::AppendKeyString(FString& KeyString, const TArray<FShaderTypeDependency>& Dependencies) const
{
	...
	const FName PreferredShaderCodeFormat = FDataDrivenShaderPlatformInfo::GetPreferredShaderCodeFormat(ShaderPlatform);
	if (PreferredShaderCodeFormat != NAME_None)
	{
		KeyString.Append(PreferredShaderCodeFormat.ToString());
		KeyString.AppendChar('_');
	}
