...
bool FApplePlatformStackWalk::ProgramCounterToHumanReadableString( int32 CurrentCallDepth, uint64 ProgramCounter, ANSICHAR* HumanReadableString, SIZE_T HumanReadableStringSize, FGenericCrashContext* Context )
{
	...
	// Get filename.
	{
		...
		FCStringAnsi::StrncatTruncateDest(HumanReadableString, HumanReadableStringSize, FileNameLine);
	}

	// For the crash reporting code this needs a Windows line ending, the caller is responsible for the '\n'
	FCStringAnsi::StrncatTruncateDest(HumanReadableString, HumanReadableStringSize, "\r");
...
int32 FApplePlatformStackWalk::GetProcessModuleSignatures(FStackWalkModuleInfo *ModuleSignatures, const int32 ModuleSignaturesSize)
{
	...
	TArray<FStackWalkModuleInfo> TempModules;
	TempModules.Reserve(ModuleCount);

	// The shared cache breaks our module size calculations. When we sum segment vmsizes from the mach_header,
	// we get inflated sizes because the cache splits segments apart - they're not contiguous. 
	// This causes wrong module attribution when checking address ranges.

	// So we use dladdr() to make it work as _dyld_get_image_header() gives us the header pointer, but
	// for cached frameworks that's not where the code actually lives. dladdr's dli_fbase gives
	// us the real base address and what we need.
	// However the order in which we get these aren't sorted by base address.
	// So we've got to collect them all.
	// Then we calculate size as the distance to the next module instead of summing segments.
	for (int32 ModuleIndex = 0; ModuleIndex < ModuleCount; ModuleIndex++)
	{
		...
		if (Header && ImageName)
		{
			...
			Dl_info DylibInfo;
			if (dladdr((const void*)Header, &DylibInfo) && DylibInfo.dli_fbase)
			{
				Info.BaseOfImage = (uint64)DylibInfo.dli_fbase;
			}
			else
			{
				// Fallback to the busted way in case things go wrong
			...
			TempModules.Add(Info);
		}
	}
	
	TempModules.Sort([](const FStackWalkModuleInfo& A, const FStackWalkModuleInfo& B)
	{
		return A.BaseOfImage < B.BaseOfImage;
	});

	for (int32 Idx = 0; Idx < TempModules.Num(); Idx++)
	{
		if (Idx + 1 < TempModules.Num())
		{
			uint64 CurrentBase = TempModules[Idx].BaseOfImage;
			uint64 NextBase = TempModules[Idx + 1].BaseOfImage;

			TempModules[Idx].ImageSize = (uint32)(NextBase - CurrentBase);
		}
		else
		{
			TempModules[Idx].ImageSize = 0xFFFFFFFF;
		}
	}

	int32 SignatureCount = FMath::Min(TempModules.Num(), ModuleSignaturesSize);
	for (int32 Idx = 0; Idx < SignatureCount; Idx++)
	{
		ModuleSignatures[Idx] = TempModules[Idx];
	}
...
#define HANDLE_CASE(a,b) case a: ErrorString += TEXT(#a #b); break;
	...
	switch (Signal)
	{
		case SIGSEGV:
			...
			ErrorString += FString::Printf(TEXT("0x%016" UINT64_x_FMT), (uint64)(uint32*)Info->si_addr);
			break;
		case SIGBUS:
