...
public:
	...
	UE_DEPRECATED(5.7, "This property will be removed since the ClothingSimulationFactory is no longer set per component.")
	UPROPERTY()
...
public:
	...
	UE_DEPRECATED(5.7, "Use GetClothingSimulationInstances instead.")
	ENGINE_API const class IClothingSimulation* GetClothingSimulation() const;

	/** Get the current clothing simulation (read/write) */
	UE_DEPRECATED(5.7, "Use GetClothingSimulationInstances instead.")
	ENGINE_API class IClothingSimulation* GetClothingSimulation();

	/** Get the current clothing simulation context (read only) */
	UE_DEPRECATED(5.7, "Use GetClothingSimulationInstances instead.")
	ENGINE_API const class IClothingSimulationContext* GetClothingSimulationContext() const;
	
	/** Get the current clothing simulation context (read/write) */
	ENGINE_API class IClothingSimulationContext* GetClothingSimulationContext();
	...
	{
		return ClothingSimulationInstances;
	}
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = Clothing)
	ENGINE_API UClothingSimulationInteractor* GetClothingSimulationInteractor(const FName ClothingAssetName = NAME_None) const;
...
private:
	...
	/** Clothing simulation instance. */
	UPROPERTY(Transient, NonTransactional)
	TArray<FClothingSimulationInstance> ClothingSimulationInstances;
...
public:
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="Physics")
	ENGINE_API void SetEnablePerPolyCollision(bool bInEnablePerPolyCollision);
...
public:
	...
	*/
	...
	ENGINE_API void InstantiatePhysicsAssetBodiesRefPose(const UPhysicsAsset& PhysAsset, TArray<FBodyInstance*>& OutBodies, FPhysScene* PhysScene = nullptr, USkeletalMeshComponent* OwningComponent = nullptr, int32 UseRootBodyIndex = INDEX_NONE, const FPhysicsAggregateHandle& UseAggregate = FPhysicsAggregateHandle()) const;
...
protected:
	...
	ENGINE_API void FindClothCollisions(FClothCollisionData& OutCollisions) const;
...
private:
	...
	friend class UE::UAF::FDataRegistry;
