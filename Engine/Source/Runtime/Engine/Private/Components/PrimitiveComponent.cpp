...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PrimitiveComponent)
...
namespace PrimitiveComponentCVars
{
	...
	static TAutoConsoleVariable<bool> CVarDisplayExcludeFromHLODLevelsProperty(
		TEXT("p.DisplayExcludeFromHLODLevelsProperty"),
		false,
		TEXT("Enable display of the legacy UPrimitiveComponent::ExcludeFromHLODLevels property in the editor."));
}
...
void UPrimitiveComponent::GetUsedTextures(TArray<UTexture*>& OutTextures, EMaterialQualityLevel::Type QualityLevel)
{
	...
	for( int32 MatIndex = 0; MatIndex < UsedMaterials.Num(); ++MatIndex )
	{
		...
		if( UsedMaterials[ MatIndex ] )
		{
			...
			UsedTextures.Reset();
			UsedMaterials[MatIndex]->GetUsedTextures(UsedTextures, QualityLevel, World ? GetFeatureLevelShaderPlatform_Checked(World->GetFeatureLevel()) : GetMaxShaderPlatformChecked());
...
void UPrimitiveComponent::OnCreatePhysicsState()
{
	...
	if(!BodyInstance.IsValidBodyInstance())
	{
		...
		if(BodySetup)
		{
			...
			{
				UE_LOG(LogPhysics, Warning, TEXT("Scale for %s has a component set to zero, which will result in a bad body instance. Scale:%s Package:%s"), *GetPathNameSafe(this), *BodyScale.ToString(), *GetPackage()->GetName());
...
void UPrimitiveComponent::RecreateInstanceBody(int32 InstanceBodyIndex)
{ 
	RecreatePhysicsState();
}
...
bool UPrimitiveComponent::ShouldComponentAddToScene() const
{
	...
	return bSceneAdd && (ShouldRender() || (CastShadow && bCastHiddenShadow && !bIsHiddenInEditor) || bAffectIndirectLightingWhileHidden || bRayTracingFarField);
}
	...
	if (BodyInstance.bSimulatePhysics)
	{
		if(UWorld* World = GetWorld())
		{
			if(World->IsGameWorld())
			{
				...
				{
					UE_LOG(LogPhysics, Warning, TEXT("Invalid Simulate Options: Body (%s) is set to simulate physics but Collision Enabled is incompatible"), *GetReadableName());
				}
			}
		}
	}
...
{
	return bEnableAutoLODGeneration && (!UWorld::IsPartitionedWorld(GetWorld()) || PrimitiveComponentCVars::CVarDisplayExcludeFromHLODLevelsProperty.GetValueOnAnyThread());
}
...
struct FPSOPrecacheFinishedTask
{
	...
	void DoTask(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
	{
		if (UPrimitiveComponent* PC = WeakPrimitiveComponent.Get())
		{
			...
			if (PC->GetWorld())
			{
				PC->GetWorld()->AddPrimitiveComponentPSOPrecacheCompleted(PC);
			}
		}
	}
...
FPrimitiveMaterialPropertyDescriptor UPrimitiveComponent::GetUsedMaterialPropertyDesc(EShaderPlatform InShaderPlatform) const
{
...
void UPrimitiveComponent::ComputeHLODHash(FHLODHashBuilder& HashBuilder) const
{
	...
	HashBuilder.HashField(GetComponentTransform(), TEXT("TransformHash"));
	HashBuilder.HashField(HLODBatchingPolicy, GET_MEMBER_NAME_CHECKED(UPrimitiveComponent, HLODBatchingPolicy));
	HashBuilder.HashField(CastShadow, GET_MEMBER_NAME_CHECKED(UPrimitiveComponent, CastShadow));
	HashBuilder.HashField(bCastStaticShadow, GET_MEMBER_NAME_CHECKED(UPrimitiveComponent, bCastStaticShadow));
	HashBuilder.HashField(bCastDynamicShadow, GET_MEMBER_NAME_CHECKED(UPrimitiveComponent, bCastDynamicShadow));
	HashBuilder.HashField(bCastFarShadow, GET_MEMBER_NAME_CHECKED(UPrimitiveComponent, bCastFarShadow));

	if (!GetCustomPrimitiveData().Data.IsEmpty())
	{
		HashBuilder.HashField(GetCustomPrimitiveData(), TEXT("CustomPrimitiveData"));
	}
...
void UPrimitiveComponent::ReleaseSceneProxy()
{
	...
	// Leave SceneData.OwnerLastRenderTimePtr valid. It might still be referenced on the rendering thread.
	SceneProxy = nullptr;
...
{
	FStreamingTextureLevelContext LevelContext{GetCurrentMaterialQualityLevelChecked(),UPrimitiveComponent::GetPrimitiveComponent(this)};
