...
const FMaterial& FMaterialInstanceResource::GetMaterialWithFallback(ERHIFeatureLevel::Type InFeatureLevel, const FMaterialRenderProxy*& OutFallbackMaterialRenderProxy) const
{
	...
	if (Parent)
	{
		if (Owner->bHasStaticPermutationResource)
		{
			...
			FMaterialResource* StaticPermutationResource = FindMaterialResource(Owner->StaticPermutationMaterialResources, GetFeatureLevelShaderPlatform_Checked(InFeatureLevel), ActiveQualityLevel, true);
...
const FMaterial* FMaterialInstanceResource::GetMaterialNoFallback(ERHIFeatureLevel::Type InFeatureLevel) const
{
	...
	if (Parent)
	{
		...
		{
			EMaterialQualityLevel::Type ActiveQualityLevel = GetCachedScalabilityCVars().MaterialQualityLevel;
			const FMaterialResource* StaticPermutationResource = FindMaterialResource(Owner->StaticPermutationMaterialResources, GetFeatureLevelShaderPlatform_Checked(InFeatureLevel), ActiveQualityLevel, true);
...
bool FMaterialInstanceResource::GetParameterValue(EMaterialParameterType Type, const FHashedMaterialParameterInfo& ParameterInfo, FMaterialParameterValue& OutValue, const FMaterialRenderContext& Context) const
{
	...
	if (!bResult)
	{
		...
		switch (Type)
		{
			...
			case EMaterialParameterType::ParameterCollection: bResult = RenderThread_GetParameterValue<const UMaterialParameterCollection*>(ParameterInfo, OutValue); break;
...
void FMaterialInstanceResource::InitMIParameters(FMaterialInstanceParameterSet& ParameterSet)
{
	...
	ParameterSet.ParameterCollectionParameters.Sort(SortMaterialInstanceParametersPredicate<const UMaterialParameterCollection*>);
	...
	ParameterCollectionParameterArray.Array = MoveTemp(ParameterSet.ParameterCollectionParameters);
	...
	ParameterCollectionParameterArray.HashAddAllItems();
}

void FMaterialInstanceResource::EvaluateParameterCollections(FRHICommandListBase& RHICmdList, FUniformExpressionCache& OutUniformExpressionCache, const FMaterialRenderContext& Context, FUniformExpressionCacheAsyncUpdater* Updater) const
{
	FMaterialRenderProxy::EvaluateParameterCollections(RHICmdList, OutUniformExpressionCache, Context, Updater);

	for (FGuid& CollectionStateId : OutUniformExpressionCache.ParameterCollections)
	{
		for (auto&& Entry : ParameterCollectionParameterArray.Array)
		{
			if (Entry.Value && Entry.Value->IsInstanceOf(CollectionStateId))
			{
				CollectionStateId = Entry.Value->StateId;
				break;
			}
		}
	}
...
bool UMaterialParameterCollection::IsInstanceOf(const FGuid& CollectionStateId) const
{
	for (const UMaterialParameterCollection* BaseCollection = this; BaseCollection; BaseCollection = BaseCollection->Base)
	{
		if (BaseCollection->StateId == CollectionStateId)
		{
			return true;
		}
	}
...
void UMaterialInstance::SwapLayerParameterIndices(int32 OriginalIndex, int32 NewIndex)
{
	if (OriginalIndex != NewIndex)
	{
		...
		SwapLayerParameterIndicesArray(ParameterCollectionParameterValues, OriginalIndex, NewIndex);
...
void UMaterialInstance::RemoveLayerParameterIndex(int32 Index)
{
	...
	RemoveLayerParameterIndicesArray(ParameterCollectionParameterValues, Index);
...
bool UMaterialInstance::UpdateParameters()
{
	...
	if(IsTemplate(RF_ClassDefaultObject)==false && EditorOnly)
	{
		...
		if(ParentMaterial)
		{
			...
			bDirty = UpdateParameterSet<FParameterCollectionParameterValue, UMaterialExpressionCollectionParameter>(ParameterCollectionParameterValues, ParentMaterial) || bDirty;
		...
		if (StaticParametersRuntime.bHasMaterialLayers && Parent)
		{
			...
			if (Parent->GetMaterialLayers(ParentLayers))
			{
					...
					RemapLayerIndices))
				{
					...
					RemapLayerParameterIndicesArray(ParameterCollectionParameterValues, RemapLayerIndices);
...
void GameThread_InitMIParameters(const UMaterialInstance& Instance)
{
	...
	ParameterSet.ParameterCollectionParameters.Reserve(Instance.ParameterCollectionParameterValues.Num());
	for (const FParameterCollectionParameterValue& Parameter : Instance.ParameterCollectionParameterValues)
	{
		auto& ParamRef = ParameterSet.ParameterCollectionParameters.AddDefaulted_GetRef();
		ParamRef.Info = Parameter.ParameterInfo;
		ParamRef.Value = FParameterCollectionParameterValue::GetValue(Parameter);
	}
...
void UMaterialInstance::GetUsedTextures(TArray<UTexture*>& OutTextures, TOptional<EMaterialQualityLevel::Type> InSpecificQualityLevel, TOptional<EShaderPlatform> InSpecificShaderPlatform) const
{
	OutTextures.Empty();
	...
	if (!FPlatformProperties::IsServerOnly())
	{
		...
		if (BaseMaterial && !BaseMaterial->IsDefaultMaterial())
		{
			...
			for (int32 QualityLevelIndex = QualityLevelRange.GetLowerBoundValue(); QualityLevelIndex <= QualityLevelRange.GetUpperBoundValue(); ++QualityLevelIndex)
			{
				for (int32 ShaderPlatformIndex = ShaderPlatformRange.GetLowerBoundValue(); ShaderPlatformIndex <= ShaderPlatformRange.GetUpperBoundValue(); ++ShaderPlatformIndex)
				{
					...
					const FMaterialResource* MaterialResource = MaterialToUse->GetMaterialResource((EShaderPlatform)ShaderPlatformIndex, (EMaterialQualityLevel::Type)QualityLevelIndex);
					if (MaterialResource)
					{
		...
		else
		{
			...
			UMaterial::GetDefaultMaterial(MD_Surface)->GetUsedTextures(OutTextures, InSpecificQualityLevel, InSpecificShaderPlatform);
		}
	}
}
...
void UMaterialInstance::LogMaterialsAndTextures(FOutputDevice& Ar, int32 Indent) const
{
	...
	else
	{
		...
		{
			const FMaterialResource* MaterialResource = FindMaterialResource(MaterialInstanceToUse->StaticPermutationMaterialResources, GetFeatureLevelShaderPlatform_Checked(FeatureLevel), QualityLevel, true);
			if (MaterialResource)
			{
...
{
	return ValidateTextureOverrides(GetFeatureLevelShaderPlatform_Checked(InFeatureLevel));
}

void UMaterialInstance::ValidateTextureOverrides(EShaderPlatform InShaderPlatform) const
{
	...
	const FMaterialResource* CurrentResource = Material->GetMaterialResource(InShaderPlatform);
...
void UMaterialInstance::GetUsedTexturesAndIndices(TArray<UTexture*>& OutTextures, TArray< TArray<int32> >& OutIndices, EMaterialQualityLevel::Type QualityLevel, EShaderPlatform InShaderPlatform) const
{
	OutTextures.Empty();
	OutIndices.Empty();
	...
	if (!FPlatformProperties::IsServerOnly())
	{
		...
		{
			const FMaterialResource* CurrentResource = FindMaterialResource(MaterialInstanceToUse->StaticPermutationMaterialResources, InShaderPlatform, QualityLevel, true);
			if (CurrentResource)
			{
		...
		{ 
			...
			if (Material)
			{
				const FMaterialResource* MaterialResource = Material->GetMaterialResource(InShaderPlatform, QualityLevel);
				if( MaterialResource )
				{
			...
			{
				UMaterial::GetDefaultMaterial(MD_Surface)->GetUsedTexturesAndIndices(OutTextures, OutIndices, QualityLevel, InShaderPlatform);
			}
		}
	}
}
...
void UMaterialInstance::OverrideTexture(const UTexture* InTextureToOverride, UTexture* OverrideTexture, ERHIFeatureLevel::Type InFeatureLevel)
{
	...
	{
		SourceMaterialResource = GetMaterialResource(GetFeatureLevelShaderPlatform_Checked(InFeatureLevel));
	}
	else
	{
		...
		SourceMaterialResource = Material->GetMaterialResource(GetFeatureLevelShaderPlatform_Checked(InFeatureLevel));
	}
...
extern bool ShouldRecacheMaterialExpressions(const FMaterialResource* Resource, const FHashedMaterialParameterInfo& ParameterInfo);

void UMaterialInstance::SetNumericParameterDefault(EMaterialParameterType Type, const FHashedMaterialParameterInfo& ParameterInfo, const UE::Shader::FValue& Value, EShaderPlatform InShaderPlatform)
{
#if WITH_EDITOR
	if (bHasStaticPermutationResource)
	{
		FMaterialResource* SourceMaterialResource = GetMaterialResource(InShaderPlatform);
		if (SourceMaterialResource)
		{
			SourceMaterialResource->TransientOverrides.SetNumericOverride(Type, ParameterInfo, Value);
			if (ShouldRecacheMaterialExpressions(SourceMaterialResource, ParameterInfo))
			{
				RecacheUniformExpressions(false);
			}
		}
...
void UMaterialInstance::ClearNumericParameterDefault(EMaterialParameterType Type, const FHashedMaterialParameterInfo& ParameterInfo, EShaderPlatform InShaderPlatform)
{
#if WITH_EDITOR
	if (bHasStaticPermutationResource)
	{
		...
		if (SourceMaterialResource)
		{
			SourceMaterialResource->TransientOverrides.ClearNumericOverride(Type, ParameterInfo);
			if (ShouldRecacheMaterialExpressions(SourceMaterialResource, ParameterInfo))
			{
				RecacheUniformExpressions(false);
			}
...
{
	return CopyScalarAndVectorParameters(SourceMaterialToCopyFrom, GetFeatureLevelShaderPlatform_Checked(FeatureLevel));
}

void UMaterialInstanceDynamic::CopyScalarAndVectorParameters(const UMaterialInterface& SourceMaterialToCopyFrom, EShaderPlatform ShaderPlatform)
{
	check(IsInGameThread());
	...
	FMaterialResource* MaterialResource = GetMaterialResource(ShaderPlatform);
...
FMaterialResource* UMaterialInstance::GetMaterialResource(EShaderPlatform InShaderPlatform, EMaterialQualityLevel::Type QualityLevel)
{
	if (bHasStaticPermutationResource)
	{
		if (QualityLevel == EMaterialQualityLevel::Num)
		...
		return FindMaterialResource(StaticPermutationMaterialResources, InShaderPlatform, QualityLevel, true);
	}

	//there was no static permutation resource
	return Parent ? Parent->GetMaterialResource(InShaderPlatform, QualityLevel) : nullptr;
}
...
const FMaterialResource* UMaterialInstance::GetMaterialResource(EShaderPlatform InShaderPlatform, EMaterialQualityLevel::Type QualityLevel) const
{
	return const_cast<UMaterialInstance*>(this)->GetMaterialResource(InShaderPlatform, QualityLevel);
}
...
void UMaterialInstance::GetAllParametersOfType(EMaterialParameterType Type, TMap<FMaterialParameterInfo, FMaterialParameterMetadata>& OutParameters) const
{
	...
	for (int32 Index = 0; Index < InstanceChain.MaterialInstances.Num(); ++Index)
	{
		...
		case EMaterialParameterType::ParameterCollection: GameThread_ApplyParameterOverrides(Instance->ParameterCollectionParameterValues, LayerIndexRemap, bSetOverride, OverridenParameters, OutParameters); break;
#if WITH_EDITORONLY_DATA
		case EMaterialParameterType::StaticComponentMask:
...
void UMaterialInstance::CacheResourceShadersForRendering(EMaterialShaderPrecompileMode PrecompileMode, FMaterialResourceDeferredDeletionArray& OutResourcesToFree)
{
	...
	if (bHasStaticPermutationResource && FApp::CanEverRender())
	{
		...
		while (FeatureLevelsToCompile != 0)
		{
			...
			FMaterialResource* CurrentResource = FindOrCreateMaterialResource(StaticPermutationMaterialResources, BaseMaterial, this, ShaderPlatform, ActiveQualityLevel);
	...
	if (bHasStaticPermutationResource)
	{
		...
		for (int32 QualityLevelIndex = 0; QualityLevelIndex < EMaterialQualityLevel::Num; QualityLevelIndex++)
		{
			...
			if (QualityLevelsUsed[QualityLevelIndex])
			{
				...
				NewResource->SetMaterial(BaseMaterial, this, ShaderPlatform, (EMaterialQualityLevel::Type)QualityLevelIndex);
		...
		{
			FMaterialResource* NewResource = AllocatePermutationResource();
			NewResource->SetMaterial(BaseMaterial, this, ShaderPlatform);
			NewResourcesToCache.Add(NewResource);
		}
		...
		OutCachedMaterialResources.Reserve(NewResourcesToCache.Num() + OutCachedMaterialResources.Num());
...
FGraphEventArray UMaterialInstance::PrecachePSOs(const FPSOPrecacheVertexFactoryDataList& VertexFactoryDataList, const FPSOPrecacheParams& InPreCacheParams, EPSOPrecachePriority Priority, TArray<FMaterialPSOPrecacheRequestID>& OutMaterialPSORequestIDs)
{
	...
	if (FApp::CanEverRender()  && (PipelineStateCache::IsPSOPrecachingEnabled() || IsPSOShaderPreloadingEnabled()))
	{
		...
		{			
			...
			{
				const ERHIFeatureLevel::Type FeatureLevel = (ERHIFeatureLevel::Type)FBitSet::GetAndClearNextBit(FeatureLevelsToCompile);
				FMaterialResource* StaticPermutationResource = FindMaterialResource(StaticPermutationMaterialResources, GetFeatureLevelShaderPlatform_Checked(FeatureLevel), ActiveQualityLevel, true/*bAllowDefaultMaterial*/);
				if (StaticPermutationResource)
				{
					GraphEvents.Append(StaticPermutationResource->CollectPSOs(VertexFactoryDataList, InPreCacheParams, Priority, OutMaterialPSORequestIDs));
				}
			}
		}
		else if (Parent)
...
void UMaterialInstance::CacheGivenTypesForCooking(EShaderPlatform ShaderPlatform, ERHIFeatureLevel::Type FeatureLevel, EMaterialQualityLevel::Type QualityLevel, const TArray<const FVertexFactoryType*>& VFTypes, const TArray<const FShaderPipelineType*> PipelineTypes, const TArray<const FShaderType*>& ShaderTypes)
{
	...
	if (bHasStaticPermutationResource)
	{
		...
		FMaterialResource* CurrentResource = FindOrCreateMaterialResource(StaticPermutationMaterialResources, BaseMaterial, this, ShaderPlatform, QualityLevel);
		check(CurrentResource);
...
bool UMaterialInstance::IsComplete() const
{
	...
	if (bHasStaticPermutationResource && FApp::CanEverRender())
	{
		...
		while (FeatureLevelsToCompile != 0)
		{
			...
			FMaterialResource* CurrentResource = FindMaterialResource(StaticPermutationMaterialResources, ShaderPlatform, ActiveQualityLevel, true);
...
void UMaterialInstance::Serialize(FArchive& Ar)
{
	...
	{
		ValidateTextureOverrides(GMaxRHIShaderPlatform);
	}
}

void UMaterialInstance::PostDuplicate(bool bDuplicateForPIE)
{
...
void UMaterialInstance::PostLoad()
{
	...
	// We have to make sure the resources are created for all used parameter collections.
	for (int32 ValueIndex = 0; ValueIndex < ParameterCollectionParameterValues.Num(); ValueIndex++)
	{
		// Make sure the collection is postloaded so the resource isn't null.
		UMaterialParameterCollection* Collection = ParameterCollectionParameterValues[ValueIndex].ParameterValue;
		if (Collection)
		{
...
bool UMaterialInstance::IsStaticPermutationAllowedForCandidateParent(UMaterialInterface* CandidateParent) const
{
	...
	// If parent is a material instance, we need to verify that itself allows for shader permutations to protect against
	// the situation where the user copies locally a material instance that wouldn't otherwise allow for permutations
	// and parents a new instance against the copy.
	UMaterialInstance* ParentAsInstance = Cast<UMaterialInstance>(CandidateParent);
	if (ParentAsInstance && !IsStaticPermutationAllowedForCandidateParent(ParentAsInstance->Parent))
	{
		UE_LOG(LogMaterial, Warning,
			   TEXT("MaterialInstance '%s' cannot allow static parameter permutations because its parent '%s' ultimately derives from a restricted material."),
			   *this->GetName(),
			   *CandidateParent->GetName());
	}
...
void UMaterialInstance::ValidateStaticPermutationAllowed()
{
	...
	// If static switches are disabled, clear any latent existing switche
	if (bDisallowStaticParameterPermutations)
	{
...
void UMaterialInstance::ReserveParameterValuesInternal(EMaterialParameterType Type, int32 Capacity)
{
	...
	case EMaterialParameterType::ParameterCollection: ParameterCollectionParameterValues.Reserve(Capacity); break;
...
void UMaterialInstance::AddParameterValueInternal(const FMaterialParameterInfo& ParameterInfo, const FMaterialParameterMetadata& Meta, EMaterialSetParameterValueFlags Flags)
{
	...
	case EMaterialParameterType::ParameterCollection: ParameterCollectionParameterValues.Emplace(ParameterInfo, Value.ParameterCollection); break;
...
void UMaterialInstance::SetParameterValueInternal(const FMaterialParameterInfo& ParameterInfo, const FMaterialParameterMetadata& Meta, EMaterialSetParameterValueFlags Flags)
{
	...
	case EMaterialParameterType::ParameterCollection: SetParameterCollectionParameterValueInternal(ParameterInfo, Value.ParameterCollection); break;
	default: checkNoEntry();
	}
}
...
void UMaterialInstance::SetParameterCollectionParameterValueInternal(const FMaterialParameterInfo& ParameterInfo, UMaterialParameterCollection* Value)
{
	LLM_SCOPE(ELLMTag::MaterialInstance);
	...
	FParameterCollectionParameterValue* ParameterValue = GameThread_FindParameterByName(ParameterCollectionParameterValues, ParameterInfo);

	bool bForceUpdate = false;
	if(!ParameterValue)
	{
		// If there's no element for the named parameter in array yet, add one.
		ParameterValue = new(ParameterCollectionParameterValues) FParameterCollectionParameterValue;
		ParameterValue->ParameterInfo = ParameterInfo;
		ParameterValue->ExpressionGUID.Invalidate();
		bForceUpdate = true;
	}
	...
	if (bForceUpdate || ParameterValue->ParameterValue != Value)
	{
		// set as an ensure, because it is somehow possible to accidentally pass non-parameters into here via blueprints...
		if (Value && ensureMsgf(Value->IsA(UMaterialParameterCollection::StaticClass()), TEXT("Expecting a UMaterialParameterCollection! Value='%s' class='%s'"), *Value->GetName(), *Value->GetClass()->GetName()))
		{
			ParameterValue->ParameterValue = Value;
			Value->AddToCluster(this, true);
			// Update the material instance data in the rendering thread.
...
void UMaterialInstance::ClearParameterValuesInternal(EMaterialInstanceClearParameterFlag Flags)
{
	...
	if (EnumHasAnyFlags(Flags, EMaterialInstanceClearParameterFlag::Numeric))
	{
		...
		ParameterCollectionParameterValues.Empty();
...
TSoftObjectPtr<UObject> UMaterialInstance::GetEnumerationObject(int32 Index) const
{
...
void UMaterialInstance::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	// To avoid unnecessary work, don't apply to child instances and RVT during interactive edits.
...
void UMaterialInstance::GetResourceSizeEx(FResourceSizeEx& CumulativeResourceSize)
{
	...
	if (Resource)
	{
		...
		CumulativeResourceSize.AddDedicatedSystemMemoryBytes(ParameterCollectionParameterValues.Num() * sizeof(THashedMaterialParameterMap<const UMaterialParameterCollection*>::TNamedParameter));

		// Record space for hash tables as well..
		if (ScalarParameterValues.Num()) CumulativeResourceSize.AddDedicatedSystemMemoryBytes(FDefaultSparseSetAllocator::GetNumberOfHashBuckets(ScalarParameterValues.Num()) * sizeof(uint16));
		if (VectorParameterValues.Num()) CumulativeResourceSize.AddDedicatedSystemMemoryBytes(FDefaultSparseSetAllocator::GetNumberOfHashBuckets(VectorParameterValues.Num()) * sizeof(uint16));
		if (DoubleVectorParameterValues.Num()) CumulativeResourceSize.AddDedicatedSystemMemoryBytes(FDefaultSparseSetAllocator::GetNumberOfHashBuckets(DoubleVectorParameterValues.Num()) * sizeof(uint16));
		if (TextureParameterValues.Num()) CumulativeResourceSize.AddDedicatedSystemMemoryBytes(FDefaultSparseSetAllocator::GetNumberOfHashBuckets(TextureParameterValues.Num()) * sizeof(uint16));
		if (RuntimeVirtualTextureParameterValues.Num()) CumulativeResourceSize.AddDedicatedSystemMemoryBytes(FDefaultSparseSetAllocator::GetNumberOfHashBuckets(RuntimeVirtualTextureParameterValues.Num()) * sizeof(uint16));
		if (SparseVolumeTextureParameterValues.Num()) CumulativeResourceSize.AddDedicatedSystemMemoryBytes(FDefaultSparseSetAllocator::GetNumberOfHashBuckets(SparseVolumeTextureParameterValues.Num()) * sizeof(uint16));
		if (FontParameterValues.Num()) CumulativeResourceSize.AddDedicatedSystemMemoryBytes(FDefaultSparseSetAllocator::GetNumberOfHashBuckets(FontParameterValues.Num()) * sizeof(uint16));
		if (ParameterCollectionParameterValues.Num()) CumulativeResourceSize.AddDedicatedSystemMemoryBytes(FDefaultSparseSetAllocator::GetNumberOfHashBuckets(ParameterCollectionParameterValues.Num()) * sizeof(uint16));
	}
}
...
USubsurfaceProfile* UMaterialInstance::GetSubsurfaceProfileRoot_Internal() const
{
	// Return the subsurface profile form the root material to create named parameters.
	checkSlow(IsInGameThread());
	return Parent ? Parent->GetSubsurfaceProfileRoot_Internal() : 0;
}
...
{
	// Recursively look for override among the ancestors
	USpecularProfile* Out = nullptr;
	if (bOverrideSpecularProfile && SpecularProfileOverride)
	{
		Out = SpecularProfileOverride;
	}
	else if (Parent)
	{
		Out = Parent->GetSpecularProfileOverride_Internal();
	}
	return Out;
}
...
void UMaterialInstance::OnCookEvent(UE::Cook::ECookEvent CookEvent, UE::Cook::FCookEventContext& CookContext)
{
	...
	if (CookEvent == UE::Cook::ECookEvent::PlatformCookDependencies && CookContext.IsCooking())
	{
		...
		UMaterialInterface* EffectiveParent = Parent ? Parent.Get() : UMaterial::GetDefaultMaterial(MD_Surface);
...
bool UMaterialInstance::Equivalent(const UMaterialInstance* CompareTo) const
{
	...
	if (!CompareValueArraysByExpressionGUID(ParameterCollectionParameterValues, CompareTo->ParameterCollectionParameterValues))
	{
		return false;
	}
...
void UMaterialInstance::GetShaderTypes(EShaderPlatform Platform, const ITargetPlatform* TargetPlatform, TArray<FDebugShaderTypeInfo>& OutShaderInfo)
{
	if (bHasStaticPermutationResource)
	{
		...
		while (FeatureLevelsToCompile != 0)
		{
			...
			// In cooked build, there is no shader compilation but this is still needed to
			// register the loaded shadermap
			FMaterialResource* CurrentResource = FindOrCreateMaterialResource(StaticPermutationMaterialResources, BaseMaterial, this, Platform, ActiveQualityLevel);
			check(CurrentResource);
		}
		...
		for (FMaterialResource* CurrentResource : StaticPermutationMaterialResources)
		{
			CurrentResource->GetShaderTypes(LayoutParams, OutShaderInfo);
		}
	}
}
#endif // WITH_EDITOR
	...
	if (!FPlatformProperties::IsServerOnly())
	{
		...
		for (int Index = Hierarchy.Num() - 1; Index >= 0; --Index)
		{
			...
			if (UMaterialInstance* AsInstance = Cast<UMaterialInstance>(Interface))
			{
				...
				MergeParameterOverrides(ParameterCollectionParameterValues, AsInstance->ParameterCollectionParameterValues);
				// No fonts?
			}
			else if (UMaterial* AsMaterial = Cast<UMaterial>(Interface))
			{
				...
				checkSlow(ParameterCollectionParameterValues.Num() == 0);

				const FMaterialResource* MaterialResource = nullptr;
				if (UWorld* World = AsMaterial->GetWorld())
				{
					MaterialResource = AsMaterial->GetMaterialResource(GetFeatureLevelShaderPlatform_Checked(World->GetFeatureLevel()));
				}

				if (!MaterialResource)
				{
					MaterialResource = AsMaterial->GetMaterialResource(GMaxRHIShaderPlatform);
				}
