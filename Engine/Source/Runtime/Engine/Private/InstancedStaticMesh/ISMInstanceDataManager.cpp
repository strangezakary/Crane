...
void FPrimitiveInstanceDataManager::InitChangeSet(const FChangeDesc &ChangeDesc, const FInstanceUpdateComponentDesc &ComponentData, FISMInstanceUpdateChangeSet &ChangeSet)
{
	...
	ChangeSet.SetSharedLocalBounds(ComponentData.StaticMeshBounds);
}
...
bool FPrimitiveInstanceDataManager::FlushChanges(FInstanceUpdateComponentDesc &&ComponentData, bool bNewPrimitiveProxy)
{
	...
	if (LegacyBuildData)
	{
		...
		DispatchInstanceDataUpdateTask(bIsUnattached, Proxy, InstanceDataBufferHeader, [ExternalChangeSet = MoveTemp(ExternalChangeSet), Proxy = Proxy] () mutable 
		{
	...
	// Init previous LocalToWorld to current on inital update
	FMatrix PrevPrimitiveLocalToWorld = GetState() == ETrackingState::Initial ? ComponentData.PrimitiveLocalToWorld : PrimitiveLocalToWorld;
	...
	// 
	{
		...
		FISMInstanceUpdateChangeSet ChangeSet(bNeedFullUpdate, MoveTemp(InstanceUpdateTracker), ComponentData.NumProxyInstances);
		...
		if (Mode == EMode::ExternalLegacyData && ChangeDesc.bPrimitiveTransformChanged)
		{
			...
			DispatchInstanceDataUpdateTask(bIsUnattached, Proxy, InstanceDataBufferHeader, [ChangeSet = MoveTemp(ChangeSet), Proxy = Proxy, NumHeaderInstances = InstanceDataBufferHeader.NumInstances] () mutable 
			{
...
bool FPrimitiveInstanceDataManager::ShouldUsePrecomputed()
{
...
// Calling this early can save performance if no features are enabled that require BeginCacheForCookedPlatformData to be called.  Specifically, that function
// does no work if this function returns false.  If additional features require BeginCacheForCookedPlatformData to be called, or logic is added outside the
// "bShouldBuild" condition in that function, this would need to be revisited.
//
// The big advantage of this early out is that unnecessarily computing the FInstanceUpdateComponentDesc argument to BeginCacheForCookedPlatformData can
// have catastrophic effects on cook performance, because it forces an early flush of the static mesh compilation queue where GetUsedMaterials() is called,
// reducing parallelism.  On a test project, this optimization saves 11% in overall cook time following a StaticMesh version bump.
...
FISMPrecomputedSpatialHashData FPrimitiveInstanceDataManager::PrecomputeOptimizationData(FInstanceUpdateComponentDesc &&ComponentData, TStridedView<FMatrix> InstanceTransforms)
{
	...
	SortBuilder.BuildOptimizedSpatialHashOrder(InstanceTransforms.Num(), MinLevel, 
