...
void FD3D12MemoryPool::Init()
{
	...
	{
		{
			...
			VERIFYD3D12RESULT(Adapter->CreateBuffer(
				HeapProps,
				GetGPUMask(),
				InitConfig.InitialD3D12Access,
				ED3D12ResourceStateMode::SingleState,
				InitConfig.InitialD3D12Access,
				PoolSize,
				BackingResource.GetInitReference(),
				TEXT("Resource PoolAllocator Underlying Buffer"),
				InitConfig.ResourceFlags));
	...
	if (IsGPUOnly(InitConfig.HeapType)
		&& EnumHasAnyFlags(InitConfig.InitialD3D12Access, ED3D12Access::BVHRead | ED3D12Access::BVHWrite))
	{
...
FD3D12ResourceInitConfig FD3D12PoolAllocator::GetResourceAllocatorInitConfig(D3D12_HEAP_TYPE InHeapType, D3D12_RESOURCE_FLAGS InResourceFlags, EBufferUsageFlags InBufferUsage)
{
	...
	if (EnumHasAnyFlags(InBufferUsage, BUF_AccelerationStructure))
	{
		...
		InitConfig.InitialD3D12Access = ED3D12Access::BVHRead | ED3D12Access::BVHWrite;
	}
	else
#endif // D3D12_RHI_RAYTRACING
	if (InitConfig.HeapType == D3D12_HEAP_TYPE_READBACK)
	{
		InitConfig.InitialD3D12Access = ED3D12Access::CopyDest;
	}
	else if (EnumHasAnyFlags(InBufferUsage, BUF_UnorderedAccess))
	{
		...
		InitConfig.InitialD3D12Access = ED3D12Access::UAVMask;
	}
	else
	{
		InitConfig.InitialD3D12Access = ED3D12Access::GenericRead;
	}
...
void FD3D12PoolAllocator::AllocDefaultResource(
	...
	FD3D12ResourceLocation& ResourceLocation)
{
	...
	{
		check(InCreateD3D12Access == ED3D12Access::CopyDest);
	}
	...
	{
		check(InCreateD3D12Access == ED3D12Access::UAVMask);
	}
#if D3D12_RHI_RAYTRACING
	else if (EnumHasAnyFlags(InBufferUsage, BUF_AccelerationStructure))
	{
		...
		check(EnumHasAnyFlags(InCreateD3D12Access, ED3D12Access::BVHRead | ED3D12Access::BVHWrite));
	}
#endif // D3D12_RHI_RAYTRACING
#endif  // DO_CHECK

	AllocateResource(
		GetParentDevice()->GetGPUIndex(),
		InHeapType,
		InDesc,
		InDesc.Width,
...
void FD3D12PoolAllocator::AllocateResource(
	uint32 GPUIndex,
	D3D12_HEAP_TYPE InHeapType,
	const FD3D12ResourceDesc& InDesc,
	uint64 InSize,
	...
	FD3D12ResourceLocation& ResourceLocation)
{
	...
	else
	{
		...
		if (Adapter->IsTrackingAllAllocations() && (InDesc.Dimension != D3D12_RESOURCE_DIMENSION_BUFFER) && !bSharedResource)
		{
		...
		else
		{
			...
			const ED3D12Access DefaultAccess =
				(InResourceStateMode == ED3D12ResourceStateMode::Default) ?
					ED3D12Access::Unknown :
					InDefaultD3D12Access;

			// Use default alignment for buffers because mights still be set to sub allocate alignment sizes which isn't valid
			FD3D12ResourceDesc Desc = InDesc;
			if (InDesc.Dimension == D3D12_RESOURCE_DIMENSION_BUFFER)
			{				
			...
			VERIFYD3D12RESULT(
				Adapter->CreateCommittedResource(
					Desc,
					GetGPUMask(),
					HeapProps,
					InInitialD3D12Access,
	...
	if (EnumHasAnyFlags(InInitialD3D12Access, ED3D12Access::BVHRead | ED3D12Access::BVHWrite)
...
void FD3D12PoolAllocator::DeallocateResource(FD3D12ResourceLocation& ResourceLocation, bool bDefragFree)
{
	...
	if (AllocationData.IsLocked())
	{
		...
		{
			if (CopyOperation.SrcResource == ResourceLocation.GetResource() ||
				CopyOperation.DstResource == ResourceLocation.GetResource())
			{
				CopyOperation.SrcResource = nullptr;
				CopyOperation.DstResource = nullptr;
				break;
			}
		}
	}
...
bool FD3D12PoolAllocator::HandleDefragRequest(FRHIContextArray const& Contexts, FRHIPoolAllocationData* InSourceBlock, FRHIPoolAllocationData& InTmpTargetBlock)
{
	...
	ED3D12Access SrcD3D12Access = ED3D12Access::Unknown;
	if (CurrentResource->RequiresResourceStateTracking())
	{
		...
		if (Owner->Owner)
		{
			if (CurrentResource->GetDesc().Dimension == D3D12_RESOURCE_DIMENSION_BUFFER)
			{
				...
				SrcD3D12Access = ConvertToD3D12Access(CurrentResourceRHIBuffer->GetTrackedAccess_Unsafe());
			}
			else
			{
				CurrentResourceRHITexture = (FD3D12Texture*)Owner->Owner;
				SrcD3D12Access = ConvertToD3D12Access(CurrentResourceRHITexture->GetTrackedAccess_Unsafe());
			}
		}

		check(SrcD3D12Access != ED3D12Access::Unknown);
	}
	else
	{
		//If the resource is a single state type and doesn't need state tracking set the access as Unknown so the scoped transition in FlushPendingCopyOps can revert back to the default state
		check(SrcD3D12Access == ED3D12Access::Unknown);
	}
	...
	ED3D12Access DstD3D12Access = ED3D12Access::Unknown;
	Owner->OnAllocationMoved(Contexts, InSourceBlock, DstD3D12Access);

	check(IsValidAccess(DstD3D12Access));
	if (!Owner->GetResource()->RequiresResourceStateTracking())
	{
		...
		DstD3D12Access = ED3D12Access::Unknown;
	}
	...
	FD3D12VRAMCopyOperation CopyOp = {
		.SrcResource          = CurrentResource,
		.SrcResourcED3D12Access = SrcD3D12Access,
		.SrcOffset            = static_cast<uint32>(CurrentOffset),
		.DstResource          = Owner->GetResource(),
		.DstResourcED3D12Access = DstD3D12Access,
		.DstOffset            = static_cast<uint32>(Owner->GetOffsetFromBaseOfResource()),
		.Size		          = InSourceBlock->GetSize(),
		.CopyType	          =
			(AllocationStrategy == EResourceAllocationStrategy::kManualSubAllocation) ?
							FD3D12VRAMCopyOperation::ECopyType::BufferRegion :
							FD3D12VRAMCopyOperation::ECopyType::Resource,
	};
	check(CopyOp.SrcResource != nullptr);
	check(CopyOp.DstResource != nullptr);
	PendingCopyOps.Add(MoveTemp(CopyOp));
...
void FD3D12PoolAllocator::FlushPendingCopyOps(FD3D12CommandContext& InCommandContext)
{
	...
	{
		// Cleared copy op?
		if (CopyOperation.SrcResource == nullptr || CopyOperation.DstResource == nullptr)
		{
			continue;
		}

#if D3D12_RHI_RAYTRACING
		if (!CopyOperation.SrcResource->RequiresResourceStateTracking() && 
			EnumHasAnyFlags(CopyOperation.SrcResource->GetDesc().Flags, D3D12_RESOURCE_FLAG_RAYTRACING_ACCELERATION_STRUCTURE))
		{
			// can't make state changes to RT resources
			check(EnumHasAnyFlags(CopyOperation.DstResource->GetDesc().Flags, D3D12_RESOURCE_FLAG_RAYTRACING_ACCELERATION_STRUCTURE));

			D3D12_GPU_VIRTUAL_ADDRESS SrcAddress = CopyOperation.SrcResource->GetResource()->GetGPUVirtualAddress() + CopyOperation.SrcOffset;
			D3D12_GPU_VIRTUAL_ADDRESS DestAddress = CopyOperation.DstResource->GetResource()->GetGPUVirtualAddress() + CopyOperation.DstOffset;
...
#endif // D3D12_RHI_RAYTRACING
		{
			...
			{
				FScopedResourceBarrier ScopedResourceBarrierSrc(
					InCommandContext,
					CopyOperation.SrcResource,
					CopyOperation.SrcResourcED3D12Access,
					ED3D12Access::CopySrc,
					D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES);
				FScopedResourceBarrier ScopedResourceBarrierDst(
					InCommandContext,
					CopyOperation.DstResource,
					CopyOperation.DstResourcED3D12Access,
				...
				case FD3D12VRAMCopyOperation::BufferRegion:
				{
					InCommandContext.CopyBufferRegionChecked(
						CopyOperation.DstResource->GetResource(), CopyOperation.DstResource->GetName(),
						CopyOperation.DstOffset,
						CopyOperation.SrcResource->GetResource(), CopyOperation.SrcResource->GetName(),
						CopyOperation.SrcOffset,
						CopyOperation.Size
					);
					break;
				}
				case FD3D12VRAMCopyOperation::Resource:
				{
					InCommandContext.GraphicsCommandList()->CopyResource(
						CopyOperation.DstResource->GetResource(),
						CopyOperation.SrcResource->GetResource()
					);
					break;
				}
				}
			...
			if (CopyOperation.SrcResource->RequiresResourceStateTracking())
			{
				check(CopyOperation.DstResource->RequiresResourceStateTracking());
				check(IsValidAccess(CopyOperation.SrcResourcED3D12Access));

				if (CopyOperation.SrcResourcED3D12Access != CopyOperation.DstResourcED3D12Access)
				{
					InCommandContext.AddBarrier(
						CopyOperation.DstResource,
						CopyOperation.DstResourcED3D12Access,
						CopyOperation.SrcResourcED3D12Access,
						D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES);
		...
		InCommandContext.UpdateResidency(CopyOperation.SrcResource);
		InCommandContext.UpdateResidency(CopyOperation.DstResource);
	}
