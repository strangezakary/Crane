...
// Number of chunk IDs that reference explicit chunks OR represents RLE an compressed range
...
// Size of the item buffer (where explicit chunk instance IDs are stored), not all are used.
...
void FSceneCulling::PublishStats()
{
	...
	int32 NumChunks = CellChunkIdAllocator.GetSparselyAllocatedSize();
	SET_DWORD_STAT(STAT_SceneCulling_ItemChunkCount, NumChunks);
	int32 NumExplicitChunks = PackedCellData.Num() / INSTANCE_HIERARCHY_MAX_CHUNK_SIZE - FreeChunks.Num();
	SET_DWORD_STAT(STAT_SceneCulling_ExplicitChunkCount, NumExplicitChunks);
	SET_DWORD_STAT(STAT_SceneCulling_CompressedChunkCount, NumChunks - NumExplicitChunks);
	SET_DWORD_STAT(STAT_SceneCulling_ItemBufferSize, PackedCellData.Num());
	float ChunkOccupancy = float(Scene.GPUScene.GetInstanceSceneDataAllocator().GetSparselyAllocatedSize()) /  float(NumChunks * INSTANCE_HIERARCHY_MAX_CHUNK_SIZE);
	SET_FLOAT_STAT(STAT_SceneCulling_ChunkOccupancy, ChunkOccupancy);
	SET_DWORD_STAT(STAT_SceneCulling_SceneInstanceCount, Scene.GPUScene.GetInstanceSceneDataAllocator().GetSparselyAllocatedSize());
