...
namespace AndroidVulkan
{
	void VKSwappyPostWaitCallback(void*, int64_t cpu_time_ns, int64_t gpu_time_ns)
	{
		...
		GetDynamicRHI<FVulkanDynamicRHI>()->ExternalGPUTime = Cycles;
	}
...
FAutoConsoleVariableRef FVulkanAndroidPlatform::CVarVulkanRequiresWaitingForFrameCompletionEvent(
	TEXT("r.Vulkan.RequiresWaitingForFrameCompletionEvent"),
	FVulkanAndroidPlatform::bRequiresWaitingForFrameCompletionEvent,
	TEXT("Whether Android VulkanRHI should wait for frame completion event.")
	TEXT("  0 = don't wait (default)\n")
	TEXT("  1 = wait"),
	ECVF_RenderThreadSafe
);
...
void FVulkanAndroidPlatform::InitDevice(FVulkanDevice* InDevice)
{
	...
	if (FVulkanAndroidPlatform::bSwappyEnabledAtRHIInit)
	{
		...
		check(GfxQueue);
		SwappyVk_setQueueFamilyIndex(InDevice->GetHandle(), GfxQueue->GetHandle(), GfxQueue->GetFamilyIndex());
	}
#endif
}
...
{
	bRequiresMergedRenderPassDepthStencilLayout = false;
	if (Device.GetVendorId() == EGpuVendorId::Arm)
	{
		const uint32_t DriverVersion = Device.GetDeviceProperties().driverVersion;
		if (DriverVersion < VK_MAKE_API_VERSION(0, 54, 1, 0))
		{
			bRequiresMergedRenderPassDepthStencilLayout = true;
			UE_LOG(LogRHI, Display, TEXT("Enabling workaround that avoids usage of dedicated stencil layout via VkAttachmentDescriptionStencilLayout."));
		}
	}
}
	...
	if (FVulkanAndroidPlatform::bSwappyEnabledAtRHIInit && CurrentFramePace != 0)
	{
		...
		if (CurrentFramePace != CachedFramePace)
		{
			...
			if (CachedSyncInterval != 0)
			{
				...
				SwappyVk_setSwapIntervalNS(Device.GetHandle(), Swapchain, (1000000000L) / (int64)CurrentFramePace);
				bVsyncMultiple = true;
			}
			else
			{
				// Unsupported frame rate. Set to higher refresh rate and use CPU frame pacer to limit to desired frame pace
				SwappyVk_setSwapIntervalNS(Device.GetHandle(), Swapchain, (1000000000L) / (int64)CachedRefreshRate);
...
FString FVulkanAndroidPlatform::GetCompiledPSOCacheTopFolderPath()
{
	...
	return GExternalFilePath / TEXT("VulkanProgramBinaryCache");
#else
	return FVulkanGenericPlatform::GetCompiledPSOCacheTopFolderPath();
#endif
}
				...
				if(ensure(ProgramResultSharedHandle > -1))
				{
					...
					if (ensure(ResultSharedBuffer))
					{
						...
						if (ensure(ResultMemSize > 0))
						{
							...
							VERIFYVULKANRESULT(VulkanRHI::vkCreatePipelineCache(Device->GetHandle(), &PipelineCacheCreateInfo, VULKAN_CPU_ALLOCATOR, &ReturnPipelineCache));
...
void FVulkanAndroidPlatform::PostInitGPU(const FVulkanDevice& InDevice)
{
	...
	SetupRequiresMergedDepthStencilLayoutWorkaround(InDevice);
...
void FVulkanAndroidPlatform::SetupImageMemoryRequirementWorkaround(const FVulkanDevice& InDevice)
{
	...
	// AFBC workarounds
	{
		...
		VERIFYVULKANRESULT(VulkanRHI::vkCreateImage(InDevice.GetHandle(), &ImageCreateInfo, VULKAN_CPU_ALLOCATOR, &Image0));
		VulkanRHI::vkGetImageMemoryRequirements(InDevice.GetHandle(), Image0, &Image0Mem);
		VulkanRHI::vkDestroyImage(InDevice.GetHandle(), Image0, VULKAN_CPU_ALLOCATOR);

		VkImage ImageMutable;
		VkMemoryRequirements ImageMutableMem;
		ImageCreateInfo.flags = VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT;
		VERIFYVULKANRESULT(VulkanRHI::vkCreateImage(InDevice.GetHandle(), &ImageCreateInfo, VULKAN_CPU_ALLOCATOR, &ImageMutable));
		VulkanRHI::vkGetImageMemoryRequirements(InDevice.GetHandle(), ImageMutable, &ImageMutableMem);
		VulkanRHI::vkDestroyImage(InDevice.GetHandle(), ImageMutable, VULKAN_CPU_ALLOCATOR);
		...
		VERIFYVULKANRESULT(VulkanRHI::vkCreateImage(InDevice.GetHandle(), &ImageCreateInfo, VULKAN_CPU_ALLOCATOR, &ImageStorage));
		VulkanRHI::vkGetImageMemoryRequirements(InDevice.GetHandle(), ImageStorage, &ImageStorageMem);
		VulkanRHI::vkDestroyImage(InDevice.GetHandle(), ImageStorage, VULKAN_CPU_ALLOCATOR);
	...
	if ((formatProperties.linearTilingFeatures & (VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT | VK_FORMAT_FEATURE_TRANSFER_DST_BIT)) == (VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT | VK_FORMAT_FEATURE_TRANSFER_DST_BIT))
	{
		...
		VERIFYVULKANRESULT(VulkanRHI::vkCreateImage(InDevice.GetHandle(), &ImageCreateInfo, VULKAN_CPU_ALLOCATOR, &ImageOptimal_ASTC));
		VulkanRHI::vkGetImageMemoryRequirements(InDevice.GetHandle(), ImageOptimal_ASTC, &ImageOptimalMem_ASTC);
		VulkanRHI::vkDestroyImage(InDevice.GetHandle(), ImageOptimal_ASTC, VULKAN_CPU_ALLOCATOR);

		ImageCreateInfo.tiling = VK_IMAGE_TILING_LINEAR;

		VkImage ImageLinear_ASTC;
		VkMemoryRequirements ImageLinearMem_ASTC;
		VERIFYVULKANRESULT(VulkanRHI::vkCreateImage(InDevice.GetHandle(), &ImageCreateInfo, VULKAN_CPU_ALLOCATOR, &ImageLinear_ASTC));
		VulkanRHI::vkGetImageMemoryRequirements(InDevice.GetHandle(), ImageLinear_ASTC, &ImageLinearMem_ASTC);
		VulkanRHI::vkDestroyImage(InDevice.GetHandle(), ImageLinear_ASTC, VULKAN_CPU_ALLOCATOR);
...
{
	// trigger a fatal message which will make it visible to crash reporter.
	GLog->Panic();
	UE_LOG(LogVulkanRHI, Fatal, TEXT("%s"), Message);
}
