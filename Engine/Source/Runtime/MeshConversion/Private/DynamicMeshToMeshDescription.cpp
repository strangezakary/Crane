...
namespace DynamicMeshToMeshDescriptionConversionHelper
{
	...
	{
		if (!ensureMsgf(AttribIndex >= 0 && AttribIndex < InstanceAttrib.GetNumChannels(), TEXT("Failed to set attributes from overlay; invalid AttribIndex %d"), AttribIndex))
		{
			return;
		}
		for (const FTriangleID TriangleID : MeshOutArg.Triangles().GetElementIDs())
		{
			...
			auto ValidTargetElement = [NumAttribElements = InstanceAttrib.GetNumElements()](const FVertexInstanceID ID) -> bool 
			{
				int32 IDValue = ID.GetValue();
				return ensureMsgf(IDValue >= 0 && IDValue < NumAttribElements, TEXT("Failed to set attributes from overlay; invalid target vertex instance %d in mesh with %d instances"), IDValue, NumAttribElements);
			};
			if (!ValidTargetElement(InstanceTri[0]) || !ValidTargetElement(InstanceTri[1]) || !ValidTargetElement(InstanceTri[2]))
			{
				continue;
			}
			
			auto SetDefaults = [&InstanceAttrib, &InstanceTri, AttribIndex, &DefaultValue]()
			{
			...
			if (Overlay->IsSetTriangle(MeshInTriIdx) && 
				ensureMsgf(MeshInArg->IsTriangle(MeshInTriIdx), TEXT("Failed to set attributes from overlay; invalid TriIdx %d not found in mesh with %d Tri IDs and %d Tri Count"), MeshInTriIdx, MeshInArg->MaxTriangleID(), MeshInArg->TriangleCount()))
			{
				FIndex3i OverlayEls = Overlay->GetTriangle(MeshInTriIdx);
				if (!ensureMsgf(Overlay->IsElement(OverlayEls.A) && Overlay->IsElement(OverlayEls.B) && Overlay->IsElement(OverlayEls.C), TEXT("Failed to set attributes from overlay; invalid overlay triangle w/ indices (%d, %d, %d) on overlay with %d element IDs"), OverlayEls.A, OverlayEls.B, OverlayEls.C, Overlay->MaxElementID()))
				{
					SetDefaults();
				}
				else
				{
					InstanceAttrib.Set(InstanceTri[0], AttribIndex, OutAttributeType(Overlay->GetElement(OverlayEls.A)));
					InstanceAttrib.Set(InstanceTri[1], AttribIndex, OutAttributeType(Overlay->GetElement(OverlayEls.B)));
					InstanceAttrib.Set(InstanceTri[2], AttribIndex, OutAttributeType(Overlay->GetElement(OverlayEls.C)));
				}
			}
			else
			{
...
void FDynamicMeshToMeshDescription::Convert_NoSharedInstances(const FDynamicMesh3* MeshIn, FMeshDescription& MeshOut, bool bCopyTangents)
{
	...
	const bool bHasMorphTargetAttributes = bHasAttributes && !MeshIn->Attributes()->GetMorphTargetAttributes().IsEmpty();
	...
	// Make sure edge hardnesses are registered, if they might be set below
	if (ConversionOptions.bSetHardEdgesFromNormalSeams && NormalOverlay)
	{
		FStaticMeshAttributes MeshOutAttributes(MeshOut);
	...
	if (bHasSkinWeightsAttributes)
	{
		...
		for (const FName Name : MeshOutAttributes.GetSkinWeightProfileNames())
		{
			MeshOutAttributes.UnregisterSkinWeightAttribute(Name);
		}
	...
	TMap<FName, TVertexAttributesRef<FVector3f>> MorphTargetsMap;
	if (bHasMorphTargetAttributes)
	{
		FSkeletalMeshAttributes MeshOutAttributes(MeshOut);
		
		for (const FName Name : MeshOutAttributes.GetMorphTargetNames())
		{
			MeshOutAttributes.UnregisterMorphTargetAttribute(Name);
		}
		
		for (const TTuple<FName, TUniquePtr<FDynamicMeshMorphTargetAttribute>>& AttributeInfo: MeshIn->Attributes()->GetMorphTargetAttributes())
		{
			FName MorphTargetName = AttributeInfo.Key;

			MeshOutAttributes.RegisterMorphTargetAttribute(MorphTargetName, false);
			MorphTargetsMap.Add(MorphTargetName, MeshOutAttributes.GetVertexMorphPositionDelta(MorphTargetName));
		}
	}
	...
	// convert normal seams to hard edges
	if (ConversionOptions.bSetHardEdgesFromNormalSeams && NormalOverlay)
	{
		FStaticMeshAttributes MeshOutAttributes(MeshOut);
		TEdgeAttributesRef<bool> EdgeHardness = MeshOutAttributes.GetEdgeHardnesses();

		// Find seam edges on the dynamic mesh
		TArray<bool> MeshSeamEdges;
		MeshSeamEdges.SetNumZeroed(MeshIn->MaxEdgeID());
		ParallelFor(MeshIn->MaxEdgeID(), [&MeshIn, &NormalOverlay, &MeshSeamEdges](int32 EID)
			{
				if (MeshIn->IsEdge(EID))
				{
					MeshSeamEdges[EID] = NormalOverlay->IsSeamEdge(EID);
				}
			});

		// For any seam edge, find the corresponding mesh description edge and tag it as hard
		// Note that this will also tag non-manifold edges (if preserved via remapping) as hard edges
		for (int32 EID = 0; EID < MeshSeamEdges.Num(); ++EID)
		{
			if (MeshSeamEdges[EID]) // will only be true for valid edges that are boundaries or seams
			{
				int32 MeshInTID = MeshIn->GetEdgeT(EID).A;
				int32 EdgeTriSubIdx = MeshIn->GetTriEdges(MeshInTID).IndexOf(EID);
				if (!ensure(EdgeTriSubIdx != IndexConstants::InvalidID))
				{
					continue;
				}

				FTriangleID MeshOutTID = IndexToTriangleIDMap[MeshInTID];
				TArrayView<FEdgeID> OutTriEdges = MeshOut.GetTriangleEdges(MeshOutTID);
				if (!ensure(OutTriEdges.IsValidIndex(EdgeTriSubIdx)))
				{
					continue;
				}
				EdgeHardness[OutTriEdges[EdgeTriSubIdx]] = true;
			}
		}
	}
	...
	if (bHasMorphTargetAttributes)
	{
		for (const TTuple<FName, TUniquePtr<FDynamicMeshMorphTargetAttribute>>& AttributeInfo: MeshIn->Attributes()->GetMorphTargetAttributes())
		{
			...
			TVertexAttributesRef<FVector3f>& MeshDescMorphTarget = MorphTargetsMap[ProfileName];

			const FDynamicMeshMorphTargetAttribute* DynaMeshMorphTarget = AttributeInfo.Value.Get();
			...
			for (int32 VertexIndex = 0; VertexIndex < MapV.Num(); VertexIndex++)
			{
				...
				{
					FVector3f Offset;
					DynaMeshMorphTarget->GetValue(VertexIndex, Offset);
					MeshDescMorphTarget.Set(VertexID, Offset);	
				}
			}
		}
	}
