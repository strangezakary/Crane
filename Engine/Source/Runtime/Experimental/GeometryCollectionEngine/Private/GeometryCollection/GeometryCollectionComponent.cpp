...
void UGeometryCollectionComponent::OnComponentDestroyed(bool bDestroyingHierarchy)
{
	if (EventDispatcher)
	{
		EventDispatcher->DestroyComponent();
...
bool UGeometryCollectionComponent::FindLeafTransformByLineTrace(const FVector Start, const FVector End, int32& OutLeafTransformIndex, FName& OutLeafTransformName) const
{
	OutLeafTransformIndex = INDEX_NONE;
	OutLeafTransformName = NAME_None;
	...
	Objects = Objects.FilterByPredicate(
		[&Interface](Chaos::FPhysicsObjectHandle Handle)
		{
			...
			return !Interface.GetInterface().HasChildren(Handle);
		}
	);
	...
	if (CollisionInterface.LineTrace(Objects, Start, End, /*bTraceComplex*/false, BestHit))
	{
		if (BestHit.Actor)
		{
			if (const Chaos::FPBDRigidParticle* RigiParticle = BestHit.Actor->CastToRigidParticle())
			{
				const FGeometryCollectionItemIndex ItemIndex = PhysicsProxy->GetItemIndexFromGTParticleNoInternalCluster_External(RigiParticle);
				OutLeafTransformIndex = ItemIndex.GetItemIndex();
				OutLeafTransformName = PhysicsProxy->GetTransformName_External(ItemIndex);
				bHaveHit = true;
			}
		}
	}
...
UPhysicalMaterial* UGeometryCollectionComponent::GetPhysicalMaterial() const
{
	...
	if(!PhysMatToUse || PhysMatToUse->GetFName() == "DefaultPhysicalMaterial" || PhysMatToUse == GEngine->DefaultPhysMaterial)
	{
	...
	if ((!PhysMatToUse || PhysMatToUse == GEngine->DefaultPhysMaterial) && RestCollection)
	{
		PhysMatToUse = RestCollection->PhysicsMaterial;
	}
...
void UGeometryCollectionComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	{
		// call the setter method to make sure everything is proeprly reset
		SetSolverActor(ChaosSolverActor);
	}
}
#endif
...
namespace 
{
	...
	bool ProcessRepDataCommon(const float DeltaTime, const float SimTime, const FGeometryCollectionRepData& RepData, FGeometryCollectionPhysicsProxy* PhysicsProxy, int32& VersionProcessed, int32& OneOffActivatedProcessed, double& LastHardsnapTimeInMs, bool bReplicateMovement)
	{
		...
		for (; OneOffActivatedProcessed < RepData.OneOffActivated.Num(); ++OneOffActivatedProcessed)
		{
			...
			if (FPBDRigidParticleHandle* OneOff = PhysicsProxy->GetParticle_Internal(ActivatedCluster.ActivatedIndex))
			{
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		SimulationParameters.AllowPartialIslandSleep = BodyInstance.GetAllowPartialIslandSleep();
		const int32 PositionSolverIterationCount = BodyInstance.GetPositionSolverIterationCount();
		SimulationParameters.PositionSolverIterations = (PositionSolverIterationCount >= 0) ? FMath::Min(PositionSolverIterationCount, 255) : 8;
		const int32 VelocitySolverIterationCount = BodyInstance.GetVelocitySolverIterationCount();
		SimulationParameters.VelocitySolverIterations = (VelocitySolverIterationCount >= 0) ? FMath::Min(VelocitySolverIterationCount, 255) : 1;
		const int32 ProjectionSolverIterationCount = BodyInstance.GetProjectionSolverIterationCount();
		SimulationParameters.ProjectionSolverIterations = (ProjectionSolverIterationCount >= 0) ? FMath::Min(ProjectionSolverIterationCount, 255) : 1;
...
void UGeometryCollectionComponent::UpdateRenderSystemsIfNeeded(bool bDynamicCollectionDirty)
{
	...
	{
		const bool bWantNanite = UseNanite(GMaxRHIShaderPlatform) && RestCollection->EnableNanite && GGeometryCollectionNanite != 0;
	...
	for (int32 ParticleIndex = 0; ParticleIndex < PhysicsObjects.Num(); ++ParticleIndex)
	{
		if (PhysicsObjects[ParticleIndex] != nullptr)
		{
			...
			if (Data)
			{
				...
				Interface->UpdateCombinedShapeFilterData(ParticleView, Chaos::Filter::FShapeFilterBuilder::BuildFromLegacyShapeFilter(ParticleData.QueryFilter, ParticleData.SimFilter));
			}
		}
	}
...
void UGeometryCollectionComponent::SetCollisionObjectType(ECollisionChannel Channel)
{
	...
	if (PhysicsProxy)
	{
		PhysicsProxy->UpdateFilterData_External(Chaos::Filter::FShapeFilterBuilder::BuildFromLegacyShapeFilter(InitialQueryFilter, InitialSimFilter));
	}
}
...
void UGeometryCollectionComponent::ResetState()
{
	...
	// if we are using a custom render, make sure everything is properly reset 
	RefreshCustomRenderer();
}
...
FGeometryCollectionEdit::~FGeometryCollectionEdit()
{
	...
	if (!!EditUpdate)
	{
		...
		auto ProcessComponent = [this](UGeometryCollectionComponent* ToProcess)
		{
			...
			ToProcess->RefreshCustomRenderer();
		};
...
FMaterialRelevance UGeometryCollectionComponent::GetMaterialRelevance(EShaderPlatform InShaderPlatform) const
{
	...
	if (RestCollection)
	{
		if (UMaterialInterface* BoneSelectedMaterial = GetRestCollection()->GetBoneSelectedMaterial())
		{
			Result |= BoneSelectedMaterial->GetRelevance_Concurrent(InShaderPlatform);
		}
	}
	return Result;
}
...
void UGeometryCollectionComponent::CollectPSOPrecacheData(const FPSOPrecacheParams& BasePrecachePSOParams, FMaterialInterfacePSOPrecacheParamsList& OutParams)
{
	...
	TSharedPtr<FGeometryCollection, ESPMode::ThreadSafe> RestGeomCollection = RestCollection->GetGeometryCollection();
	const TManagedArray<FGeometryCollectionSection>& SectionsArray = RestGeomCollection->Sections;
