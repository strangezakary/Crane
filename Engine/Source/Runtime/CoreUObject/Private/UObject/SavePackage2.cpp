...
int32 GMemoryMappedBulkDataAlignment = 16;
static FAutoConsoleVariableRef CVar_MemoryMappedBulkDataAlignment(
	TEXT("cook.MemoryMappedBulkDataAlignment"),
	GMemoryMappedBulkDataAlignment,
	TEXT("0 - Target platform memory mapping alignment is respected, might result in big zero-padding")
	TEXT("1 - No special alignment required, 1-byte alignment used")															  
	TEXT("Any other non-zero value - overrides the memory mapped data alignment, implies padding")
);
		...
		SaveContext.GetHarvestedRealm(ESaveRealm::Game).GetExports().Num())
	{
		...
		for (auto It = OptionalContext.GetExports().CreateIterator(); It; ++It)
		{
			if (!FPackageHarvester::ShouldObjectBeHarvestedInOptionalRealm(It->Obj, SaveContext))
			{
				...
				{
					// TODO: Suppress this error and leave the object in the optional realm on Server builds, because it does not work in
					// some cases on multiplatform cooks. EditorOptional is a global commandline setting that applies to all platforms in
					// a multiplatform cook, but on some projects the project cooks Client and Server together and wants editoroptional
					// only on client, so it cooks both of them as editoroptional. But then server doesn't support editoroptional for
					// objects that are in ClassesExcludedOnDedicatedServer but that are referenced both from optional and from gamerealm
					// objects; we get this error because the object is stripped from the game realm but remains in the optional realm.
					// 
					// Change EditorOptional to be specifiable per platform, and remove this suppression.
					if (!SaveContext.GetTargetPlatform()->IsServerOnly())
					{
	...
	ESaveRealm HarvestedRealm)
{
	...
	if (ObjectsToCheck)
	{
		...
		FString CulpritDiagnostics;
		UE::SavePackageUtilities::FindMostLikelyCulprit(*ObjectsToCheck, SaveContext, HarvestedRealm, MostLikelyCulprit,
			Referencer, ReferencerProperty, IsCulpritArchetype, CulpritDiagnostics);
		...
		UE_LOG(LogSavePackage, Warning, TEXT("%s"), *CulpritDiagnostics);
		if (SaveContext.IsGenerateSaveError())
		{
			SaveContext.GetError()->Logf(ELogVerbosity::Warning, TEXT("%s"), *ErrorMessage);
		}
...
ESavePackageResult ValidateImports(FSaveContext& SaveContext)
{
	...
	ESaveRealm HarvestedRealm = SaveContext.GetCurrentHarvestingRealm();
	...
	{
		return ValidateIllegalReferences(SaveContext, PrivateObjects, PrivateContentObjects, ObjectsInOtherMaps, HarvestedRealm);
	}
...
ESavePackageResult CreateLinker(FSaveContext& SaveContext)
{
	...
	LinkerSave->AssignSaveContext(&SaveContext);
...
ESavePackageResult BuildLinker(FSaveContext& SaveContext)
{
	...
	FLinkerSave* Linker = SaveContext.GetLinker();
	{
		...
		if (SaveContext.IsCooking())
		{
			...
			if (const ITargetPlatform* TargetPlatform = SaveContext.GetTargetPlatform())
			{
				...
				{
					int64 MemoryMapAlignment = GMemoryMappedBulkDataAlignment > 0 ? GMemoryMappedBulkDataAlignment : TargetPlatform->GetMemoryMappingAlignment();
					Linker->SetMemoryMapAlignment(MemoryMapAlignment);
				}
	...
	TMap<Verse::VCell*, Verse::VUniqueString*> VersePathMap;
	{
		...
		for (Verse::VCell* Export : SaveContext.GetCellExports())
		{
			...
			{
				if (Verse::VPackage* VersePackage = Context.PackageForCell(Export))
				{
					VersePackages.Add(VersePackage);
				}
			}
		}
		...
		for (Verse::VPackage* VersePackage : VersePackages)
		{
			...
			{
				if (Verse::VCell* Cell = VersePackage->GetDefinition(Index).ExtractCell())
				{
					// Packages can re-export cells from other packages; only use their original Verse path here.
					if (Context.PackageForCell(Cell) == VersePackage)
					{
	...
	{
		SCOPED_SAVETIMER(UPackage_Save_BuildSoftPackagesAndSearchableNames);
		...
		if (!SaveContext.IsFilterEditorOnly())
		{
			...
			for (TPair<TObjectPtr<UObject>, TArray<FName>>& SearchableNamePair : SaveContext.GetSearchableNamesObjectMap())
			{
				...
				if (ensureMsgf(!PackageIndex.IsNull(), TEXT("%s: Searchable object %s with is not mapped"), *Linker->LinkerRoot->GetFullName(), *SearchableNamePair.Key->GetFullName()))
				{
...
#if WITH_EDITORONLY_DATA
		// Build ImportTypeHierarchies
		if (!SaveContext.IsCooking() && UE::Serialization::Private::FImportTypeHierarchy::IsSerializationEnabled())
		{
			SCOPED_SAVETIMER(UPackage_Save_BuildImportTypeHierarchies);

			for (FObjectExport& Export : Linker->ExportMap)
			{
				if (!Export.ClassIndex.IsNull() && Export.ClassIndex.IsImport())
				{
					FObjectImport& ClassImport = Linker->ImportMap[Export.ClassIndex.ToImport()];

					if (ClassImport.XObject)
					{
						UStruct* Struct = Cast<UStruct>(ClassImport.XObject);
						if (Struct)
						{
							const FPackageIndex PackageIndex = Linker->MapObject(ClassImport.XObject);
							// This should always be in the import table already.
							if (ensureMsgf(!PackageIndex.IsNull(), TEXT("%s: Struct object: '%s' is not mapped"), *Linker->LinkerRoot->GetFullName(), *Struct->GetFullName()))
							{
								// Skip already processed class imports.
								if (!Linker->ImportTypeHierarchies.Contains(PackageIndex))
								{
									// To avoid bloating the ImportTypeHierarchy table only create entries for types 
									// that support PropertyBag placeholders.
									if (UE::CanCreatePropertyBagPlaceholderForType(Struct->GetClass(), Struct))
									{
										UE::Serialization::Private::FImportTypeHierarchy& ImportTypeHierarchy = Linker->ImportTypeHierarchies.Add(PackageIndex);

										UStruct* SuperStruct = Struct->GetSuperStruct();

										while (SuperStruct && (SuperStruct != UObject::StaticClass()))
										{
											UClass* SuperStructClass = SuperStruct->GetClass();
											UPackage* SuperStructPackage = SuperStruct->GetPackage();

											UE::Serialization::Private::FTypeResource SuperType;
											SuperType.TypeName = SuperStruct->GetFName();
											SuperType.PackageName = SuperStructPackage ? SuperStructPackage->GetFName() : NAME_None;
											SuperType.ClassName = SuperStructClass ? SuperStructClass->GetFName() : NAME_None;
											SuperType.ClassPackageName = (SuperStructClass && SuperStructClass->GetPackage()) ? SuperStructClass->GetPackage()->GetFName() : NAME_None;

											ImportTypeHierarchy.AddSuperType(MoveTemp(SuperType));
...
#if WITH_VERSE_VM || defined(__INTELLISENSE__)
void GetCellsWithOuter(Verse::FAllocationContext Context, FSaveContext& SaveContext, UObject* Outer, TArray<Verse::VCell*>& Stack, TSet<Verse::VCell*>& Results)
{
	struct FSubCellVisitor : UE::SavePackageUtilities::Private::FArchiveSavePackageCollector
	{
		TSet<Verse::VCell*>& SubCells;
		TArray<Verse::VCell*>& SubCellStack;

		FSubCellVisitor(FSaveContext& InContext, TSet<Verse::VCell*>& InSubCells, TArray<Verse::VCell*>& InSubCellStack)
			: FArchiveSavePackageCollector(InContext.GetArchiveSavePackageData(),
				InContext.IsFilterEditorOnly(), InContext.IsSaveUnversionedProperties(), InContext.IsCooking())
			, SubCells(InSubCells)
			, SubCellStack(InSubCellStack)
		{
		}

		virtual FArchive& operator<<(Verse::VCell*& InCell) override
		{
			if (InCell != nullptr)
			{
				if (InCell->IsA<Verse::VRef>() ||
					InCell->IsA<Verse::VNativeStruct>() ||
					InCell->IsA<Verse::VValueObject>() ||
					InCell->IsA<Verse::VArrayBase>() ||
					InCell->IsA<Verse::VMapBase>() ||
					InCell->IsA<Verse::VOption>() ||
					InCell->IsA<Verse::VFunction>() ||
					InCell->IsA<Verse::VNativeFunction>())
				{
					bool bIsAlreadyInSet = false;
					SubCells.Add(InCell, &bIsAlreadyInSet);
					if (!bIsAlreadyInSet)
					{
						SubCellStack.Push(InCell);
					}
				}
			}
			return *this;
		}
	} Visitor{SaveContext, Results, Stack};
	Outer->Serialize(Visitor);
	while (Stack.Num() > 0)
	{
		Verse::VCell* SubCell = Stack.Pop(EAllowShrinking::No);
		SubCell->Serialize(Context, Visitor);
	}
}
#endif
...
void SavePreloadDependencies(FStructuredArchive::FRecord& StructuredArchiveRoot, FSaveContext& SaveContext)
{
	...
	auto IncludeObjectAsDependency = [Linker, &SaveContext](int32 CallSite, TSet<FPackageIndex>& AddTo, TObjectPtr<UObject> ToTest, UObject* ForObj, bool bMandatory, bool bOnlyIfInLinkerTable)
	{
		...
		if (ToTest)
		{
			...
			FPackageIndex Index = Linker->MapObject(ToTest, false);
	...
	if (SaveContext.IsCooking())
	{
		...
		for (int32 ExportIndex = 0; ExportIndex < Linker->ExportMap.Num(); ++ExportIndex)
		{
			...
			}
			{
...
#if WITH_VERSE_VM || defined(__INTELLISENSE__)
					// These VCells must be serialized before this CDO is used as an archetype.
					// This is similar to the GetObjectsWithOuter(CDO) call above. We handle CDO VCells here instead because:
					//   1) Not all these CDO VCells are importable, but they are all in the CDO's package.
					//   2) This lets us add these dependency edges once rather than repeated for all instances.
					GetCellsWithOuter(Context, SaveContext, Export.Object, SubCellStack, SubCells);
					for (Verse::VCell* SubCell : SubCells)
					{
						SerializationBeforeSerializationDependencies.Add(Linker->CellIndicesMap[SubCell]);
					}
					SubCells.Reset();
#endif
				}
				...
				Verse::VClass* Class = nullptr;
				if (UVerseClass* VerseClass = Cast<UVerseClass>(Export.Object))
				{
					Class = VerseClass->Class.Get();
					check(Class != nullptr);
				}
				if (UVerseStruct* VerseStruct = Cast<UVerseStruct>(Export.Object))
				{
					Class = VerseStruct->Class.Get();
					if (VerseStruct->IsTuple())
					{
						check(Class == nullptr);
						FUtf8String MangledNameString = VerseStruct->GetFName().ToUtf8String();
						...
						SerializationBeforeSerializationDependencies.Add(Linker->CellIndicesMap[TupleType]);
					}
					else
					{
						check(Class != nullptr);
...
#if WITH_VERSE_VM || defined(__INTELLISENSE__)
			{
				...
				if (Verse::VClass* Class = Export.Cell->DynamicCast<Verse::VClass>())
				{
					...
					for (int32 Index = 0; Index < Archetype.NumEntries; ++Index)
					{
						...
						Verse::VValue Type = Entry.Type.Follow();
						if (Type.IsCellOfType<Verse::VType>())
						{
							SerializationBeforeSerializationDependencies.Add(Linker->CellIndicesMap[Type.ExtractCell()]);
						}
						// Constructing a VShape inspects the values to determine whether they are methods.
						Verse::VValue Value = Entry.Value.Follow();
						if (Value.IsCellOfType<Verse::VFunction>() || Value.IsCellOfType<Verse::VNativeFunction>())
						{
							SerializationBeforeSerializationDependencies.Add(Linker->CellIndicesMap[Value.ExtractCell()]);
						}
					}
				}
				...
				if (Verse::VNativeStruct* NativeStruct = Export.Cell->DynamicCast<Verse::VNativeStruct>())
				{
					Verse::VClass& Class = NativeStruct->GetEmergentType()->Type->StaticCast<Verse::VClass>();
					SerializationBeforeSerializationDependencies.Add(Linker->CellIndicesMap[&Class]);

					FPackageIndex Struct = Linker->ObjectIndicesMap.FindRef(Class.GetUEType<UObject>());
					IncludeIndexAsDependency(SerializationBeforeSerializationDependencies, Struct);
				}
			}
#endif
			{
...
void WriteGatherableText(FStructuredArchive::FRecord& StructuredArchiveRoot, FSaveContext& SaveContext)
{
	...
	if (!SaveContext.IsCooking()
		...
		//	3) Don't contain text initialized from a string which includes text properties pasted into the property matrix 
		&& !EnumHasAnyFlags(SaveContext.GatherableTextResultFlags, EPropertyLocalizationGathererResultFlags::HasScript | EPropertyLocalizationGathererResultFlags::HasTextWithInvalidPackageLocalizationID | EPropertyLocalizationGathererResultFlags::HasTextInitializedFromString))
	{
		FLinkerSave* Linker = SaveContext.GetLinker();
...
ESavePackageResult WritePackageHeader(FStructuredArchive::FRecord& StructuredArchiveRoot, FSaveContext& SaveContext)
{
	...
	// Write ImportTypeHierarchies
#if WITH_EDITORONLY_DATA
	Linker->Summary.ImportTypeHierarchiesCount = 0;
	Linker->Summary.ImportTypeHierarchiesOffset = 0;

	if (!SaveContext.IsCooking() && UE::Serialization::Private::FImportTypeHierarchy::IsSerializationEnabled())
	{
		SCOPED_SAVETIMER(UPackage_Save_SaveImportTypeHierarchies);
		
		Linker->Summary.ImportTypeHierarchiesOffset = (int32)Linker->Tell();
		Linker->Summary.ImportTypeHierarchiesCount = Linker->ImportTypeHierarchies.Num();

		// Sort before saving to keep the entry order consistent.
		Linker->ImportTypeHierarchies.KeySort(TLess<FPackageIndex>());

		FStructuredArchive::FStream ImportTypeHierarchyStream = StructuredArchiveRoot.EnterStream(TEXT("ImportTypeHierarchies"));

		ImportTypeHierarchyStream << Linker->ImportTypeHierarchies;
	}
#endif
...
ESavePackageResult SaveHarvestedRealms(FSaveContext& SaveContext, ESaveRealm HarvestingContextToSave)
{
	...
	// Write Exports 
	{
		...
		// mark the linker archive as currently serializing Optional data
		bool bOriginalSavingOptionalObject = Linker.IsSavingOptionalObject();
		Linker.SetSavingOptionalObject(HarvestingContextToSave == ESaveRealm::Optional);
		...
		Linker.SetSavingOptionalObject(bOriginalSavingOptionalObject);
	}
