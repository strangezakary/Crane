...
void FLODUtilities::RemoveLOD(FSkeletalMeshUpdateContext& UpdateContext, int32 DesiredLOD )
{
	...
	if(DesiredLOD < SkelMeshModel->LODModels.Num() )
	{
		...
		SkeletalMesh->RemoveSourceModel(DesiredLOD);
	...
	)
{
	...
	if (SkeletalMesh->GetLODInfo(DesiredLOD) != nullptr)
	{
		...
		if (SkeletalMeshResource->LODModels.IsValidIndex(DesiredLOD) && (!SkeletalMesh->GetLODInfo(DesiredLOD)->bHasBeenSimplified || DesiredLOD == Settings.BaseLOD))
		{
			...
			{
				// Iterate each original morph target source index to fill the NewMorphTargetDeltas array with the TargetMatchData.
				// If there's no morph data for the LOD, the returned array view is empty, and so nothing happens.
				...
				MorphDeltasArray.Append(MorphTarget->GetMorphTargetDeltas(DesiredLOD));
			}
			...
			if (DesiredLOD == 0)
			{
				const UAssetImportData* AssetImportData = SkeletalMesh->GetAssetImportData();
				SkeletalMesh->GetLODInfo(DesiredLOD)->SourceImportFilename = AssetImportData ? AssetImportData->GetFirstFilename() : FString();
			}
		}
	}
		...
		UMorphTarget* MorphTarget = nullptr;
		{
			...
			if (!MorphTarget)
			{
				if (LODIndex == 0)
				{
					...
					// Check if there's an orphaned morph target object floating about. We use StaticFindObjectFastInternal
					// rather than StaticFindObjectFast since we might have a situation where the orphaned morph target might
					// still have the Async flag set, which StaticFindObjectFast will skip over.
					MorphTarget = Cast<UMorphTarget>(StaticFindObjectFastInternal(UMorphTarget::StaticClass(), BaseSkelMesh, ObjectName, EFindObjectFlags::ExactClass));
					if (MorphTarget)
					{
						// Temporarily set the async flag on it so that it doesn't vanish once we leave the GC scope.
						MorphTarget->SetInternalFlags(EInternalObjectFlags::Async);
						
						// Eviscerate this morph target for 
						MorphTarget->GetMorphLODModels().Reset();
					}
					else
					{
						// This will already have the async flag on it, since we're creating the object in a worker thread.
...
void FLODUtilities::UnbindClothingAndBackup(USkeletalMesh* SkeletalMesh, TArray<ClothingAssetUtils::FClothingAssetMeshBinding>& ClothingBindings)
{
	...
	if (ClothingAssetUtils::HasAnyMeshClothingAssetBindings(SkeletalMesh))  // Don't run the PostEditChange unless strictly necessary
	{
...
{
	if (!ClothingAssetUtils::HasAnyLodMeshClothingAssetBindings(SkeletalMesh, LODIndex))  // Early exit to avoid the FScopedSkeletalMeshPostEditChange whenever possible
	{
		return;
	}
	// Guard the unbinding with a scoped PostEditChange
	FScopedSkeletalMeshPostEditChange SkeletalMeshPostEditChange(SkeletalMesh);
	...
	for (ClothingAssetUtils::FClothingAssetMeshBinding& Binding : ClothingBindings)
	{
		if (Binding.LODIndex == LODIndex)
		{
			...
			if (FSkelMeshSourceSectionUserData* const SectionUserData = LODModel.UserSectionsData.Find(OriginalDataSectionIndex);
				ensureMsgf(SectionUserData, TEXT("A FScopedSkeletalMeshPostEditChange guard must be used to ensure that the model isn't getting rebuilt while this function is called.")))
			{
...
void FLODUtilities::RestoreClothingFromBackup(USkeletalMesh* SkeletalMesh, TArray<ClothingAssetUtils::FClothingAssetMeshBinding>& ClothingBindings)
{
	if (!SkeletalMesh ||
		...
		!ClothingBindings.Num())  // Early exit to avoid the FScopedSkeletalMeshPostEditChange whenever possible
	{
		return;
	}
	// Guard the binding with a scoped PostEditChange
	FScopedSkeletalMeshPostEditChange SkeletalMeshPostEditChange(SkeletalMesh);
	for (int32 LODIndex = 0; LODIndex < SkeletalMesh->GetImportedModel()->LODModels.Num(); ++LODIndex)
	{
...
void FLODUtilities::RestoreClothingFromBackup(USkeletalMesh* SkeletalMesh, TArray<ClothingAssetUtils::FClothingAssetMeshBinding>& ClothingBindings, const int32 LODIndex)
{
	if (!SkeletalMesh ||
		!SkeletalMesh->GetImportedModel() ||
		!SkeletalMesh->GetImportedModel()->LODModels.IsValidIndex(LODIndex) ||
		!ClothingBindings.Num())  // Early exit to avoid the FScopedSkeletalMeshPostEditChange whenever possible
	{
		return;
	}
		...
		for (int32 SectionIndex = 0; SectionIndex < LODModel.Sections.Num(); ++SectionIndex)
		{
			...
			if (Binding.LODIndex == LODIndex && Binding.Asset)
			{
				if (Binding.Asset->BindToSkeletalMesh(SkeletalMesh, Binding.LODIndex, SectionIndex, Binding.AssetInternalLodIndex))
				{
					...
					if (FSkelMeshSourceSectionUserData* const SectionUserData = LODModel.UserSectionsData.Find(Binding.SectionIndex);
						ensureMsgf(SectionUserData, TEXT("A FScopedSkeletalMeshPostEditChange guard must be used to ensure that the model isn't getting rebuilt while this function is called.")))
					{
