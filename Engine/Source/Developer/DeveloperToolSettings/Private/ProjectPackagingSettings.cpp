...
const FTargetInfo* FindBestTargetInfo(const FString& TargetName, bool bContentOnlyUsesEngineTargets, bool* bOutIsProjectTarget=nullptr)
{
	...
	// Added in 5.7.2 to help diagnose UE-316569
	if (DefaultGameTarget == nullptr && DefaultClientTarget == nullptr)
	{
		UE_LOG(LogProjectPackagingSettings, Log, TEXT("UProjectPackagingSettings FindBestTargetInfo for '%s' resulted in null FTargetInfo*.  Listing targets that were searched:"), *TargetName);
		for (const FTargetInfo& Target : TargetsRef)
		{
			UE_LOG(LogProjectPackagingSettings, Log, TEXT("  %i %s"), Target.Type , *Target.Name);
		}
		UE_LOG(LogProjectPackagingSettings, Log, TEXT("  End of target list."));
	}
