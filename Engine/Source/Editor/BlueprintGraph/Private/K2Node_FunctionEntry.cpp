...
#include UE_INLINE_GENERATED_CPP_BY_NAME(K2Node_FunctionEntry)
...
void UK2Node_FunctionEntry::AddReferencedObjects(UObject* InThis, FReferenceCollector& Collector)
{
	UK2Node_FunctionEntry* This = CastChecked<UK2Node_FunctionEntry>(InThis);
	TSharedPtr<FStructOnScope> FunctionVariableCache = This->FunctionVariableCache;
	if (FunctionVariableCache.IsValid())
	{
		TWeakObjectPtr<const UStruct> StructPtr = FunctionVariableCache->GetStructPtr();
		if (StructPtr.IsValid())
		{
			Collector.AddPropertyReferences(StructPtr.Get(), FunctionVariableCache->GetStructMemory());
		}
	}
	return UK2Node_FunctionTerminator::AddReferencedObjects(InThis, Collector);
}
