...
void FFbxImporter::SkinControlPointsToPose(FSkeletalMeshImportData& ImportData, FbxMesh* FbxMesh, FbxShape* FbxShape, bool bUseT0 )
{
	...
	if (ClusterCount)
	{
		...
		{
			FbxAMatrix TotalMatrixForNormal = MeshMatrix.Inverse().Transpose();
			...
			bool OddNegativeScale = IsOddNegativeScale(MeshMatrix);

			FbxLayerElement::EReferenceMode NormalReferenceMode(LayerElementNormal ? LayerElementNormal->GetReferenceMode() : FbxLayerElement::eDirect);
			FbxLayerElement::EMappingMode NormalMappingMode(LayerElementNormal ? LayerElementNormal->GetMappingMode() : FbxLayerElement::eByControlPoint);
			
			FbxLayerElement::EReferenceMode TangentReferenceMode(LayerElementTangent ? LayerElementTangent->GetReferenceMode() : FbxLayerElement::eDirect);
			FbxLayerElement::EMappingMode TangentMappingMode(LayerElementTangent ? LayerElementTangent->GetMappingMode() : FbxLayerElement::eByControlPoint);

			FbxLayerElement::EReferenceMode BinormalReferenceMode(LayerElementBinormal ? LayerElementBinormal->GetReferenceMode() : FbxLayerElement::eDirect);
			FbxLayerElement::EMappingMode BinormalMappingMode(LayerElementBinormal ? LayerElementBinormal->GetMappingMode() : FbxLayerElement::eByControlPoint);

			const int32 TriangleCount = FbxMesh->GetPolygonCount();
			const int32 ExistingTriangleCount = ImportData.Faces.Num();
			const int32 StartTriangleIndex = ExistingTriangleCount - TriangleCount;
			for (int32 TriangleIndex = 0; TriangleIndex < TriangleCount; ++TriangleIndex)
			{
				SkeletalMeshImportData::FTriangle& Triangle = ImportData.Faces[StartTriangleIndex + TriangleIndex];
				...
				for (int32 VertexIndex = 0; VertexIndex < 3; ++VertexIndex)
				{
					...
					int32 ControlPointIndex = FbxMesh->GetPolygonVertex(TriangleIndex, VertexIndex);
					...
					FbxAMatrix ClusterDeformation = ClusterDeformations[ControlPointIndex].Inverse().Transpose();
					...
					if (ImportData.bHasNormals)
					{
						if (ImportData.bHasTangents)
						{
							if (LayerElementTangent)
							{
								...
								FbxVector4 TangentX = LayerElementTangent->GetDirectArray().GetAt(TangentValueIndex);
								TangentX = ClusterDeformation.MultT(TangentX);
								TangentX = TotalMatrixForNormal.MultT(TangentX);
								Triangle.TangentX[UnrealVertexIndex] = (FVector3f)Converter.ConvertDir(TangentX);
								Triangle.TangentX[UnrealVertexIndex].Normalize();
							}
							else
							{
								Triangle.TangentX[UnrealVertexIndex] = FVector3f();
							}
							...
							if (LayerElementBinormal)
							{
								...
								FbxVector4 TangentY = LayerElementBinormal->GetDirectArray().GetAt(BinormalValueIndex);
								TangentY = ClusterDeformation.MultT(TangentY);
								TangentY = TotalMatrixForNormal.MultT(TangentY);
								Triangle.TangentY[UnrealVertexIndex] = (FVector3f)Converter.ConvertDir(TangentY);
								Triangle.TangentY[UnrealVertexIndex].Normalize();
							}
							else
							{
								Triangle.TangentY[UnrealVertexIndex] = FVector3f();
							}
						}

						if (LayerElementNormal)
						{
							FbxVector4 TangentZ = LayerElementNormal->GetDirectArray().GetAt(NormalValueIndex);
							TangentZ = ClusterDeformation.MultT(TangentZ);
							TangentZ = TotalMatrixForNormal.MultT(TangentZ);
							Triangle.TangentZ[UnrealVertexIndex] = (FVector3f)Converter.ConvertDir(TangentZ);
							Triangle.TangentZ[UnrealVertexIndex].Normalize();
						}
						else
						{
							Triangle.TangentZ[UnrealVertexIndex] = FVector3f();
						}
					}
				}
			}
...
USkeletalMesh* UnFbx::FFbxImporter::ImportSkeletalMesh(FImportSkeletalMeshArgs &ImportSkeletalMeshArgs)
{
	...
	{
		UObject* ExistingObject = StaticFindObjectFast(UObject::StaticClass(), ImportSkeletalMeshArgs.InParent, ImportSkeletalMeshArgs.Name, EFindObjectFlags::None, RF_NoFlags, EInternalObjectFlags::Garbage);
...
USkeletalMesh* UnFbx::FFbxImporter::ReimportSkeletalMesh(USkeletalMesh* Mesh, UFbxSkeletalMeshImportData* TemplateImportData, uint64 SkeletalMeshFbxUID, TArray<FbxNode*> *OutSkeletalMeshArray)
{
	...
	if (FbxNodes)
	{
		...
		FScopedSlowTask SlowTask(100.0f, NSLOCTEXT("SkeletalMeshReimportFactory", "SkeletalMeshReimportTask", "Reimporting skeletal mesh..."));
		SlowTask.MakeDialog();
