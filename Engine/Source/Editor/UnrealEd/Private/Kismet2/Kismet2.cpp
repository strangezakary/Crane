...
{
	/** Flag to indicate whether a log should be produced when users explicitly request a compilation of a blueprint class */
	bool bLogExplicitCompilationRequests = true;
	static FAutoConsoleVariableRef CVarLogExplicitCompilationRequests(
		TEXT("BP.bLogExplicitCompilationRequests"), bLogExplicitCompilationRequests,
		TEXT("If true, CompileBlueprint will emit a log indicating the class getting recompiled."),
		ECVF_Default);
}
...
bool FKismetEditorUtilities::IsReferencedByUndoBuffer(UBlueprint* Blueprint)
{
	...
	if (Transactor)
	{
		Transactor->DisableObjectSerialization();
	}
	...
	if (Transactor)
	{
		Transactor->EnableObjectSerialization();
	}
...
void FKismetEditorUtilities::CompileBlueprint(UBlueprint* BlueprintObj, EBlueprintCompileOptions CompileFlags, FCompilerResultsLog* pResults)
{
	...
	if (UE::Private::bLogExplicitCompilationRequests
		&& !EnumHasAnyFlags(CompileFlags, EBlueprintCompileOptions::IsRegeneratingOnLoad))
	{
		LOG_SCOPE_VERBOSITY_OVERRIDE(LogBlueprint, ELogVerbosity::Log);
		UE_LOG(LogBlueprint, Log, TEXT("Compiling Blueprint '%s'"), *GetPathNameSafe(BlueprintObj));
	}
