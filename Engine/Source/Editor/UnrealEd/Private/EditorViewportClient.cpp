...
static TAutoConsoleVariable<bool> CVarEditorDisableFogInOrthoDebugViews(
	TEXT("r.Editor.DisableFogInOrthoDebugViews"),
	true,
	TEXT("Fog can obstruct the view when displaying it in ortho debug views without the correct settings adjusted, so this override disables by default."),
	ECVF_RenderThreadSafe);
...
static TAutoConsoleVariable<int32> CVarEditorViewportManualOrthographicDepthControl(
	TEXT("r.Editor.ManualOrthoDepth"),
	1,
	TEXT("Enable or disable the new orthographic depth tools."),
	ECVF_RenderThreadSafe);
...
/**
 * Returns true if any viewport interaction of the specified type is active
 */
bool IsViewportInteractionActive(const FEditorModeTools* InModeTools, FName InViewportInteraction)
{
	if (InModeTools)
	{
		if (UViewportInteractionsBehaviorSource* InputToolsBehaviorSource =
				InModeTools->GetInteractiveToolsContext()->GetViewportInteractionsBehaviorSource())
		{
			return InputToolsBehaviorSource->IsViewportInteractionActive(InViewportInteraction);
		}
	}
...
namespace PreviewLightConstants
{
	...
	const float MouseLengthToArrowLengthRatio = 0.2f;
...
namespace GeometryInspectionZebraMaterialParameterNames
{
	const FName Orientation = TEXT("XYZ");
	const FName StripeCount = TEXT("Stripe Counts");
}
...
//
///////////////////////////////////////////////////////////////////////////////////////////////////
FViewportCursorLocation::FViewportCursorLocation(const FSceneView* View, FEditorViewportClient* InViewportClient, int32 X, int32 Y, const bool bInClampToBounds)
	: Origin(ForceInit), Direction(ForceInit), CursorPos(X, Y)
{
	const FVector4 ScreenPos =
		bInClampToBounds
		? View->CursorToScreen(static_cast<float>(X), static_cast<float>(Y), 0)
		: View->PixelToScreen(static_cast<float>(X), static_cast<float>(Y), 0);
...
{
	return GetCameraSpeedSettings().GetCurrentSpeed();
}

const FEditorViewportCameraSpeedSettings& FEditorViewportClient::GetCameraSpeedSettings() const
{
	return CameraSpeedSettings;
}

void FEditorViewportClient::SetCameraSpeedSettings(const FEditorViewportCameraSpeedSettings& InCameraSpeedSettings)
{
	CameraSpeedSettings = InCameraSpeedSettings;
}
...
FEditorViewportClient::FEditorViewportClient(FEditorModeTools* InModeTools, FPreviewScene* InPreviewScene, const TWeakPtr<SEditorViewport>& InEditorViewportWidget)
	...
	, ViewportNavigationHelper(new FViewportClientNavigationHelper())
	...
	, bIsTrackingBeingStopped(false)
{
	...
	FCoreUObjectDelegates::OnObjectTransacted.AddRaw(this, &FEditorViewportClient::OnObjectTransacted);
	FEditorDelegates::MapChange.AddRaw(this, &FEditorViewportClient::OnPostLoadMap);
}
...
FVector FEditorViewportClient::GetForwardVector() const
{
	switch (GetViewportType())
	{
	case LVT_Perspective:
		return ViewTransformPerspective.GetRotation().Vector();
	case LVT_OrthoFreelook:
		return ViewTransformOrthographic.GetRotation().Vector();
	case LVT_OrthoTop:
...
void FEditorViewportClient::FocusViewportOnBox( const FBox& BoundingBox, bool bInstant /* = false */ )
{
...

	{
		...
		{
			if (SupportsManualOrthographicDepthControl())
			{
				const FVector StartingLocation = ViewTransform.GetLocation();
				const FVector Forward = GetForwardVector();
				
				// Adjust the camera by the current forward instantly
				const FVector LocationDelta = Position - StartingLocation;
				const FVector ForwardLocationDelta = LocationDelta.ProjectOnToNormal(Forward);
				ViewTransform.SetLocation(StartingLocation + ForwardLocationDelta);
				
				// Adjust the near & far planes so that they remain fixed in world space
				const double ForwardDelta = ForwardLocationDelta.Dot(Forward);
				
				if (OrthographicNearPlane.IsSet())
				{
					OrthographicNearPlane = OrthographicNearPlane.GetValue() - ForwardDelta;
				}
				
				if (OrthographicFarPlane.IsSet())
				{
					OrthographicFarPlane = OrthographicFarPlane.GetValue() - ForwardDelta;
				}
				
				if (GetClipToSelectionOnFocus())
				{
					// Shift the orthographic near view plane back so that it is just before the object
					OrthographicNearPlane = -Radius;
					
					// Ensure the orthographic far view plane can display the object
					if (OrthographicFarPlane.IsSet())
					{
						OrthographicFarPlane = FMath::Max(OrthographicFarPlane.GetValue(), Radius);
					}
				}
			}
			...
			if (!(Viewport->KeyState(EKeys::LeftControl) || Viewport->KeyState(EKeys::RightControl)))
			{
				...
				*/
				...
				if (ViewportSize.X > 0 && ViewportSize.Y > 0)
				{
					...
					NewOrthoZoom = Zoom * (MinAxisSize * 15.0f);
				}
...
void FEditorViewportClient::SetClipToSelectionOnFocus(bool bInClipToSelection)
{
	bClipToSelectionOnFocus = bInClipToSelection;
}
...
FSceneView* FEditorViewportClient::CalcSceneView(FSceneViewFamily* ViewFamily, const int32 StereoViewIndex)
{
	...
	else
	{
		...
		else
		{
			...
			if (SupportsManualOrthographicDepthControl())
			{
				...
				CalculatePlanesViewInfo.bUseCameraHeightAsViewTarget = false;
				CalculatePlanesViewInfo.OrthoNearClipPlane = -UE_OLD_WORLD_MAX;
				CalculatePlanesViewInfo.OrthoFarClipPlane = UE_OLD_WORLD_MAX;

				SDepthBar::FDepthSpace DepthSpace(GetViewLocation(), GetForwardVector(), GetWorldBounds());
				
				if (OrthographicNearPlane.IsSet())
				{
					CalculatePlanesViewInfo.OrthoNearClipPlane = OrthographicNearPlane.GetValue();
				}
				else if (!ViewFamily->EngineShowFlags.Wireframe)
				{
					CalculatePlanesViewInfo.OrthoNearClipPlane = DepthSpace.GetAlignedMin();
				}
				
				if (OrthographicFarPlane.IsSet())
				{
					CalculatePlanesViewInfo.OrthoFarClipPlane = OrthographicFarPlane.GetValue();
				}
				else if (!ViewFamily->EngineShowFlags.Wireframe)
				{
					CalculatePlanesViewInfo.OrthoFarClipPlane = DepthSpace.GetAlignedMax();
				}
				
				ViewInitOptions.UpdateOrthoPlanes(CalculatePlanesViewInfo);
				const float Denominator = CalculatePlanesViewInfo.OrthoFarClipPlane - CalculatePlanesViewInfo.OrthoNearClipPlane;
				if (Denominator > 0.0f)
				{
					ZScale = 1.0f / Denominator;
			...
			/**
			* Unmanaged fog obscures the Ortho lit view modes so we disable it for these views unless the user overrides the setting.
			*/
			if (CVarEditorDisableFogInOrthoDebugViews->GetBool())
			{
	...
	if (RenderViewportFeedback && (StereoViewIndex == INDEX_NONE || StereoViewIndex == 0))
	{
		View->ViewportFeedback = RenderViewportFeedback->MakeCollector();
	}
...
void FEditorViewportClient::Tick(float DeltaTime)
{
	...
	if (!CachedWorldBounds.IsValid)
	{
		if (UWorld* World = GetWorld())
		{
			...
			{
				CachedWorldBounds = WorldPartition->GetEditorWorldBounds();
			}
			else
			{
				CachedWorldBounds = FBox(ForceInitToZero);
				
				for (int32 LevelIndex = 0; LevelIndex < World->GetNumLevels(); ++LevelIndex)
				{
					if (ULevel* Level = World->GetLevel(LevelIndex))
					{
						if (ALevelBounds* LevelBounds = Level->LevelBoundsActor.Get())
						{
							CachedWorldBounds += LevelBounds->GetComponentsBoundingBox();
						}
						else
						{
							CachedWorldBounds += ALevelBounds::CalculateLevelBounds(Level);
						}
					}
				}
			}
...
EMouseCursor::Type FEditorViewportClient::GetCursor(FViewport* InViewport,int32 X,int32 Y)
{
	...
	else if (InViewport->IsCursorVisible() && !bWidgetAxisControlledByDrag)
	{
		...
		else if (!ModeTools->HasOngoingTransform())
		{
...
TOptional<TSharedRef<SWidget>> FEditorViewportClient::MapCursor(FViewport* InViewport, const FCursorReply& CursorReply)
{
	// allow editor modes to override cursor
	if (const TSharedPtr<SWidget> MappedCursor = ModeTools->MapCursor();
		MappedCursor.IsValid())
	{
		return MappedCursor.ToSharedRef();
	}
	
	return FCommonViewportClient::MapCursor(InViewport, CursorReply);
}
...
void FEditorViewportClient::UpdateCameraMovement( float DeltaTime )
{
	...
	if( Viewport != nullptr && IsPerspective() && !ShouldOrbitCamera() )
	{
		...
		const bool bUsingFlightInput = IsFlightCameraInputModeActive() || bIsUsingTrackpad || ViewportNavigationHelper->bIsMouseLooking;
		...
		const bool bRemapArrowKeys = bUnmodifiedPress || IsViewportInteractionActive(ModeTools.Get(), UE::Editor::ViewportInteractions::CameraTranslate);

		const bool bITFCameraTranslateActive = IsViewportInteractionActive(ModeTools.Get(),UE::Editor::ViewportInteractions::CameraTranslate);
		const bool bITFCameraRotateActive = IsViewportInteractionActive(ModeTools.Get(),UE::Editor::ViewportInteractions::CameraRotate);
		const bool bITFFOVActive = IsViewportInteractionActive(ModeTools.Get(),UE::Editor::ViewportInteractions::FOV);
		...
		// Check if we need to use legacy translation input
		if (!bITFCameraTranslateActive)
		{
			// Forward/back
		...
		// Check if we need to use legacy FOV input
		if (!bITFFOVActive)
		{
			// Zoom FOV out/in
		...
		// Check if we need to use legacy rotation input
		if (!bITFCameraRotateActive)
		{
			// Rotate up/down
		...
		// Add camera inputs coming from ITF, if any
		if (bITFCameraTranslateActive || bITFCameraRotateActive || bITFFOVActive)
		{
			ViewportNavigationHelper->ConsumeImpulseData(*CameraUserImpulseData);
			CameraController->GetConfig().bForceRotationalPhysics = true;
		}
		...
		if(bTransformDifferent || bFOVDifferent)
		{
			...
			const bool bInvalidateHitProxies= UE::Editor::ViewportInteractions::UseEditorViewportInteractions()
            	? !ViewportNavigationHelper->bIsMouseLooking
...
void FEditorViewportClient::UpdateMouseDelta()
{
	...
	if( Viewport )
	{
		// If mouse delta tracker has delta OR there is transform delta coming from ITF Viewport Interaction, handle it
		if (!DragDelta.IsNearlyZero() || ViewportNavigationHelper->HasTransformDelta())
		{
			...
			// Try to handle ITF Viewport inputs before legacy (currently zoom and camera drag ones)
			if (IsViewportInteractionActive(ModeTools.Get(), UE::Editor::ViewportInteractions::Zoom) ||
				IsViewportInteractionActive(ModeTools.Get(), UE::Editor::ViewportInteractions::CameraDrag))
			{
				ViewportNavigationHelper->ConsumeTransformDelta(Drag, Rot);
			}
			else if ( IsOrtho() && ( LeftMouseButtonDown || bIsUsingTrackpad ) && RightMouseButtonDown )
			{
				bWidgetAxisControlledByDrag = false;
			...
			else
			{
				...
				{
					// TODO: yet to be checked out for ITF Parity
...
{
	// Check if there is an active ITF Orbit interaction, and if there's an orbit delta it might have generated 
	const bool bUseITFOrbitDelta = IsViewportInteractionActive(ModeTools.Get(), UE::Editor::ViewportInteractions::Orbit)
					  && !ViewportNavigationHelper->OrbitDelta.IsNearlyZero();

	FVector DragDelta;

	if (bUseITFOrbitDelta)
	{
		ViewportNavigationHelper->ConsumeOrbitDelta(DragDelta);
	}
	else // no ITF delta, try to use the one from mouse delta tracker
	{
...
{
	EViewModeIndex ViewModeIndex = GetVirtualShadowMapVisualizationData().GetViewModeIndexForVisualization(InName);
...
{
	return GetVirtualShadowMapVisualizationData().IsVSMViewMode(GetViewMode()) && CurrentVirtualShadowMapVisualizationMode == InName;
}

FText FEditorViewportClient::GetCurrentVirtualShadowMapVisualizationModeDisplayName() const
{
	checkf(GetVirtualShadowMapVisualizationData().IsVSMViewMode(GetViewMode()), TEXT("In order to call GetCurrentVirtualShadowMapVisualizationMode(), the ViewMode must first be set to VMI_VisualizeVirtualShadowMap or another compatible one."));
...
bool FEditorViewportClient::Internal_InputKey(const FInputKeyEventArgs& EventArgs)
{
	...
	bool bRouteToToolsContext = !bIsTracking || UE::Editor::ViewportInteractions::UseEditorViewportInteractions();
	...
	if (ModeTools->InputKey(this, Viewport, Key, Event, bRouteToToolsContext))
	{
		return true;
	}

	if (GetViewportNavigationHelper()->bIsMouseLooking)
	{
		return true;
	}
	...
	const bool bIsITFSingleClickActive = IsViewportInteractionActive(ModeTools.Get(), UE::Editor::ViewportInteractions::SingleClick);
	...
	if ( bIsTracking && !InputState.IsAnyMouseButtonDown() && InputState.IsMouseButtonEvent() && !bIsITFSingleClickActive)
	{
...
void FEditorViewportClient::Draw(FViewport* InViewport, FCanvas* Canvas)
{
	...
	// Set the view hit proxies flag as early as possible (but after EngineShowFlagOverride) so that view extensions can check whether they render in the hit proxy pass in SetupView
...
void FEditorViewportClient::DrawPreviewLightVisualization(const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
	...
	if ((PreviewScene != nullptr) && (PreviewScene->DirectionalLight != nullptr) && (MovingPreviewLightTimer > 0.0f))
	{
		...
		const float ArrowLength = FMath::Max(PreviewLightConstants::MinArrowLength, LightToMouseRadius * PreviewLightConstants::MouseLengthToArrowLengthRatio);
...
bool FEditorViewportClient::Internal_InputAxis(FViewport* InViewport, FInputDeviceId DeviceID, FKey Key, float Delta, float DeltaTime, int32 NumSamples, bool bGamepad)
{
	...
	// No need to process anything else here, since some ITF Viewport interaction is handling the camera
	if (GetViewportNavigationHelper()->bIsMouseLooking)
	{
		return true;
	}
	...
	case LVT_OrthoFreelook:
		{
			...
			else if( MiddleMouseButtonDown || bIsUsingTrackpad || ( ( LeftMouseButtonDown || bIsUsingTrackpad ) && RightMouseButtonDown ) )
			{
				...
				// Treat the middle mouse pan as a distinct gesture from the left+right mouse button 
				if (MiddleMouseButtonDown && !ViewportSettings->bWorldSpaceVerticalPan)
				{
					// Pan relative to the camera
					InDragDelta = FQuat(ViewRotation) * FVector( 0.0f, InDelta.X, InDelta.Y) * (bInvert ? -1.0f : 1.0f);
				}
				else
				{
...
bool FEditorViewportClient::SupportsManualOrthographicDepthControl() const
{
	return CVarEditorViewportManualOrthographicDepthControl.GetValueOnAnyThread() > 0;
}

TOptional<double> FEditorViewportClient::GetOrthographicNearPlaneOverride() const
{
	return OrthographicNearPlane;
}

void FEditorViewportClient::SetOrthographicNearPlaneOverride(const TOptional<double>& InValue)
{
	OrthographicNearPlane = InValue;
}

TOptional<double> FEditorViewportClient::GetOrthographicFarPlaneOverride() const
{
	return OrthographicFarPlane;
}

void FEditorViewportClient::SetOrthographicFarPlaneOverride(const TOptional<double>& InValue)
{
	OrthographicFarPlane = InValue;
}

FBox FEditorViewportClient::GetWorldBounds() const
{
...
{
	// If it has been decided that there is more important things to do than check hit proxies, or our mouse location is not valid, then don't check them.
	if( !bShouldCheckHitProxy || bWidgetAxisControlledByDrag == true || CurrentMousePos == FIntPoint(-1, -1) )
	{
		return;
	}
...
void FEditorViewportClient::SetZebraViewMode(bool bHorizontalZebraMode)
{
	if (UMaterialInstanceDynamic* MaterialInstance = Cast<UMaterialInstanceDynamic>(GEngine->ZebraMaterialInstance))
	{
		FLinearColor Color  = bHorizontalZebraMode ? FLinearColor(0, 0, 1) : FLinearColor(1, 0, 0);

		MaterialInstance->SetVectorParameterValue(GeometryInspectionZebraMaterialParameterNames::Orientation, Color);
	}

	SetViewMode(VMI_Zebra);
}

bool FEditorViewportClient::IsZebraViewModeEnabled(bool bHorizontal) const
{
	if (IsViewModeEnabled(VMI_Zebra))
	{
		if (UMaterialInstanceDynamic* MaterialInstance = Cast<UMaterialInstanceDynamic>(GEngine->ZebraMaterialInstance))
		{
			FLinearColor Color;
			if (MaterialInstance->GetVectorParameterValue(GeometryInspectionZebraMaterialParameterNames::Orientation, Color))
			{
				if (Color.B == 1.0f && bHorizontal)
				{
					return true;
				}

				if (Color.R == 1.0f && !bHorizontal)
				{
					return true;
				}
...
void FEditorViewportClient::SetZebraViewModeStripeCount(uint8 InStripeCount)
{
	if (InStripeCount>0 && InStripeCount<32)
	{
		if (UMaterialInstanceDynamic* MaterialInstance = Cast<UMaterialInstanceDynamic>(GEngine->ZebraMaterialInstance))
		{
			MaterialInstance->SetScalarParameterValue(GeometryInspectionZebraMaterialParameterNames::StripeCount, InStripeCount);
...
uint8 FEditorViewportClient::GetZebraViewModeStripeCount()
{
	if (UMaterialInstanceDynamic* MaterialInstance = Cast<UMaterialInstanceDynamic>(GEngine->ZebraMaterialInstance))
	{
		FHashedMaterialParameterInfo ParameterInfo(GeometryInspectionZebraMaterialParameterNames::StripeCount);
		float Value;
...
void FEditorViewportClient::OnObjectTransacted(UObject* Object, const FTransactionObjectEvent& Event)
{
	if (Event.GetEventType() == ETransactionObjectEventType::Snapshot)
	{
		return;
	}
	...
	{
		CachedWorldBounds.Init();
	}
}

void FEditorViewportClient::OnPostLoadMap(uint32 MapChangeFlags)
{
	if (GWorld == GetWorld())
	{
		CachedWorldBounds.Init();
		OrthographicFarPlane.Reset();
		OrthographicNearPlane.Reset();
	}
}
...
UE_DEFINE_TCOMMANDS(FViewportNavigationCommands)
