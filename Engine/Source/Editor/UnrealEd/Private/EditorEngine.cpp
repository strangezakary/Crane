...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EditorEngine)
...
void FPreviewPlatformInfo::InternalSetFriendlyName()
{
	...
	{
		PreviewShaderPlatformFriendlyName = FDataDrivenShaderPlatformInfo::GetFriendlyName(GetShaderPlatform(), DeviceProfileName);
	}
}

void FAssetReferenceFilterContext::AddReferencingAsset(const UObject* InReferencingAsset, FAssetData::ECreationFlags InCreationFlags, EAssetReferenceFilterProperties InProperties)
{
	if (InReferencingAsset != nullptr)
	{
		FAssetData ReferencingObjectData(InReferencingAsset, InCreationFlags);

		ReferencingAssetInfo.Emplace(ReferencingObjectData, InProperties);
...
void FAssetReferenceFilterContext::AddReferencingAssets(TConstArrayView<UObject*> InReferencingAssets, FAssetData::ECreationFlags InCreationFlags, EAssetReferenceFilterProperties InProperties)
{
	for (const UObject* ReferencingAsset : InReferencingAssets)
	{
		AddReferencingAsset(ReferencingAsset, InCreationFlags, InProperties);
	}
}
...
void FAssetReferenceFilterContext::AddReferencingAssets(const TArray<FAssetData>& InReferencingAssets, EAssetReferenceFilterProperties InProperties)
{
	...
	{
		AddReferencingAsset(ReferencingAsset, InProperties);
	}
}

void FAssetReferenceFilterContext::AddReferencingAssetsFromPropertyHandle(const TSharedPtr<IPropertyHandle>& PropertyHandle, FAssetData::ECreationFlags InCreationFlags)
{
	...
	{
		AddReferencingAsset(ReferencingObject, InCreationFlags, ReferencerProperties);
	}
}
...
void UEditorEngine::Init(IEngineLoop* InEngineLoop)
{
	...
	FWorldDelegates::OnSeamlessTravelTransition.AddUObject(this, &UEditorEngine::OnSeamlessTravelTransition);
	...
	LoadEditorShaderPlatformPreview();
...
UAudioComponent* UEditorEngine::ResetPreviewAudioComponent(USoundBase* Sound, USoundNode* SoundNode)
{
	if (PreviewSoundCue)
	{
		PreviewSoundCue->FirstNode = nullptr;
		PreviewSoundCue = nullptr;
	}

	// The audio engine may be disabled
	if (!UseSound())
	{
		return nullptr;
	}

	// 1. Initialize & overwrite new component for previewing
	...
	if (Sound)
	{
		PreviewAudioComponent = FAudioDevice::CreateComponent(Sound);
	}
	else if (SoundNode)
	{
	...
	{
		PreviewAudioComponent = NewObject<UAudioComponent>();
	}

	check(PreviewAudioComponent);

	// Mark as a preview component so the distance calculations is ignored
	PreviewAudioComponent->bPreviewComponent = true;
	PreviewAudioComponent->bIsPreviewSound = true;
	PreviewAudioComponent->bAutoDestroy = false;
	PreviewAudioComponent->bIsUISound = true;
	PreviewAudioComponent->bAllowSpatialization = false;
	PreviewAudioComponent->bReverb = false;
	PreviewAudioComponent->bCenterChannelOnly = false;

	PreviewAudioComponent->AudioDeviceID = INDEX_NONE;
	if (FAudioDeviceManager* DeviceManager = GEngine->GetAudioDeviceManager())
	{
		if (FAudioDeviceHandle ActiveDevice = DeviceManager->GetActiveAudioDevice(); ActiveDevice.IsValid())
		{
			PreviewAudioComponent->AudioDeviceID = ActiveDevice->DeviceID;
		}
	}
	...
	{
		check(PreviewSoundCue);
		PreviewSoundCue->FirstNode = SoundNode;
		PreviewAudioComponent->Sound = PreviewSoundCue;
		PreviewSoundCue->CacheAggregateValues();
	}

	// 2. Determine whether or not to apply submix behavior
	{
		const UAudioEditorSettings* AudioEditorSettings = GetDefault<UAudioEditorSettings>();
		check(AudioEditorSettings);
		PreviewAudioComponent->bIsDryPreviewPlayback = !AudioEditorSettings->bUseSubmixesForPreviewSound;
		if (const USoundBase* PreviewSound = PreviewAudioComponent->Sound)
		{
			if (PreviewSound->EditorData.OverrideUseSubmixesOnPreview != EUseSubmixOnPreviewMode::UseEditorPreference)
			{
				PreviewAudioComponent->bIsDryPreviewPlayback = PreviewSound->EditorData.OverrideUseSubmixesOnPreview == EUseSubmixOnPreviewMode::PlayPreviewsWithoutSubmixes;
			}
		}
	}
...
void UEditorEngine::ClearPreviewComponents()
{
	if (PreviewAudioComponent)
	{
		...
		PreviewAudioComponent->Sound = nullptr;
...
UBrushBuilder* UEditorEngine::FindBrushBuilder( UClass* BrushBuilderClass )
{
	...
	if( ContainsObjectOfClass<UBrushBuilder>( BrushBuilders, BrushBuilderClass, EFindObjectFlags::ExactClass, &FoundBuilders ) )
	{
	...
	if (ISourceControlModule::Get().IsEnabled())
	{
		...
		if(SourceControlProvider.IsAvailable())
		{
			...
			if (Result == ECommandResult::Failed && !SourceControlFailureNotificationPtr.Pin().IsValid())
			{
				...
				if (SourceControlProvider.UsesChangelists() || SourceControlProvider.UsesUncontrolledChangelists())
				{
...
void UEditorEngine::OnPreSaveWorld(UWorld* World, FObjectPreSaveContext ObjectSaveContext)
{
	...
	const bool bAutosave = ObjectSaveContext.IsFromAutoSave();
	if (bAutosave)
	{
...
void UEditorEngine::OnSeamlessTravelTransition(UWorld* InWorld)
{
	if (InWorld->IsPlayInEditor())
	{
		EndTransaction();
...
void UEditorEngine::UpdateAutoLoadProject()
{
	...
	// Sync up the new LoadAtStartup setting with the old one
	if (UClass* SettingsClass = FindObject<UClass>(nullptr, TEXT("/Script/MainFrame.HomeScreenSettings")))
	{
		UObject* DefaultObj = SettingsClass->GetDefaultObject();
		FProperty* Prop = SettingsClass->FindPropertyByName("LoadAtStartup");
		if (Prop && DefaultObj && Prop->IsA<FEnumProperty>())
		{
			FEnumProperty* AutoLoadProperty = CastFieldChecked<FEnumProperty>(Prop);
			EAutoLoadProject Value = bShouldLoadRecentProjects ? EAutoLoadProject::LastProject : EAutoLoadProject::HomeScreen;
			AutoLoadProperty->SetValue_InContainer(DefaultObj, &Value);
		}
	}
...
static void SaveShaderPlatformAsDisabled(FPreviewPlatformInfo& PreviewPlatform)
{
...
void UEditorEngine::SetPreviewPlatform(const FPreviewPlatformInfo& NewPreviewPlatform, bool bSaveSettings)
{
	...
	SaveShaderPlatformAsDisabled(PreviewPlatform);
	...
	if (bSaveSettings)
	{
		SaveEditorShaderPlatformPreview();
	}
}
...
EShaderPlatform UEditorEngine::GetActiveShaderPlatform() const
{
	if (PreviewPlatform.bPreviewFeatureLevelActive)
	{
		return GShaderPlatformForFeatureLevel[PreviewPlatform.PreviewFeatureLevel];
	}
	else
	{
...
FText UEditorEngine::GetActiveShaderPlatformFriendlyName() const
{
	if (PreviewPlatform.bPreviewFeatureLevelActive)
	{
		return PreviewPlatform.GetFriendlyName();
	}
	else
	{
		return FDataDrivenShaderPlatformInfo::GetFriendlyName(GMaxRHIShaderPlatform);
	}
}
...
void UEditorEngine::LoadEditorShaderPlatformPreview()
{
	auto* Settings = GetMutableDefault<UEditorPerProjectUserSettings>();
	...
	if (Settings->PreviewFeatureLevel >= 0 && Settings->PreviewFeatureLevel < (int32)ERHIFeatureLevel::Num && ShaderPlatformToPreview < EShaderPlatform::SP_NumPlatforms)
	{
		...
		SetPreviewPlatform(FPreviewPlatformInfo(
			(ERHIFeatureLevel::Type)Settings->PreviewFeatureLevel, 
			ShaderPlatformToPreview, 
			Settings->PreviewPlatformName, 
			Settings->PreviewShaderFormatName, 
			Settings->PreviewDeviceProfileName, 
			Settings->bPreviewFeatureLevelActive, 
			PreviewShaderPlatformName, 
			FText(),
			Settings->PreviewConstrainedAspectRatio,
			Settings->PreviewSafeZones),
			false);
	}
}

void UEditorEngine::SaveEditorShaderPlatformPreview()
{
	auto* Settings = GetMutableDefault<UEditorPerProjectUserSettings>();
	Settings->PreviewFeatureLevel = (int32)PreviewPlatform.PreviewFeatureLevel;
	Settings->PreviewPlatformName = PreviewPlatform.PreviewPlatformName;
	...
	Settings->PreviewDeviceProfileName = PreviewPlatform.DeviceProfileName;
	Settings->PreviewShaderPlatformName = PreviewPlatform.PreviewShaderPlatformName;
	Settings->PreviewConstrainedAspectRatio = PreviewPlatform.GetConstrainedAspectRatio();
	Settings->PreviewSafeZones = PreviewPlatform.GetSafeZones();
