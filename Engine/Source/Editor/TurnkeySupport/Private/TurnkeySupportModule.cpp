...
namespace 
{
	...
	// The host platform's IniPlatformName, used to pick defaults.
#if PLATFORM_WINDOWS
	const FName GHostIniPlatformName("Windows");
#elif PLATFORM_MAC
	const FName GHostIniPlatformName("Mac");
#else
	// NAME_None disables this behavior and uses the fallback behavior for choosing a default.
	const FName GHostIniPlatformName(NAME_None);
#endif
}
...
public:
	...
	static void CookOrPackage(FName IniPlatformName, const FString& BuildTargetName, EPrepareContentMode Mode)
	{
		...
		const FTargetInfo* BuildTargetInfo = PlatformsSettings->GetBuildTargetInfo(BuildTargetName, bIsProjectBuildTarget);
		...
		// don't report warnings and errors twice
		BuildCookRunParams += TEXT(" -SkipCookingErrorSummary");

		// use structured logging if available
		BuildCookRunParams += TEXT(" -JsonStdOut");
		...
		if (!Architecture.IsEmpty())
		{
			...
			{
				// Added in 5.7.2 to help diagnose UE-316569, a crash at the switch statement above because BuildTargetInfo is null.
				ensureMsgf(BuildTargetInfo, TEXT("CookOrPackage BuildTargetName='%s' IniPlatformName='%s' Mode='%i' BuildCookRunParams='%s 'where BuildTargetInfo is null and architecture '%s' is specified.  One way to do this is to add c++ code to a project, but not build the project editor, then package. Returning without cook or package instead of asserting."), *BuildTargetName, *IniPlatformName.ToString(), Mode, *BuildCookRunParams, *Architecture);
				return;
			}
		}
		...
		if (Mode == EPrepareContentMode::Package)
		{
			...
			EProjectPackagingBuildConfigurations BuildConfig = PlatformsSettings->GetPackageBuildConfiguration();
	...
	static FName GetPlatformForCustomBuild(ECustomBuildType Type, FString CustomBuildName)
	{
		return GetDefault<UPlatformsMenuSettings>()->GetPlatformForCustomBuild(Type, CustomBuildName);
	}

	static void SetPlatformForCustomBuild(ECustomBuildType Type, FString CustomBuildName, FName IniPlatformName)
	{
		UPlatformsMenuSettings* PlatformsSettings = GetMutableDefault<UPlatformsMenuSettings>();
		PlatformsSettings->SetPlatformForCustomBuild(Type, CustomBuildName, IniPlatformName);
		PlatformsSettings->SaveConfig();
	}

	static bool PlatformForCustomBuildIsChecked(ECustomBuildType Type, FString CustomBuildName, FName IniPlatformName)
	{
		const UPlatformsMenuSettings* PlatformsSettings = GetDefault<UPlatformsMenuSettings>();
		FName CheckedPlatform = PlatformsSettings->GetPlatformForCustomBuild(Type, CustomBuildName);
		return CheckedPlatform == IniPlatformName;
	}

	// Per-device custom builds show a list of devices rather than platforms. Since we know the platform associated with
	// each device, we set both at once to ensure the platform is set.
	static void SetDeviceIdForCustomBuild(ECustomBuildType Type, FString CustomBuildName, FName IniPlatformName, FString DeviceId)
	{
		UPlatformsMenuSettings* PlatformsSettings = GetMutableDefault<UPlatformsMenuSettings>();
		PlatformsSettings->SetPlatformForCustomBuild(Type, CustomBuildName, IniPlatformName);
		PlatformsSettings->SetDeviceIdForCustomBuild(Type, CustomBuildName, DeviceId);
		PlatformsSettings->SaveConfig();
	}

	static bool DeviceIdForCustomBuildIsChecked(ECustomBuildType Type, FString CustomBuildName, FString DeviceId)
	{
		const UPlatformsMenuSettings* PlatformsSettings = GetDefault<UPlatformsMenuSettings>();
		FString CheckedDeviceId = PlatformsSettings->GetDeviceIdForCustomBuild(Type, CustomBuildName);
		return CheckedDeviceId == DeviceId;
	}

	static void SetBuildConfigurationForCustomBuild(ECustomBuildType Type, FString CustomBuildName, EProjectPackagingBuildConfigurations BuildConfiguration)
	{
		UPlatformsMenuSettings* PlatformsSettings = GetMutableDefault<UPlatformsMenuSettings>();
		PlatformsSettings->SetBuildConfigurationForCustomBuild(Type, CustomBuildName, BuildConfiguration);
		PlatformsSettings->SaveConfig();
	}

	static bool BuildConfigurationForCustomBuildIsChecked(ECustomBuildType Type, FString CustomBuildName, EProjectPackagingBuildConfigurations BuildConfiguration)
	{
		const UPlatformsMenuSettings* PlatformsSettings = GetDefault<UPlatformsMenuSettings>();
		EProjectPackagingBuildConfigurations CheckedBuildConfiguration = PlatformsSettings->GetBuildConfigurationForCustomBuild(Type, CustomBuildName);
		return CheckedBuildConfiguration == BuildConfiguration;
	}

	static void SetBuildTargetForCustomBuild(ECustomBuildType Type, FString CustomBuildName, FString TargetName)
	{
		UPlatformsMenuSettings* PlatformsSettings = GetMutableDefault<UPlatformsMenuSettings>();
		PlatformsSettings->SetBuildTargetForCustomBuild(Type, CustomBuildName, TargetName);
		PlatformsSettings->SaveConfig();
	}

	static bool BuildTargetForCustomBuildIsChecked(ECustomBuildType Type, FString CustomBuildName, FString TargetName)
	{
		const UPlatformsMenuSettings* PlatformsSettings = GetDefault<UPlatformsMenuSettings>();
		FString CheckedTargetName = PlatformsSettings->GetBuildTargetForCustomBuild(Type, CustomBuildName);
		return CheckedTargetName == TargetName;
	}

	static bool CanExecuteCustomBuild(ECustomBuildType Type, FString BuildName, bool bNeedsDevice)
	{
		// Read the settings associated with this build
		const UPlatformsMenuSettings* MenuSettings = GetDefault<UPlatformsMenuSettings>();

		FName IniPlatformName = MenuSettings->GetPlatformForCustomBuild(Type, BuildName);
		...
		if (bNeedsDevice)
		{
			FString DeviceId = MenuSettings->GetDeviceIdForCustomBuild(Type, BuildName);
	...
	static FString GetCustomBuildCommandLine(FName IniPlatformName, EProjectPackagingBuildConfigurations BuildConfigSetting, const FString& BuildTargetName, const FString& DeviceId, const FProjectBuildSettings& Build)
	{
		const FString ProjectPath = GetProjectPathForTurnkey();
	...
	static void ExecuteCustomBuild(ECustomBuildType Type, FProjectBuildSettings Build)
	{
		// Read the settings associated with this build
		const UPlatformsMenuSettings* MenuSettings = GetDefault<UPlatformsMenuSettings>();

		FName IniPlatformName = MenuSettings->GetPlatformForCustomBuild(Type, Build.Name);
		FString DeviceId = MenuSettings->GetDeviceIdForCustomBuild(Type, Build.Name);
		FString BuildTargetName = MenuSettings->GetBuildTargetForCustomBuild(Type, Build.Name);
		EProjectPackagingBuildConfigurations BuildConfig = MenuSettings->GetBuildConfigurationForCustomBuild(Type, Build.Name);
		...
		CommandLine += GetCustomBuildCommandLine(IniPlatformName, BuildConfig, BuildTargetName, DeviceId, Build);
		...
		if (bIsControlHeld)
		{
			...
			FTurnkeyEditorSupport::RunUAT(CommandLine, PlatformInfo->DisplayName, LOCTEXT("Turnkey_GettingCommandLine", "Copying Commandline"), LOCTEXT("Turnkey_CustomTaskNameCmdLine", "CustomCommandLine"), nullptr /* TaskIcon */, nullptr /* OptionalAnalyticsParamArray */,
	...
	static void SetPackageBuildConfiguration(EProjectPackagingBuildConfigurations BuildConfiguration)
	{
		UPlatformsMenuSettings* PlatformsSettings = GetMutableDefault<UPlatformsMenuSettings>();
		PlatformsSettings->SetPackageBuildConfiguration(BuildConfiguration);
		PlatformsSettings->SaveConfig();
	}

	static bool PackageBuildConfigurationIsChecked(EProjectPackagingBuildConfigurations BuildConfiguration)
	{
		EProjectPackagingBuildConfigurations CheckedBuildConfiguration = GetDefault<UPlatformsMenuSettings>()->GetPackageBuildConfiguration();
		return CheckedBuildConfiguration == BuildConfiguration;
	}
	...
	static void SetActiveArchitecture(FName IniPlatformName, FString Architecture)
	{
		UPlatformsMenuSettings* PlatformsSettings = GetMutableDefault<UPlatformsMenuSettings>();
		PlatformsSettings->SetArchitectureForPlatform(IniPlatformName, Architecture);
		PlatformsSettings->SaveConfig();
	}

	static bool SetActiveArchitectureIsChecked(FName IniPlatformName, FString Architecture)
	{
		return GetDefault<UPlatformsMenuSettings>()->GetArchitectureForPlatform(IniPlatformName) == Architecture;
	}

	static void SetPackageBuildTarget(FString TargetName)
	{
		UPlatformsMenuSettings* PlatformsSettings = GetMutableDefault<UPlatformsMenuSettings>();
		PlatformsSettings->PackageBuildTarget = MoveTemp(TargetName);
		PlatformsSettings->SaveConfig();
	}

	static bool PackageBuildTargetIsChecked(FString TargetName)
	{
		return GetDefault<UPlatformsMenuSettings>()->PackageBuildTarget == TargetName;
	}

	static void SetLaunchOnBuildConfiguration(EProjectPackagingBuildConfigurations BuildConfiguration)
	{
		UPlatformsMenuSettings* PlatformsSettings = GetMutableDefault<UPlatformsMenuSettings>();
		PlatformsSettings->SetLaunchOnBuildConfiguration(BuildConfiguration);
		PlatformsSettings->SaveConfig();
	}

	static bool LaunchOnBuildConfigurationIsChecked(EProjectPackagingBuildConfigurations BuildConfiguration)
	{
		EProjectPackagingBuildConfigurations CheckedBuildConfiguration = GetDefault<UPlatformsMenuSettings>()->GetLaunchOnBuildConfiguration();
		return CheckedBuildConfiguration == BuildConfiguration;
	}
	...
	{
		UPlatformsMenuSettings* PlatformsSettings = GetMutableDefault<UPlatformsMenuSettings>();
		PlatformsSettings->LaunchOnTarget = MoveTemp(TargetName);
		PlatformsSettings->SaveConfig();
	}
	...
	static FName GetCookPlatform()
	{
		return GetDefault<UPlatformsMenuSettings>()->CookPlatform;
	}

	static void SetCookPlatform(FName IniPlatformName)
	{
		UPlatformsMenuSettings* PlatformsSettings = GetMutableDefault<UPlatformsMenuSettings>();
		PlatformsSettings->CookPlatform = IniPlatformName;
		PlatformsSettings->SaveConfig();
	}

	static bool CookPlatformIsChecked(FName IniPlatformName)
	{
		return GetDefault<UPlatformsMenuSettings>()->CookPlatform == IniPlatformName;
	}

	static void SetCookBuildTarget(FString TargetName)
	{
		UPlatformsMenuSettings* PlatformsSettings = GetMutableDefault<UPlatformsMenuSettings>();
		PlatformsSettings->CookBuildTarget = MoveTemp(TargetName);
		PlatformsSettings->SaveConfig();
	}

	static bool CookBuildTargetIsChecked(FString TargetName)
	{
		return GetDefault<UPlatformsMenuSettings>()->CookBuildTarget == TargetName;
	}

	static bool CanCookContent()
	{
		return CanCookOrPackage(GetDefault<UPlatformsMenuSettings>()->CookPlatform, EPrepareContentMode::CookOnly);
	}

	static void CookContent()
	{
		const UPlatformsMenuSettings* PlatformsSettings = GetDefault<UPlatformsMenuSettings>();
		CookOrPackage(PlatformsSettings->CookPlatform, PlatformsSettings->CookBuildTarget, EPrepareContentMode::CookOnly);
	}

	static FName GetPackagePlatform()
	{
		return GetDefault<UPlatformsMenuSettings>()->PackagePlatform;
	}

	static void SetPackagePlatform(FName IniPlatformName)
	{
		UPlatformsMenuSettings* PlatformsSettings = GetMutableDefault<UPlatformsMenuSettings>();
		PlatformsSettings->PackagePlatform = IniPlatformName;
		PlatformsSettings->SaveConfig();
	}

	static bool PackagePlatformIsChecked(FName IniPlatformName)
	{
		return GetDefault<UPlatformsMenuSettings>()->PackagePlatform == IniPlatformName;
	}

	static bool CanPackageProject()
	{
		return CanCookOrPackage(GetDefault<UPlatformsMenuSettings>()->PackagePlatform, EPrepareContentMode::Package);
	}

	static void PackageProject()
	{
		const UPlatformsMenuSettings* PlatformsSettings = GetDefault<UPlatformsMenuSettings>();
		CookOrPackage(PlatformsSettings->PackagePlatform, PlatformsSettings->PackageBuildTarget, EPrepareContentMode::Package);
	}

	static FName GetPrepareForDebuggingPlatform()
	{
		return GetDefault<UPlatformsMenuSettings>()->PrepareForDebuggingPlatform;
	}

	static void SetPrepareForDebuggingPlatform(FName IniPlatformName)
	{
		UPlatformsMenuSettings* PlatformsSettings = GetMutableDefault<UPlatformsMenuSettings>();
		PlatformsSettings->PrepareForDebuggingPlatform = IniPlatformName;
		PlatformsSettings->SaveConfig();
	}

	static bool PrepareForDebuggingPlatformIsChecked(FName IniPlatformName)
	{
		return GetDefault<UPlatformsMenuSettings>()->PrepareForDebuggingPlatform == IniPlatformName;
	}

	static void SetPrepareForDebuggingBuildTarget(FString TargetName)
	{
		UPlatformsMenuSettings* PlatformsSettings = GetMutableDefault<UPlatformsMenuSettings>();
		PlatformsSettings->PrepareForDebuggingBuildTarget = MoveTemp(TargetName);
		PlatformsSettings->SaveConfig();
	}

	static bool PrepareForDebuggingBuildTargetIsChecked(FString TargetName)
	{
		return GetDefault<UPlatformsMenuSettings>()->PrepareForDebuggingBuildTarget == TargetName;
	}

	static bool CanPrepareForDebugging()
	{
		return CanCookOrPackage(GetDefault<UPlatformsMenuSettings>()->PrepareForDebuggingPlatform, EPrepareContentMode::PrepareForDebugging);
	}

	static void PrepareForDebugging()
	{
		const UPlatformsMenuSettings* PlatformsSettings = GetDefault<UPlatformsMenuSettings>();
		CookOrPackage(PlatformsSettings->PrepareForDebuggingPlatform, PlatformsSettings->PrepareForDebuggingBuildTarget, EPrepareContentMode::PrepareForDebugging);
	}
...
// Returns the IniPlatformName of all platforms categorized into "supported", "unsupported", and "uncompiled" matching
// the different behaviors used by the Platforms menu. "Uncompiled" indicates there is no compiled-in support for that
// platform. "Unsupported" indicates the platform was disabled via the project settings.
static void GetTurnkeyCategorizedPlatforms(TArray<FName>* OutSupportedPlatforms, TArray<FName>* OutUnsupportedPlatforms, TArray<FName>* OutUncompiledPlatforms)
{
	if (OutSupportedPlatforms != nullptr)
	{
		OutSupportedPlatforms->Empty();
	}

	if (OutUnsupportedPlatforms != nullptr)
	{
		OutUnsupportedPlatforms->Empty();
	}

	if (OutUncompiledPlatforms != nullptr)
	{
		OutUncompiledPlatforms->Empty();
	}
	...
	for (const auto& Pair : FDataDrivenPlatformInfoRegistry::GetAllPlatformInfos())
	{
		...
		FName IniPlatformName = Pair.Key;
		if (FDataDrivenPlatformInfoRegistry::IsPlatformHiddenFromUI(IniPlatformName))
		{
			continue;
		}

		if (!FDataDrivenPlatformInfoRegistry::HasCompiledSupportForPlatform(
				IniPlatformName, FDataDrivenPlatformInfoRegistry::EPlatformNameType::Ini
			))
		{
			if (OutUncompiledPlatforms != nullptr)
			{
				OutUncompiledPlatforms->Add(IniPlatformName);
			}

			continue;
		}

		if (bProjectStatusIsValid && !ProjectStatus.IsTargetPlatformSupported(IniPlatformName))
		{
			if (OutUnsupportedPlatforms != nullptr)
			{
				OutUnsupportedPlatforms->Add(IniPlatformName);
			}
		...
		if (OutSupportedPlatforms != nullptr)
		{
			OutSupportedPlatforms->Add(IniPlatformName);
		}
	}
}

// Shows a sub-menu for the platform allowing selection of the flavor or architecture where appropriate
static void MakeTurnkeyPerPlatformMenu(UToolMenu* Menu, FName IniPlatformName, TArray<const PlatformInfo::FTargetPlatformInfo*> ValidFlavors, TArray<FString> Architectures)
{
	if (ValidFlavors.Num() > 1)
	{
		FToolMenuSection& FlavorSection = Menu->AddSection("FlavorSelection", LOCTEXT("TurnkeySection_FlavorSelection", "Flavor Selection"));
	...
	{
		FToolMenuSection& ArchitectureSection = Menu->AddSection("ArchitectureSelection", LOCTEXT("TurnkeySection_ArchitectureSelection", "Architecture Selection"));
		...
		ArchitectureSection.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetActiveArchitecture, IniPlatformName, FString()),
				FCanExecuteAction(),
				FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::SetActiveArchitectureIsChecked, IniPlatformName, FString())
			),
			EUserInterfaceActionType::RadioButton
		);
		...
		{
			bool bIsHostArchitecture = (IniPlatformName == FPlatformProperties::IniPlatformName()) && (Architecture == TargetPlatform->GetHostArchitecture());
			...
			ArchitectureSection.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetActiveArchitecture, IniPlatformName, Architecture),
					FCanExecuteAction(),
					FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::SetActiveArchitectureIsChecked, IniPlatformName, Architecture)
				),
				EUserInterfaceActionType::RadioButton
			);
		}
...
static void MakeCookContentMenuItems(UToolMenu* Menu)
{
	FToolMenuSection& TopSection = Menu->AddSection("CookSection");
	{
		TopSection.AddMenuEntry(
			NAME_None,
			LOCTEXT("Turnkey_CookContent", "Cook Content"),
			LOCTEXT("TurnkeyTooltip_CookContent", "Cook this project for the selected Platform and target"),
			FSlateIcon(FAppStyle::GetAppStyleSetName(), "MainFrame.CookContent"),
			FUIAction(
				FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::CookContent),
				FCanExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::CanCookContent)
			)
		);
	}

	FToolMenuSection& PlatformsSection = Menu->AddSection("CookPlatformsSection", LOCTEXT("TurnkeySection_CookPlatforms", "Cook For"));
	{
		PlatformsSection.AddDynamicEntry(NAME_None, FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& PlatformsSection)
		{
			...
			TArray<FName> SupportedPlatforms;
			GetTurnkeyCategorizedPlatforms(&SupportedPlatforms, nullptr, nullptr);

			// Always ensure a platform is selected to avoid confusion when the button is disabled. This occurs when
			// there is no saved setting such as on first launch, or if the saved platform is no longer supported due to
			// an SDK being uninstalled.
			FName SelectedPlatform = FTurnkeySupportCallbacks::GetCookPlatform();
			bool bUseNextPlatformAsDefault = !SupportedPlatforms.Contains(SelectedPlatform);

			// Use the host platform as the default selection as long as it's in the list and supported.
			if (bUseNextPlatformAsDefault && SupportedPlatforms.Contains(GHostIniPlatformName))
			{
				FTurnkeySupportCallbacks::SetCookPlatform(GHostIniPlatformName);
				bUseNextPlatformAsDefault = !FTurnkeySupportCallbacks::CanCookContent();
			}

			for (const FName& IniPlatformName : SupportedPlatforms)
			{
				...
				TArray<const PlatformInfo::FTargetPlatformInfo*> ValidFlavors;

				// Architecture selection exists for platforms targeting multiple CPU architectures, such as Windows on
				// x64 and ARM64.
				TArray<FString> Architectures;

				if (VanillaInfo != nullptr)
				{
					ValidFlavors = VanillaInfo->Flavors.FilterByPredicate([](const PlatformInfo::FTargetPlatformInfo* Target)
						{
				...
				// Platforms that have multiple flavors or architectures are displayed as a radio button that also pops
				// out a sub-menu to choose the flavor and/or architecture.
				if (ValidFlavors.Num() > 1 || Architectures.Num() > 1)
				{
					FToolMenuEntry Entry = FToolMenuEntry::InitSubMenu(
						NAME_None,
						MakeSdkStatusAttribute(IniPlatformName, nullptr),
						FText::FromString(IniPlatformName.ToString()),
						FNewToolMenuDelegate::CreateStatic(&MakeTurnkeyPerPlatformMenu, IniPlatformName, ValidFlavors, Architectures),
						FUIAction(
							FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetCookPlatform, IniPlatformName),
							FCanExecuteAction(),
							FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::CookPlatformIsChecked, IniPlatformName)
						),
						EUserInterfaceActionType::RadioButton,
						false,
						MakePlatformSdkIconAttribute(IniPlatformName, nullptr)
					);

					PlatformsSection.AddEntry(Entry);
				}
				else
				{
				...
				// If the host platform is not supported, choose the first supported platform. If there are none, the
				// last item in the list ends up selected.
				if (bUseNextPlatformAsDefault)
				{
					FTurnkeySupportCallbacks::SetCookPlatform(IniPlatformName);
					bUseNextPlatformAsDefault = !FTurnkeySupportCallbacks::CanCookContent();
				}
			}
		}));
	...
	if (Targets.Num() > 0)
	{
		...
		if (ValidTargets.Num() > 1)
		{
			...
			FToolMenuSection& TargetSection = Menu->AddSection("BuildTarget", LOCTEXT("TurnkeySection_BuildTarget", "Build Target"));

			// Using the project default setting is represented by an empty string
			TargetSection.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetCookBuildTarget, FString()),
					FCanExecuteAction(),
					FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::CookBuildTargetIsChecked, FString())
					),
				EUserInterfaceActionType::RadioButton
			);
			...
			for (const FTargetInfo& Target : ValidTargets)
			{
				TargetSection.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
						FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetCookBuildTarget, Target.Name),
						FCanExecuteAction(),
						FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::CookBuildTargetIsChecked, Target.Name)
					),
					EUserInterfaceActionType::RadioButton
				);
			}
...
static void MakePackageProjectMenuItems(UToolMenu* Menu)
{
	FToolMenuSection& TopSection = Menu->AddSection("PackageSection");
	{
		TopSection.AddMenuEntry(
			"PackageProject",
			...
			FUIAction(
				FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::PackageProject),
				FCanExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::CanPackageProject)
			)
		);
	}

	FToolMenuSection& PlatformsSection = Menu->AddSection("PackagePlatformsSection", LOCTEXT("PackagePlatformsSection", "Package For"));
	{
		PlatformsSection.AddDynamicEntry(NAME_None, FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& PlatformsSection)
		{
			ITargetDeviceServicesModule* TargetDeviceServicesModule = static_cast<ITargetDeviceServicesModule*>(FModuleManager::Get().LoadModule(TEXT("TargetDeviceServices")));
			...
			// there is no saved setting such as on first launch, or if the saved platform is no longer supported due to
			// an SDK being uninstalled.
			FName SelectedPlatform = FTurnkeySupportCallbacks::GetPackagePlatform();
			bool bUseNextPlatformAsDefault = !SupportedPlatforms.Contains(SelectedPlatform);

			// Use the host platform as the default selection as long as it's in the list and supported.
			if (bUseNextPlatformAsDefault && SupportedPlatforms.Contains(GHostIniPlatformName))
			{
				FTurnkeySupportCallbacks::SetPackagePlatform(GHostIniPlatformName);
				bUseNextPlatformAsDefault = !FTurnkeySupportCallbacks::CanPackageProject();
			}
			...
			for (const FName& IniPlatformName : SupportedPlatforms)
			{
				...
				if (ValidFlavors.Num() > 1 || Architectures.Num() > 1)
				{
					FToolMenuEntry Entry = FToolMenuEntry::InitSubMenu(
						...
						FNewToolMenuDelegate::CreateStatic(&MakeTurnkeyPerPlatformMenu, IniPlatformName, ValidFlavors, Architectures),
						FUIAction(
							FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetPackagePlatform, IniPlatformName),
							FCanExecuteAction(),
							FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::PackagePlatformIsChecked, IniPlatformName)
						),
						EUserInterfaceActionType::RadioButton,
						false,
						MakePlatformSdkIconAttribute(IniPlatformName, nullptr)
				...
				if (bUseNextPlatformAsDefault)
				{
					FTurnkeySupportCallbacks::SetPackagePlatform(IniPlatformName);
					bUseNextPlatformAsDefault = !FTurnkeySupportCallbacks::CanPackageProject();
				}
			}
		}));
	...
	FToolMenuSection& ConfigSection = Menu->AddSection("BuildConfig", LOCTEXT("TurnkeySection_BuildConfig", "Build Configuration"));
	{
		const UProjectPackagingSettings::FConfigurationInfo& DefaultConfigInfo = UProjectPackagingSettings::ConfigurationInfo[static_cast<int32>(AllPlatformPackagingSettings->BuildConfiguration)];
		ConfigSection.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetPackageBuildConfiguration, EProjectPackagingBuildConfigurations::PPBC_MAX),
				FCanExecuteAction(),
				FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::PackageBuildConfigurationIsChecked, EProjectPackagingBuildConfigurations::PPBC_MAX)
			),
			EUserInterfaceActionType::RadioButton
		);
		...
		for (EProjectPackagingBuildConfigurations PackagingConfiguration : PackagingConfigurations)
		{
			...
			if (FInstalledPlatformInfo::Get().IsValid(TOptional<EBuildTargetType>(), TOptional<FString>(), ConfigurationInfo.Configuration, ProjectType, EInstalledPlatformState::Downloaded))
			{
				ConfigSection.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
						FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetPackageBuildConfiguration, PackagingConfiguration),
						FCanExecuteAction(),
						FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::PackageBuildConfigurationIsChecked, PackagingConfiguration)
					),
					EUserInterfaceActionType::RadioButton
				);
			}
	...
	if (Targets.Num() > 0)
	{
		...
		if (ValidTargets.Num() > 1)
		{
			...
			TargetSection.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetPackageBuildTarget, FString()),
					FCanExecuteAction(),
					FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::PackageBuildTargetIsChecked, FString())
					),
				EUserInterfaceActionType::RadioButton
			);
			...
			for (const FTargetInfo& Target : ValidTargets)
			{
				TargetSection.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
						FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetPackageBuildTarget, Target.Name),
						FCanExecuteAction(),
						FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::PackageBuildTargetIsChecked, Target.Name)
					),
					EUserInterfaceActionType::RadioButton
				);
			}
...
static void MakePrepareForDebuggingMenuItems(UToolMenu* Menu)
{
	FToolMenuSection& TopSection = Menu->AddSection("PrepareSection");
	{
		TopSection.AddMenuEntry(
			"PrepareForDebugging",
			LOCTEXT("Turnkey_PrepareForDebugging", "Prepare For Debugging"),
			LOCTEXT("Turnkey_PrepareForDebugging_Tooltip", "Create files to help streaming the debugging pipeline on some platforms"),
			FSlateIcon(FAppStyle::GetAppStyleSetName(), "Debug"),
			FUIAction(
				FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::PrepareForDebugging),
				FCanExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::CanPrepareForDebugging)
			)
		);
	}

	FToolMenuSection& PlatformsSection = Menu->AddSection("PlatformsSection", LOCTEXT("TurnkeySection_PrepareForDebuggingPlatforms", "Prepare For"));
	{
		PlatformsSection.AddDynamicEntry(NAME_None, FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& PlatformsSection)
		{
			TArray<FName> SupportedPlatforms;
			...
			// there is no saved setting such as on first launch, or if the saved platform is no longer supported due to
			// an SDK being uninstalled.
			FName SelectedPlatform = FTurnkeySupportCallbacks::GetPrepareForDebuggingPlatform();
			bool bUseNextPlatformAsDefault = !SupportedPlatforms.Contains(SelectedPlatform) || (SelectedPlatform != TEXT("IOS") && SelectedPlatform != TEXT("TvOS"));
			...
			for (const FName& IniPlatformName : SupportedPlatforms)
			{
				...
				// Create a radio button per-platform
				FToolMenuEntry Entry = FToolMenuEntry::InitMenuEntry(
					NAME_None,
					MakeSdkStatusAttribute(IniPlatformName, nullptr),
					FText::Format(LOCTEXT("TurnkeyTooltip_PrepareForDebuggingPlatform", "Expects a working remote toolchain and an IPA package with the same name as the project file in the Binaries/{0}/ folder."), FText::FromString(IniPlatformName.ToString().ToUpper())),
					MakePlatformSdkIconAttribute(IniPlatformName, nullptr),
					FUIAction(
						FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetPrepareForDebuggingPlatform, IniPlatformName),
						FCanExecuteAction(),
						FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::PrepareForDebuggingPlatformIsChecked, IniPlatformName)
					),
					EUserInterfaceActionType::RadioButton
				);
				...
				// Choose the first supported platform. If there are none, the last item in the list ends up selected.
				if (bUseNextPlatformAsDefault)
				{
					FTurnkeySupportCallbacks::SetPrepareForDebuggingPlatform(IniPlatformName);
					bUseNextPlatformAsDefault = !FTurnkeySupportCallbacks::CanPrepareForDebugging();
				}
			}
		}));
	}
	...
	if (Targets.Num() > 0)
	{
		...
		if (ValidTargets.Num() > 1)
		{
			...
			TargetSection.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetPrepareForDebuggingBuildTarget, FString()),
					FCanExecuteAction(),
					FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::PrepareForDebuggingBuildTargetIsChecked, FString())
					),
				EUserInterfaceActionType::RadioButton
			);
			...
			for (const FTargetInfo& Target : ValidTargets)
			{
				TargetSection.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
						FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetPrepareForDebuggingBuildTarget, Target.Name),
						FCanExecuteAction(),
						FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::PrepareForDebuggingBuildTargetIsChecked, Target.Name)
					),
					EUserInterfaceActionType::RadioButton
				);
			}
...
static void MakePerCustomBuildMenu(UToolMenu* ToolMenu, ECustomBuildType Type, FProjectBuildSettings CustomBuild, FText ToolTip)
{
	const bool bHasDeviceEntry = CustomBuild.BuildCookRunParams.Contains(TEXT("{DeviceId}"));

	FToolMenuSection& CommandSection = ToolMenu->AddSection("CommandSection");
	{
		CommandSection.AddMenuEntry(
			"ExecuteCustomBuild",
			FText::FromString(CustomBuild.Name),
			ToolTip,
			FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Advanced"),
			FUIAction(
				FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::ExecuteCustomBuild, Type, CustomBuild),
				FCanExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::CanExecuteCustomBuild, Type, CustomBuild.Name, bHasDeviceEntry)
			)
		);
	}

	// Custom builds either show a list of platforms or a list of devices
	if (bHasDeviceEntry)
	{
		FToolMenuSection& DevicesSection = ToolMenu->AddSection("DevicesSection", LOCTEXT("TurnkeySection_Device", "Device"));
		...
		// Always ensure a device is selected to avoid confusion when the button is disabled. This occurs when
		// there is no saved setting such as on first launch, or if the saved platform is no longer supported due to
		// an SDK being uninstalled.
		bool bUseNextDeviceAsDefault = !FTurnkeySupportCallbacks::CanExecuteCustomBuild(Type, CustomBuild.Name, bHasDeviceEntry);

		for (const FName& IniPlatformName : SupportedPlatforms)
		{
			if (CustomBuild.SpecificPlatforms.IsEmpty() || CustomBuild.SpecificPlatforms.Contains(IniPlatformName))
			{
				...
				for (const TSharedPtr<ITargetDeviceProxy>& Proxy : DeviceProxies)
				{
					...
					DevicesSection.AddMenuEntry(
						NAME_None,
						MakeSdkStatusAttribute(IniPlatformName, Proxy),
						FText::Format(LOCTEXT("TurnkeyTooltip_CustomBuildDevice", "{0} ({1})"), FText::FromString(DeviceName), FText::FromName(IniPlatformName)),
						MakePlatformSdkIconAttribute(IniPlatformName, Proxy),
						FUIAction(
							FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetDeviceIdForCustomBuild, Type, CustomBuild.Name, IniPlatformName, DeviceId),
							FCanExecuteAction(),
							FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::DeviceIdForCustomBuildIsChecked, Type, CustomBuild.Name, DeviceId)
						),
						EUserInterfaceActionType::RadioButton
					);

					// Choose the first valid device as a default.
					if (bUseNextDeviceAsDefault)
					{
						FTurnkeySupportCallbacks::SetDeviceIdForCustomBuild(Type, CustomBuild.Name, IniPlatformName, DeviceId);
						bUseNextDeviceAsDefault = !FTurnkeySupportCallbacks::CanExecuteCustomBuild(Type, CustomBuild.Name, bHasDeviceEntry);
					}
				}
			}
		}
	...
	{
		FToolMenuSection& PlatformsSection = ToolMenu->AddSection("PlatformsSection", LOCTEXT("TurnkeySection_Platform", "Platform"));
		{
			TArray<FName> SupportedPlatforms;
			GetTurnkeyCategorizedPlatforms(&SupportedPlatforms, nullptr, nullptr);
			...
			// there is no saved setting such as on first launch, or if the saved platform is no longer supported due to
			// an SDK being uninstalled.
			FName SelectedPlatform = FTurnkeySupportCallbacks::GetPlatformForCustomBuild(Type, CustomBuild.Name);
			bool bUseNextPlatformAsDefault = !SupportedPlatforms.Contains(SelectedPlatform);

			// Use the host platform as the default selection as long as it's in the list.
			if (bUseNextPlatformAsDefault && SupportedPlatforms.Contains(GHostIniPlatformName) &&
				(CustomBuild.SpecificPlatforms.IsEmpty() || CustomBuild.SpecificPlatforms.Contains(GHostIniPlatformName)))
			{
				FTurnkeySupportCallbacks::SetPlatformForCustomBuild(Type, CustomBuild.Name, GHostIniPlatformName);
				bUseNextPlatformAsDefault = !FTurnkeySupportCallbacks::CanExecuteCustomBuild(Type, CustomBuild.Name, bHasDeviceEntry);
			}
				...
				if (CustomBuild.SpecificPlatforms.IsEmpty() || CustomBuild.SpecificPlatforms.Contains(IniPlatformName))
				{
					...
					if (ValidFlavors.Num() > 1 || Architectures.Num() > 1)
					{
						FToolMenuEntry Entry = FToolMenuEntry::InitSubMenu(
							...
							FNewToolMenuDelegate::CreateStatic(&MakeTurnkeyPerPlatformMenu, IniPlatformName, ValidFlavors, Architectures),
							FUIAction(
								FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetPlatformForCustomBuild, Type, CustomBuild.Name, IniPlatformName),
								FCanExecuteAction(),
								FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::PlatformForCustomBuildIsChecked, Type, CustomBuild.Name, IniPlatformName)
							),
							EUserInterfaceActionType::RadioButton,
							false,
							MakePlatformSdkIconAttribute(IniPlatformName, nullptr)
					...
					// If the host platform is not supported, choose the first platform.
					if (bUseNextPlatformAsDefault)
					{
						FTurnkeySupportCallbacks::SetPlatformForCustomBuild(Type, CustomBuild.Name, IniPlatformName);
						bUseNextPlatformAsDefault = !FTurnkeySupportCallbacks::CanExecuteCustomBuild(Type, CustomBuild.Name, bHasDeviceEntry);
					}
				}
			}
	...
	FToolMenuSection& ConfigSection = ToolMenu->AddSection("BuildConfig", LOCTEXT("TurnkeySection_BuildConfig", "Build Configuration"));
	{
		const UProjectPackagingSettings::FConfigurationInfo& DefaultConfigInfo = UProjectPackagingSettings::ConfigurationInfo[static_cast<int32>(AllPlatformPackagingSettings->BuildConfiguration)];
		ConfigSection.AddMenuEntry(
			NAME_None,
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetBuildConfigurationForCustomBuild, Type, CustomBuild.Name, EProjectPackagingBuildConfigurations::PPBC_MAX),
				FCanExecuteAction(),
				FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::BuildConfigurationForCustomBuildIsChecked, Type, CustomBuild.Name, EProjectPackagingBuildConfigurations::PPBC_MAX)
			),
			EUserInterfaceActionType::RadioButton
		);
		...
		for (EProjectPackagingBuildConfigurations PackagingConfiguration : PackagingConfigurations)
		{
			...
			if (FInstalledPlatformInfo::Get().IsValid(TOptional<EBuildTargetType>(), TOptional<FString>(), ConfigurationInfo.Configuration, ProjectType, EInstalledPlatformState::Downloaded))
			{
				ConfigSection.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
						FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetBuildConfigurationForCustomBuild, Type, CustomBuild.Name, PackagingConfiguration),
						FCanExecuteAction(),
						FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::BuildConfigurationForCustomBuildIsChecked, Type, CustomBuild.Name, PackagingConfiguration)
					),
					EUserInterfaceActionType::RadioButton
				);
			}
	...
	if (Targets.Num() > 0)
	{
		...
		if (ValidTargets.Num() > 1)
		{
			...
			TargetSection.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetBuildTargetForCustomBuild, Type, CustomBuild.Name, FString()),
					FCanExecuteAction(),
					FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::BuildTargetForCustomBuildIsChecked, Type, CustomBuild.Name, FString())
					),
				EUserInterfaceActionType::RadioButton
			);
			...
			for (const FTargetInfo& Target : ValidTargets)
			{
				TargetSection.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
						FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetBuildTargetForCustomBuild, Type, CustomBuild.Name, Target.Name),
						FCanExecuteAction(),
						FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::BuildTargetForCustomBuildIsChecked, Type, CustomBuild.Name, Target.Name)
					),
					EUserInterfaceActionType::RadioButton
				);
			}
...
static void MakeCustomBuildsMenuItems(UToolMenu* Menu)
{
	UProjectPackagingSettings* AllPlatformPackagingSettings = GetMutableDefault<UProjectPackagingSettings>();

	// Items from EngineCustomBuilds and ProjectCustomBuilds are shown in separate sections. Since they are separate
	// arrays, it's possible they both contain an entry with the same name.
	FToolMenuSection& EngineSection = Menu->AddSection("EngineCustomBuilds", LOCTEXT("TurnkeySection_EngineCustomBuilds", "Engine Custom Builds"));

	for (FProjectBuildSettings& CustomBuild : AllPlatformPackagingSettings->EngineCustomBuilds)
	{
#if ALLOW_CONTROL_TO_COPY_COMMANDLINE
		...
		FFormatNamedArguments Args;
		Args.Add(TEXT("BuildName"), FText::FromString(CustomBuild.Name));
		Args.Add(TEXT("BuildHelp"), FText::FromString(CustomBuild.HelpText));

		FText ToolTip = FText::Format(ToolTipFormat, Args);

		EngineSection.AddSubMenu(
			NAME_None,
			FText::FromString(CustomBuild.Name),
			ToolTip,
			FNewToolMenuDelegate::CreateStatic(&MakePerCustomBuildMenu, ECustomBuildType::EngineCustomBuilds, CustomBuild, ToolTip),
			false,
			FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Advanced"),
			true
		);
	...
	FToolMenuSection& ProjectSection = Menu->AddSection("ProjectCustomBuilds", LOCTEXT("TurnkeySection_ProjectCustomBuilds", "Project Custom Builds"));

	for (FProjectBuildSettings& CustomBuild : AllPlatformPackagingSettings->ProjectCustomBuilds)
	{
#if ALLOW_CONTROL_TO_COPY_COMMANDLINE
		FText ToolTipFormat = LOCTEXT("TurnkeyTooltip_ProjectCustomBuild_WithCopy", "Execute this project's custom '{BuildName}' command.\n{BuildHelp}\n--\nHold Control to copy the final BuildCookRun commandline to the clipboard.\nThis custom command comes from Project Settings.");
#else
		FText ToolTipFormat = LOCTEXT("TurnkeyTooltip_ProjectCustomBuild", "Execute this project's custom '{BuildName}' command.\n{BuildHelp}\n--\nThis custom command comes from Project Settings.");
#endif
		...
		ProjectSection.AddSubMenu(
			...
			FText::FromString(CustomBuild.Name),
			ToolTip,
			FNewToolMenuDelegate::CreateStatic(&MakePerCustomBuildMenu, ECustomBuildType::ProjectCustomBuilds, CustomBuild, ToolTip),
			false,
			FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Advanced"),
			true
		);
...
static void MakeSdkManagementPerPlatformPlatformMenu(UToolMenu* ToolMenu, FName IniPlatformName, ITargetDeviceServicesModule* TargetDeviceServicesModule)
{
...
static void MakeSdkManagementMenuItems(UToolMenu* SdkManagementMenu)
{
	SdkManagementMenu->AddDynamicSection(NAME_None, FNewToolMenuDelegate::CreateLambda([](UToolMenu* Menu)
	{
		FToolMenuSection& SupportedPlatformsSection = Menu->AddSection("SupportedPlatforms", LOCTEXT("TurnkeySection_SupportedPlatforms", "Platforms"));

		ITargetDeviceServicesModule* TargetDeviceServicesModule = static_cast<ITargetDeviceServicesModule*>(FModuleManager::Get().LoadModule(TEXT("TargetDeviceServices")));

		TArray<FName> SupportedPlatforms;
		TArray<FName> UnsupportedPlatforms;
		TArray<FName> UncompiledPlatforms;
		GetTurnkeyCategorizedPlatforms(&SupportedPlatforms, &UnsupportedPlatforms, &UncompiledPlatforms);

		for (const FName& IniPlatformName : SupportedPlatforms)
		{
			SupportedPlatformsSection.AddSubMenu(
				NAME_None,
				MakeSdkStatusAttribute(IniPlatformName, nullptr),
				FText::FromString(IniPlatformName.ToString()),
				FNewToolMenuDelegate::CreateStatic(&MakeSdkManagementPerPlatformPlatformMenu, IniPlatformName, TargetDeviceServicesModule),
				false,
				MakePlatformSdkIconAttribute(IniPlatformName, nullptr),
				true
			);
		...
		{
			FToolMenuSection& UnsupportedPlatformsSection = Menu->AddSection("UnsupportedPlatforms", LOCTEXT("Turnkey_UnsupportedPlatforms", "Unsupported Platforms"));

			for (const FName& IniPlatformName : UnsupportedPlatforms)
			{
				UnsupportedPlatformsSection.AddSubMenu(
					NAME_None,
					MakeSdkStatusAttribute(IniPlatformName, nullptr),
					FText::FromString(IniPlatformName.ToString()),
					FNewToolMenuDelegate::CreateStatic(&MakeSdkManagementPerPlatformPlatformMenu, IniPlatformName, TargetDeviceServicesModule),
		...
		{
			FToolMenuSection& UncompiledPlatformsSection = Menu->AddSection("UncompiledPlatforms", LOCTEXT("Turnkey_UncompiledPlatforms", "Platforms With No Compiled Support"));

			for (const FName& IniPlatformName : UncompiledPlatforms)
			{
				UncompiledPlatformsSection.AddSubMenu(
					NAME_None,
					MakeSdkStatusAttribute(IniPlatformName, nullptr),
					FText::FromString(IniPlatformName.ToString()),
					FNewToolMenuDelegate::CreateStatic(&MakeSdkManagementPerPlatformPlatformMenu, IniPlatformName, TargetDeviceServicesModule),
		...
		FToolMenuSection& RefreshSection = Menu->AddSection("RefreshSection");

		RefreshSection.AddSeparator(NAME_None);
		
		RefreshSection.AddMenuEntry(
			NAME_None,
...
{
	FToolMenuSection& MenuSection = Menu->AddSection("QuickLaunchDevices", LOCTEXT("Devices", "Devices"));
	...
	FToolMenuSection& ConfigSection = Menu->AddSection("BuildConfig", LOCTEXT("TurnkeySection_BuildConfig", "Build Configuration"));
	{
		...
		ConfigSection.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetLaunchOnBuildConfiguration, EProjectPackagingBuildConfigurations::PPBC_MAX),
				FCanExecuteAction(),
				FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::LaunchOnBuildConfigurationIsChecked, EProjectPackagingBuildConfigurations::PPBC_MAX)
			),
			EUserInterfaceActionType::RadioButton
		);
		...
		for (EProjectPackagingBuildConfigurations PackagingConfiguration : PackagingConfigurations)
		{
			...
			if (FInstalledPlatformInfo::Get().IsValid(TOptional<EBuildTargetType>(), TOptional<FString>(), ConfigurationInfo.Configuration, ProjectType, EInstalledPlatformState::Downloaded))
			{
				ConfigSection.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
						FExecuteAction::CreateStatic(&FTurnkeySupportCallbacks::SetLaunchOnBuildConfiguration, PackagingConfiguration),
						FCanExecuteAction(),
						FIsActionChecked::CreateStatic(&FTurnkeySupportCallbacks::LaunchOnBuildConfigurationIsChecked, PackagingConfiguration)
					),
					EUserInterfaceActionType::RadioButton
				);
			}
	...
	{
		FToolMenuSection& CookOnTheFlySection = Menu->AddSection("CookOnTheFlySection");

		CookOnTheFlySection.AddSeparator(NAME_None);

		CookOnTheFlySection.AddMenuEntry(
			NAME_None,
			LOCTEXT("CookOnTheFlyOnLaunch", "Cook on the Fly"),
...
TSharedRef<SWidget> FTurnkeySupportModule::MakeTurnkeyMenuWidget() const
{
	...
	static const FName MenuName("LevelEditor.MainMenu.Platforms");

	if (!UToolMenus::Get()->IsMenuRegistered(MenuName))
	{
		UToolMenu* Menu = UToolMenus::Get()->RegisterMenu(MenuName);

		FToolMenuSection& LaunchSection = Menu->AddSection("Launch", LOCTEXT("TurnkeySection_Launch", "Launch"));
		{
			LaunchSection.AddSubMenu(
				"QuickLaunchSubMenu",
				LOCTEXT("QuickLaunch", "Quick Launch Current Level"),
				LOCTEXT("QuickLaunch_ToolTip", "Launch only the current level on the device for fast iteration"),
				FNewToolMenuDelegate::CreateLambda([this](UToolMenu* SubMenu)
					{
						FOnQuickLaunchSelected EmptyFunc;
						MakeQuickLaunchItems(SubMenu, EmptyFunc);
					}
				),
		...
		FToolMenuSection& ProjectLauncherSection = Menu->AddSection("ProjectLauncher", LOCTEXT("TurnkeySection_ProjectLauncher", "Project Launcher"));
		{
			if (IPluginManager::Get().FindEnabledPlugin(TEXT("ProjectLauncher")) != nullptr)
			{
				ProjectLauncherSection.AddMenuEntry(
					NAME_None,
			...
			ProjectLauncherSection.AddMenuEntry(
				NAME_None,
				LOCTEXT("OpenLegacyProjectLauncher", "Project Launcher (Legacy)..."),
		...
		FToolMenuSection& ContentManagementSection = Menu->AddSection("ContentManagementSection", LOCTEXT("TurnkeySection_ContentManagement", "Cook or Package"));
		{
			ContentManagementSection.AddSubMenu(
				"CookSubMenu",
				LOCTEXT("Turnkey_CookContent", "Cook Content"),
				LOCTEXT("TurnkeyTooltip_CookContentSubMenu", "Cook this project"),
				FNewToolMenuDelegate::CreateLambda([this](UToolMenu* SubMenu)
					{
						MakeCookContentMenuItems(SubMenu);
					}
				),
				false,
				FSlateIcon(FAppStyle::GetAppStyleSetName(), "MainFrame.CookContent")
			...
			ContentManagementSection.AddSubMenu(
				...
				FNewToolMenuDelegate::CreateLambda([this](UToolMenu* SubMenu)
					{
						MakePackageProjectMenuItems(SubMenu);
					}
				),
				false,
				FSlateIcon(FAppStyle::GetAppStyleSetName(), "MainFrame.PackageProject")
			...
			// Prepare For Debugging is only on iOS and tvOS platforms. The menu entry is not visible unless those
			// platforms are supported.
			{
				TArray<FName> SupportedPlatforms;
				GetTurnkeyCategorizedPlatforms(&SupportedPlatforms, nullptr, nullptr);

				if (SupportedPlatforms.Contains(TEXT("IOS")) || SupportedPlatforms.Contains(TEXT("TvOS")))
				{
					ContentManagementSection.AddSubMenu(
						"PrepareSubMenu",
						LOCTEXT("Turnkey_PrepareForDebugging", "Prepare For Debugging"),
						LOCTEXT("TurnkeyTooltip_PrepareForDebugging", "Create files to help streamline the debugging pipeline on some platforms"),
						FNewToolMenuDelegate::CreateLambda([this](UToolMenu* SubMenu)
							{
								MakePrepareForDebuggingMenuItems(SubMenu);
							}
						),
						false,
						FSlateIcon(FAppStyle::GetAppStyleSetName(), "Debug")
		...
		// Custom builds menu is only displayed when custom builds have been configured.
		FToolMenuSection& CustomBuildSection = Menu->AddSection("CustomBuildsSection", LOCTEXT("TurnkeySection_CustomBuilds", "Custom Builds"));
		{
			TArray<FName> SupportedPlatforms;
			GetTurnkeyCategorizedPlatforms(&SupportedPlatforms, nullptr, nullptr);

			bool bHasCustomBuilds = false;

			for (const FName& IniPlatformName : SupportedPlatforms)
			{
				UProjectPackagingSettings* PackagingSettings = FTurnkeySupportCallbacks::GetPackagingSettingsForPlatform(IniPlatformName);

				if (!PackagingSettings->EngineCustomBuilds.IsEmpty() || !PackagingSettings->ProjectCustomBuilds.IsEmpty())
				{
					bHasCustomBuilds = true;
					break;
				}
			}
			...
			if (bHasCustomBuilds)
			{
				CustomBuildSection.AddSubMenu(
					"CustomBuildsSubMenu",
					LOCTEXT("Turnkey_CustomBuilds", "Custom Builds"),
					LOCTEXT("TurnkeyTooltip_CustomBuilds", "Run custom builds commands defined in config files"),
					FNewToolMenuDelegate::CreateLambda([this](UToolMenu* SubMenu)
						{
							MakeCustomBuildsMenuItems(SubMenu);
						}
					),
					false,
					FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Advanced")
		...
		FToolMenuSection& OptionsSection = Menu->AddSection("Options", LOCTEXT("TurnkeySection_Options", "Options and Settings"));
		{
			OptionsSection.AddSubMenu("SDKManagementSubMenu",
				LOCTEXT("SDKManagement", "SDK Management"),
				LOCTEXT("SDKManagement_ToolTip", "Manage SDK Installations"),
				FNewToolMenuDelegate::CreateLambda([this](UToolMenu* SubMenu)
					{
						MakeSdkManagementMenuItems(SubMenu);	
					}
				),
				false,
				FSlateIcon(FAppStyle::Get().GetStyleSetName(), "MainFrame.Project")
			);
...
void FTurnkeySupportModule::MakeTurnkeyMenu(FToolMenuSection& MenuSection, const FToolMenuInsert& InsertPosition) const
{
	...
	FToolMenuEntry& PlatformsEntry = MenuSection.AddSubMenu(
		"Platforms",
	...
	PlatformsEntry.InsertPosition = InsertPosition;
}
