...
static FString GetSlateFromMovieSceneSequence(UMovieSceneSequence* MovieSceneSequence)
{
	...
	static UClass* TakeMetaDataClass = FindObject<UClass>(nullptr, TEXT("/Script/TakesCore.TakeMetaData"), EFindObjectFlags::None);
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS  // Use FAllSpawnableRestoreState
	...
	, SharedPlaybackState(InSharedPlaybackState)
{
	for (int32 SpawnableIndex = 0; SpawnableIndex < WeakMovieScene->GetSpawnableCount(); ++SpawnableIndex)
	{
		...
		FAllSpawnableRestoreState::SaveStateForSpawnable(bWasChanged, SpawnOwnershipMap, WeakMovieScene, Spawnable.GetGuid(), Spawnable.GetSpawnOwnership(), [&Spawnable](ESpawnOwnership SpawnOwnership) { Spawnable.SetSpawnOwnership(SpawnOwnership);});
	}
		...
		if (FMovieSceneBindingReferences* BindingReferences = Sequence->GetBindingReferences())
		{
			for (FMovieSceneBindingReference& BindingReference : BindingReferences->GetAllReferences())
			{
				if (BindingReference.CustomBinding)
				{
					...
					{
						FAllSpawnableRestoreState::SaveStateForSpawnable(bWasChanged, SpawnOwnershipMap, WeakMovieScene, BindingReference.ID, SpawnableBinding->SpawnOwnership, [SpawnableBinding](ESpawnOwnership SpawnOwnership) { SpawnableBinding->SpawnOwnership = SpawnOwnership; });
					}								
				}
			}
		}
...
FSpawnableRestoreState::~FSpawnableRestoreState()
{
	...
	const bool bOrigSquelchTransactionNotification = GEditor->bSquelchTransactionNotification;
	GEditor->bSquelchTransactionNotification = true;
	const bool bOrigIgnoreSelectionChange = GEditor->bIgnoreSelectionChange;
	GEditor->bIgnoreSelectionChange = true;
	const bool bOrigSuspendBroadcastPostUndoRedo = GEditor->bSuspendBroadcastPostUndoRedo;
...
void FAllSpawnableRestoreState::SaveStateForSpawnable(bool& bInOutWasChanged, TMap<FGuid, ESpawnOwnership>& InOutSpawnOwnershipMap, TWeakObjectPtr<UMovieScene>& WeakMovieScene, FGuid BindingGuid, ESpawnOwnership PreviousSpawnOwnership, TFunction<void(ESpawnOwnership)> SpawnOwnershipSetter)
{
	if (WeakMovieScene.IsValid() == false)
	{
		return;
	}
	// Spawnable could be in a subscene, so temporarily override it to persist throughout
	InOutSpawnOwnershipMap.Add(BindingGuid, PreviousSpawnOwnership);
...
FAllSpawnableRestoreState::FAllSpawnableRestoreState(TSharedPtr<UE::MovieScene::FSharedPlaybackState> InSharedPlaybackState, UMovieScene* InMovieScene)
	: SharedPlaybackState(InSharedPlaybackState)
{
	...
	if (!SharedPlaybackStateToUse)
	{
		UE_LOG(LogMovieScene, Warning, TEXT("FAllSpawnableRestoreState requires a valid FSharedPlaybackState"));
		return;
	}
	if (InMovieScene)
	{
		FPerMovieScene PerMovieScene;
		PerMovieScene.WeakMovieScene = InMovieScene;
		PerMovieScene.SequenceID = MovieSceneSequenceID::Root;
		OwnershipMaps.Add(InMovieScene, PerMovieScene);
	}
	else
	{
		const FMovieSceneSequenceHierarchy* Hierarchy = SharedPlaybackStateToUse->GetHierarchy();
		if (!Hierarchy)
		{
			if (UMovieSceneSequence* Sequence = SharedPlaybackStateToUse->GetSequence(MovieSceneSequenceID::Root))
			{
				if (UMovieScene* MovieScene = Sequence->GetMovieScene())
				{
					FPerMovieScene PerMovieScene;
					PerMovieScene.WeakMovieScene = MovieScene;
					PerMovieScene.SequenceID = MovieSceneSequenceID::Root;
					OwnershipMaps.Add(MovieScene, PerMovieScene);
				}
			}
			else
			{
				UE_LOG(LogMovieScene, Warning, TEXT("FAllSpawnableRestoreState requires a valid Root Sequence from FSharedPlaybackState"));
				return;
			}
		}
		else
		{
			...
			TArray<TWeakObjectPtr<UMovieSceneSequence>> Sequences;
			for (const FMovieSceneSequenceID& SequenceID : SequenceIDs)
			{
				if (UMovieSceneSequence* Sequence = SharedPlaybackStateToUse->GetSequence(SequenceID))
				{
					if (UMovieScene* MovieScene = Sequence->GetMovieScene())
					{
						FPerMovieScene PerMovieScene;
						PerMovieScene.WeakMovieScene = MovieScene;
						PerMovieScene.SequenceID = SequenceID;
						OwnershipMaps.Add(MovieScene, PerMovieScene);
					}
				}
			}
	...
	for (TPair <TWeakObjectPtr<UMovieScene>, FPerMovieScene>& OwnershipMap : OwnershipMaps)
	{
		if (OwnershipMap.Key.IsValid() == false)
		{
			continue;
		}
		for (int32 SpawnableIndex = 0; SpawnableIndex < OwnershipMap.Key->GetSpawnableCount(); ++SpawnableIndex)
		{
			FMovieSceneSpawnable& Spawnable = OwnershipMap.Key->GetSpawnable(SpawnableIndex);

			FAllSpawnableRestoreState::SaveStateForSpawnable(bWasChanged, OwnershipMap.Value.SpawnOwnershipMap, OwnershipMap.Key, Spawnable.GetGuid(), Spawnable.GetSpawnOwnership(), [&Spawnable](ESpawnOwnership SpawnOwnership) { Spawnable.SetSpawnOwnership(SpawnOwnership); });
		}

		ensure(SharedPlaybackStateToUse.IsValid());
		if (UMovieSceneSequence* Sequence = OwnershipMap.Key->GetTypedOuter<UMovieSceneSequence>())
		{
			if (FMovieSceneBindingReferences* BindingReferences = Sequence->GetBindingReferences())
			{
				for (FMovieSceneBindingReference& BindingReference : BindingReferences->GetAllReferences())
				{
					if (BindingReference.CustomBinding)
					{
						if (UMovieSceneSpawnableBindingBase* SpawnableBinding = BindingReference.CustomBinding->AsSpawnable(SharedPlaybackStateToUse.ToSharedRef()))
						{
							FAllSpawnableRestoreState::SaveStateForSpawnable(bWasChanged, OwnershipMap.Value.SpawnOwnershipMap, OwnershipMap.Key, BindingReference.ID, SpawnableBinding->SpawnOwnership, [SpawnableBinding](ESpawnOwnership SpawnOwnership) { SpawnableBinding->SpawnOwnership = SpawnOwnership; });
						}
					}
				}
			}
...
FAllSpawnableRestoreState::~FAllSpawnableRestoreState()
{
	if (bWasChanged)
	{
		GEditor->EndTransaction();
	...
	if (!bWasChanged || OwnershipMaps.IsEmpty())
	{
		return;
	}
	...
	for (TPair <TWeakObjectPtr<UMovieScene>, FPerMovieScene>& OwnershipMap : OwnershipMaps)
	{
		...
		{
			FMovieSceneSpawnable& Spawnable = OwnershipMap.Key->GetSpawnable(SpawnableIndex);
			if (ESpawnOwnership* SpawnOwnership = OwnershipMap.Value.SpawnOwnershipMap.Find(Spawnable.GetGuid()))
			{
				Spawnable.SetSpawnOwnership(*SpawnOwnership);
			}
		}
		...
		if (!SharedPlaybackStateToUse)
		{
			...
			UE::MovieScene::FSharedPlaybackStateCreateParams CreateParams;
			CreateParams.PlaybackContext = GEditor->GetEditorWorldContext().World();
			UMovieSceneSequence* ThisSequence = OwnershipMap.Key->GetTypedOuter<UMovieSceneSequence>();
			SharedPlaybackStateToUse = MakeShared<UE::MovieScene::FSharedPlaybackState>(*ThisSequence, CreateParams);
			...
			if (FMovieSceneBindingReferences* BindingReferences = Sequence->GetBindingReferences())
			{
				for (FMovieSceneBindingReference& BindingReference : BindingReferences->GetAllReferences())
				{
					if (BindingReference.CustomBinding)
					{
						if (UMovieSceneSpawnableBindingBase* SpawnableBinding = BindingReference.CustomBinding->AsSpawnable(SharedPlaybackStateToUse.ToSharedRef()))
						{
							if (ESpawnOwnership* SpawnOwnership = OwnershipMap.Value.SpawnOwnershipMap.Find(BindingReference.ID))
							{
								SpawnableBinding->SpawnOwnership = *SpawnOwnership;
							}
						}
	...
	const FBakingAnimationKeySettings& InSettings)
{
	if (SequencerPtr.IsValid() && Sections.Num() > 1 && OwnerTrack)
	{
			...
			if (BaseSection && Section && BaseSection->GetBlendType().IsValid() && Section->GetBlendType().IsValid())
			{
				...
				if (Section->IsActive())//active sections merge them
				{
					...
					if (BaseDoubleChannels.Num() > 0)
					{
						...
						MovieSceneToolHelpers::MergeTwoSections<FMovieSceneDoubleChannel>(BT, Range, TrackSections);
					...
					else if (BaseFloatChannels.Num() > 0)
					{
						...
						MovieSceneToolHelpers::FBaseTopSections BT(BaseSection,
							Section, StartIndex, EndIndex, StartIndex, EndIndex);
						MovieSceneToolHelpers::MergeTwoSections<FMovieSceneFloatChannel>(BT, Range, TrackSections);
					}
				}
		...
		if (InSettings.bReduceKeys && BaseSection)
		{
			...
			for (FMovieSceneDoubleChannel* DChannel : DoubleChannels)
			{
				DChannel->Optimize(Params); //should also auto tangent
			}
