...
public:
	...
	virtual ELandscapeToolTargetTypeFlags GetSupportedTargetTypes() override
	{
		return ELandscapeToolTargetTypeFlags::Visibility;
	}
};
...
public:
	...
	virtual void Apply(FEditorViewportClient* ViewportClient, FLandscapeBrush* Brush, const ULandscapeEditorObject* UISettings, const TArray<FLandscapeToolInteractorPosition>& InteractorPositions)
	{
		...
		if (LandscapeInfo)
		{
			...
			// expand the area by one vertex in each direction to capture the height of the corner vertices these components will attach to
			X1 -= 1;
			Y1 -= 1;
			X2 += 1;
			Y2 += 1;
			...
			TMap<FGuid, TArray<uint16>> PersistentEditLayerData;
			// Copy the persistent edit layer data using FLandscapeHeightCache on the region. That will extrapolate the data for the missing components : 
			for (const ULandscapeEditLayerBase* LandscapeEditLayer : Landscape->GetEditLayersConst())
			{
				...
				TArray<uint16>& Data = PersistentEditLayerData.FindOrAdd(LandscapeEditLayer->GetGuid());
				...
				//  those will have a heightmap created and initialized via AddDefaultLayerData but we won't copy any data into it :
				if (LandscapeEditLayer->SupportsEditingTools())
				{
					Data.AddZeroed((1 + X2 - X1) * (1 + Y2 - Y1));
					// Use FLandscapeEditDataInterface to cache / interpolate the existing edit layer's height data, that allows to use SetShouldDirtyPackage(false)
					FLandscapeEditDataInterface LandscapeEdit(LandscapeInfo, LandscapeEditLayer->GetGuid(), /*bInUploadTextureChangesToGPU = */false);
					// Ideally, we wouldn't even need to use FLandscapeEditDataInterface to simply cache the data, since that is intended to mutate the data 
					//  but it's the class that allows extrapolating the data for missing components... At least, we can force it not to dirty the package when doing so : 
					LandscapeEdit.SetShouldDirtyPackage(false);

					int32 TempX1 = X1; // GetHeightData overwrites its input min/max x/y
					int32 TempX2 = X2;
					int32 TempY1 = Y1;
					int32 TempY2 = Y2;
					LandscapeEdit.GetHeightData(TempX1, TempY1, TempX2, TempY2, Data.GetData(), /*Stride = */0);
				}
			}
				...
				for (int32 ComponentIndexX = ComponentIndexX1; ComponentIndexX <= ComponentIndexX2; ComponentIndexX++)
				{
					...
					if (!LandscapeComponent)
					{
						...
						if (ALandscapeProxy* LandscapeProxy = LandscapeSubsystem->FindOrAddLandscapeProxy(LandscapeInfo, ComponentBase))
						{
							...
							// Let the component be known to the ULandscapeInfo. This needs to be done before restoring the edit layers data via FLandscapeHeightCache 
							...
							// First create the component's final heightmap, it's needed for AddDefaultLayerData... 
							...
							LandscapeComponent->InitHeightmapData(HeightData, /*bUpdateCollision = */false);

							// For each layer that needs some persistent height data
							for (TPair<FGuid, TArray<uint16>> LayerDataPair : PersistentEditLayerData)
							{
								const FGuid& LayerGuid = LayerDataPair.Key;
								TMap<UTexture2D*, UTexture2D*> CreatedTextures;
								LandscapeComponent->AddDefaultLayerData(LayerGuid, { LandscapeComponent }, CreatedTextures);

								TArray<uint16>& CachedExtrapolatedData = LayerDataPair.Value;
								if (!CachedExtrapolatedData.IsEmpty())
								{
									// Finally, copy our (previously extrapolated) edit layer data onto this new component :
									FScopedSetLandscapeEditingLayer LayerScope(Landscape, LayerGuid);
									// We have cached the data for the entire X1, Y1, X2, Y2 region, which is larger than this new component's area so we need
									//  to extract the component's data into another array : 
									TArray<uint16> ComponentExtrapolatedCachedData;
									{
										const int32 SrcDataStride = X2 - X1 + 1;
										const int32 DstDataStride = ComponentVerts;
										ComponentExtrapolatedCachedData.AddZeroed(FMath::Square(ComponentVerts));
										uint16* DstDataPtr = ComponentExtrapolatedCachedData.GetData();
										for (int32 Y = ComponentBase.Y; Y < ComponentBase.Y + ComponentVerts; ++Y)
										{
											const FIntPoint SrcDataOffset(ComponentBase.X - X1, Y - Y1);
											check((SrcDataOffset.X >= 0) && (SrcDataOffset.Y >= 0));
											uint16* SrcDataPtr = CachedExtrapolatedData.GetData() + SrcDataOffset.X + SrcDataOffset.Y * SrcDataStride;
											check(SrcDataPtr + DstDataStride < SrcDataPtr + SrcDataStride);
											FMemory::Memcpy(DstDataPtr, SrcDataPtr, DstDataStride * sizeof(uint16));
											DstDataPtr += DstDataStride;
										}
									}

									FLandscapeEditDataInterface LandscapeEdit(LandscapeInfo, LayerGuid);
									LandscapeEdit.SetHeightData(ComponentBase.X, ComponentBase.Y, ComponentBase.X + ComponentVerts - 1, ComponentBase.Y + ComponentVerts - 1, ComponentExtrapolatedCachedData.GetData(), /*InStride = */0, /*InCalcNormals = */false);
								}
							}

							LandscapeComponent->UpdateMaterialInstances();
							// We need to update the bounds before registering the component, otherwise, the transform verification that are done on register will fail
							LandscapeComponent->UpdateCachedBounds();
							LandscapeComponent->UpdateBounds();
							LandscapeComponent->RegisterComponent();
							// The collisions will be generated after the edit layers merge readback has been done but we still to create the new collision component here
							//  so that it's part of the transaction and can be automatically deleted upon undo. UpdateCollisionData handles that :
							LandscapeComponent->UpdateCollisionData();
						}
					}
				}
			}

			// We need to invalidate the splines in case there's one that interacts with one of the newly added components : 
...
public:
	...
	void Apply(FEditorViewportClient* ViewportClient, FLandscapeBrush* Brush, const ULandscapeEditorObject* UISettings, const TArray<FLandscapeToolInteractorPosition>& InteractorPositions)
	{
		...
		ALandscapeGizmoActiveActor* Gizmo = EdMode->UISettings->CurrentGizmoActor.Get();
...
public:
	...
	virtual ELandscapeToolTargetTypeFlags GetSupportedTargetTypes() override
	{
		return UE::Landscape::GetLandscapeToolTargetTypeAsFlags(ToolTarget::TargetType);
	}
	...
	void Apply(FEditorViewportClient* ViewportClient, FLandscapeBrush* Brush, const ULandscapeEditorObject* UISettings, const TArray<FLandscapeToolInteractorPosition>& InteractorPositions)
	{
		...
		if (LandscapeInfo && Gizmo && Gizmo->GetRootComponent())
		{
			...
			for (ULandscapeLayerInfoObject* LayerInfo : Gizmo->LayerInfos)
			{
				...
				{
					LayerInfoIndex = LandscapeInfo->GetLayerInfoIndex(LayerInfo->GetLayerName());
					if (LayerInfoIndex != INDEX_NONE)
					{
...
public:
	...
	virtual void EnterTool() override
	{
		...
		ALandscapeGizmoActiveActor* Gizmo = this->EdMode->UISettings->CurrentGizmoActor.Get();
		if (Gizmo)
		{
	...
	virtual void ExitTool() override
	{
		if (ALandscapeGizmoActiveActor* Gizmo = this->EdMode->UISettings->CurrentGizmoActor.Get())
		{
...
void FEdModeLandscape::CopyDataToGizmo()
{
	...
	{
		GEditor->SelectNone(false, true);
		GEditor->SelectActor(UISettings->CurrentGizmoActor.Get(), true, true, true);
	}
}
...
public:
	...
	virtual FVector GetWidgetLocation() const override
	{
		...
		{
			if (ALandscapeGizmoActiveActor* GizmoActor = EdMode->UISettings->CurrentGizmoActor.Get())
			{
				return EdMode->UISettings->CurrentGizmoActor->GetActorLocation();
			}	
		}
		return MinLocation;
	}
