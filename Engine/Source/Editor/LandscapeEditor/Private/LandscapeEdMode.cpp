...
void FEdModeLandscape::UpdateToolModes()
{
	...
	FLandscapeToolMode* ToolMode_Manage = new(LandscapeToolModes)FLandscapeToolMode(TEXT("ToolMode_Manage"), ELandscapeToolTargetTypeFlags::None);
	...
	FLandscapeToolMode* ToolMode_Sculpt = new(LandscapeToolModes)FLandscapeToolMode(TEXT("ToolMode_Sculpt"), ELandscapeToolTargetTypeFlags::Heightmap | ELandscapeToolTargetTypeFlags::Visibility);
	...
	FLandscapeToolMode* ToolMode_Paint = new(LandscapeToolModes)FLandscapeToolMode(TEXT("ToolMode_Paint"), ELandscapeToolTargetTypeFlags::Weightmap);
...
void FEdModeLandscape::SetLandscapeInfo(ULandscapeInfo* InLandscapeInfo)
{
	...
	const bool bIsInitialLandscapeInfo = CurrentToolTarget.LandscapeInfo == nullptr;
	...
	{
		{
			...
			// Only update the selected edit layer index for the initial landscape, 
			// this avoids setting the last saved index to 0 when switching between existing landscapes
			if (bIsInitialLandscapeInfo)
			{
...
void FEdModeLandscape::Enter()
{
	...
	for (TActorIterator<ALandscapeGizmoActiveActor> It(GetWorld()); It; ++It)
	{
		...
		{
			UISettings->CurrentGizmoActor = *It;
			break;
		}
	}


	if (!UISettings->CurrentGizmoActor.IsValid())
	{
		FActorSpawnParameters SpawnParams;
		SpawnParams.ObjectFlags |= RF_Transient;
		UISettings->CurrentGizmoActor = GetWorld()->SpawnActor<ALandscapeGizmoActiveActor>(SpawnParams);
		UISettings->CurrentGizmoActor->ImportFromClipboard();
	}
	...
	TWeakObjectPtr<ALandscapeGizmoActiveActor> CurrentGizmoActor = UISettings->CurrentGizmoActor;
...
void FEdModeLandscape::Exit()
{
	...
	UISettings->CurrentGizmoActor = nullptr;
	...
	if (NewLandscapePreviewMode == ENewLandscapePreviewMode::None)
	{
		...
		if (CurrentToolTarget.LandscapeInfo.IsValid())
		{
			...
			else
			{
				...
				if (CurrentBrush != GizmoBrush && UISettings->CurrentGizmoActor.IsValid() && GizmoBrush && (GLandscapeEditRenderMode & ELandscapeEditRenderMode::Gizmo))
				{
...
bool FEdModeLandscape::MouseMove(FEditorViewportClient* InViewportClient, FViewport* InViewport, int32 MouseX, int32 MouseY)
{
	...
	if (ToolActiveViewport && ensure(CurrentTool) && !bIsPaintingInVR)
	{
		...
		const FLandscapeEditorCommands& LandscapeActions = FLandscapeEditorCommands::Get();
		const bool bIsDragChordBrushSizeAndFalloffPressed = InViewportClient->IsCommandChordPressed(LandscapeActions.DragBrushSizeAndFalloff);
		const bool bIsDragChordBrushSizeOnlyPressed = InViewportClient->IsCommandChordPressed(LandscapeActions.DragBrushSize);
		const bool bIsDragChordBrushFalloffOnlyPressed = InViewportClient->IsCommandChordPressed(LandscapeActions.DragBrushFalloff);
		const bool bIsDragChordStrengthPressed = InViewportClient->IsCommandChordPressed(LandscapeActions.DragBrushStrength);
		const bool bIsDragChordPressed = bIsDragChordBrushSizeAndFalloffPressed || bIsDragChordBrushSizeOnlyPressed || bIsDragChordBrushFalloffOnlyPressed || bIsDragChordStrengthPressed;
		...
		if (FMath::Abs(MouseXDelta) > 0 || FMath::Abs(MouseYDelta) > 0)
		{
			...
			if (bIsDragChordBrushSizeAndFalloffPressed)
			{
			...
			if (bIsDragChordBrushSizeOnlyPressed)
			{
				ChangeBrushSize(bIncrease);
				return true;
			}

			if (bIsDragChordBrushFalloffOnlyPressed)
			{
				ChangeBrushFalloff(bIncrease);
				return true;
			}

			if (bIsDragChordStrengthPressed)
			{
			...
			|| (LandscapeEditorControlType == ELandscapeFoliageEditorControlType::RequireCtrl && !IsCtrlDown(InViewport)))
		{
			...
			// Don't let go of the mouse capture while using one the drag shortcuts in order to maintain the cursor updated every frame : 
			if (!bIsDragChordPressed)
			{
...
bool FEdModeLandscape::StartTracking(FEditorViewportClient* InViewportClient, FViewport* InViewport)
{
	...
	if (UISettings->CurrentGizmoActor.IsValid() && UISettings->CurrentGizmoActor->IsSelected() && GLandscapeEditRenderMode & ELandscapeEditRenderMode::Gizmo)
	{
...
EEditAction::Type FEdModeLandscape::GetActionEditDelete()
{
	...
	if (NewLandscapePreviewMode == ENewLandscapePreviewMode::None)
	{
		...
		if (Result == EEditAction::Skip)
		{
			...
			if (UISettings->CurrentGizmoActor.IsValid())
			{
				if (UISettings->CurrentGizmoActor->IsSelected())
				{
					if (GEditor->GetSelectedActors()->Num() > 1)
					{
						GEditor->GetSelectedActors()->Deselect(UISettings->CurrentGizmoActor.Get());
...
EEditAction::Type FEdModeLandscape::GetActionEditCopy()
{
	...
	if (NewLandscapePreviewMode == ENewLandscapePreviewMode::None)
	{
		...
		if (Result == EEditAction::Skip)
		{
			...
			{
				if (UISettings->CurrentGizmoActor.IsValid() && GizmoBrush && UISettings->CurrentGizmoActor->TargetLandscapeInfo.IsValid())
				{
...
bool FEdModeLandscape::ProcessEditCopy()
{
	...
	if (NewLandscapePreviewMode == ENewLandscapePreviewMode::None)
	{
		...
		if (!Result)
		{
			...
			if (!CurrentlySelectedBPBrush)
			{
				bool IsSlowTask = IsSlowSelect(UISettings->CurrentGizmoActor->TargetLandscapeInfo.Get());
				if (IsSlowTask)
				{
				...
				UISettings->CurrentGizmoActor->Modify();
				UISettings->CurrentGizmoActor->FitToSelection();
				CopyDataToGizmo();
...
bool FEdModeLandscape::InputKey(FEditorViewportClient* ViewportClient, FViewport* Viewport, FKey Key, EInputEvent Event)
{
	...
	else
	{
		...
		if (Key == EKeys::LeftMouseButton && Event == IE_Pressed)
		{
			...
			{
				if (CurrentTool && (CurrentTool->GetSupportedTargetTypes() == ELandscapeToolTargetTypeFlags::None || CurrentToolTarget.TargetType != ELandscapeToolTargetType::Invalid))
				{
					FVector HitLocation;
					if (LandscapeMouseTrace(ViewportClient, HitLocation))
					{
						if (CurrentTool->AffectsEditLayers() && !CanEditSelectedTargetLayer(&ErrorReasonOnMouseUp))
						{
							return true;
						}
			...
			if (UISettings->CurrentGizmoActor.IsValid() && UISettings->CurrentGizmoActor->IsSelected() && (GLandscapeEditRenderMode & ELandscapeEditRenderMode::Gizmo) && IsAltDown(Viewport))
			{ 
...
void FEdModeLandscape::SetCurrentToolMode(FName ToolModeName, bool bRestoreCurrentTool /*= true*/)
{
	...
	{
		if (FLandscapeToolMode* NewToolMode = Algo::FindBy(LandscapeToolModes, ToolModeName, &FLandscapeToolMode::ToolModeName))
		{
			// Save current tool mode before changing
			LastActiveToolModeName = CurrentToolMode ? CurrentToolMode->ToolModeName : FName("ToolMode_Sculpt");

			CurrentToolMode = NewToolMode;

			if (bRestoreCurrentTool)
			{
				if (CurrentToolMode->CurrentToolName.IsNone())
				{
...
void FEdModeLandscape::SetCurrentTool(FName ToolName, FName TargetLayerName)
{
	...
	for (int32 i = 0; i < LandscapeTools.Num(); ++i)
	{
		...
		{
			if (EnumHasAnyFlags(Tool->GetSupportedTargetTypes(), CurrentToolMode->SupportedTargetTypes))
			{
				ToolIndex = i;
				break;
			}
...
{
	// Save current tool name before changing
	LastActiveToolName = CurrentTool ? CurrentTool->GetToolName() : FName("Sculpt");
	...
	{
		// if tool isn't valid for this mode then automatically switch modes. this mostly happens with shortcut keys
		FLandscapeToolMode* BackupToolMode = LandscapeToolModes.FindByPredicate([NewToolName = NewTool->GetToolName()](const FLandscapeToolMode& ToolMode)
			{
				return ToolMode.ValidTools.Contains(NewToolName);
			});

		// if no valid mode is found, keep the mode and fallback to the first valid tool
		if (BackupToolMode != nullptr)
		{
			SetCurrentToolMode(BackupToolMode->ToolModeName, false);
		}
		else
		{
			SetCurrentTool(CurrentToolMode->ValidTools[0]);
	...
	if (CurrentTool->GetSupportedTargetTypes() == ELandscapeToolTargetTypeFlags::None)
	{
	...
	{
		const ELandscapeToolTargetTypeFlags TargetTypeMask = CurrentTool->GetSupportedTargetTypes();
		checkSlow(TargetTypeMask != ELandscapeToolTargetTypeFlags::None);
		// GetLandscapeToolTargetTypeAsFlags does not support ELandscapeToolTargetType::Invalid so explicitly check this first 
		if (CurrentToolTarget.TargetType == ELandscapeToolTargetType::Invalid || !EnumHasAnyFlags(UE::Landscape::GetLandscapeToolTargetTypeAsFlags(CurrentToolTarget.TargetType), TargetTypeMask))
		{
			auto filter = [TargetTypeMask, TargetLayerName](const TSharedRef<FLandscapeTargetListInfo>& Target) { return (EnumHasAnyFlags(UE::Landscape::GetLandscapeToolTargetTypeAsFlags(Target->TargetType), TargetTypeMask)) && (TargetLayerName == NAME_None || TargetLayerName == Target->LayerName); };
	...
	if (UISettings->CurrentGizmoActor.IsValid() && CurrentToolTarget.LandscapeInfo.IsValid())
	{
		UISettings->CurrentGizmoActor->SetTargetLandscape(CurrentToolTarget.LandscapeInfo.Get());
	}
		...
		auto AddLandscape = [this, &CurrentIndex, &Index](ALandscape* Landscape, ULandscapeInfo* LandscapeInfo)
		{
			...
			if (LandscapeProxy)
			{
				if (CurrentToolTarget.LandscapeInfo == LandscapeInfo)
				{
					...
					if (UISettings->CurrentGizmoActor.IsValid())
					{
						UISettings->CurrentGizmoActor->SetTargetLandscape(LandscapeInfo);
					}
				}
...
bool FEdModeLandscape::CanEditCurrentTarget(FText* Reason) const
{
	...
	if (GetLandscape() == nullptr)
	{
		...
		LocalReason = NSLOCTEXT("UnrealEd", "LandscapeActorNotLoaded", "Landscape actor is not loaded. An edit layer landscape is required for editing.");
		return false;
	}
	
	// To edit a Landscape with layers all components need to be registered
	if(Proxy && !CurrentToolTarget.LandscapeInfo->AreAllComponentsRegistered())
	{
...
void FEdModeLandscape::UpdateTargetList(bool bRegenerateThumbnails)
{
	...
	if (CurrentToolTarget.LandscapeInfo.IsValid())
	{
		...
		if (Landscape != nullptr)
		{
			...
			UpdateTargetLayerDisplayOrder(UISettings->TargetDisplayOrder, UISettings->bTargetDisplayOrdersAscending);
		}
	}
...
void FEdModeLandscape::UpdateTargetLayerDisplayOrder(ELandscapeLayerDisplayMode InTargetDisplayOrder, bool bInIsAscendingOrder)
{
	if (!CurrentToolTarget.LandscapeInfo.IsValid())
	{
		return;
	...
	switch (InTargetDisplayOrder)
	{
		...
		case ELandscapeLayerDisplayMode::ByBlendMethod:
		{
			SavedTargetNameList.Empty();

			TArray<TSharedRef<FLandscapeTargetListInfo>> SortedTargetList;
			// Add only layers to be able to sort them by blend type
			for (int32 i = GetTargetLayerStartingIndex(); i < LandscapeTargetList.Num(); ++i)
			{
				SortedTargetList.Add(LandscapeTargetList[i]);
			}

			SortedTargetList.Sort([](const TSharedRef<FLandscapeTargetListInfo>& InLHS, const TSharedRef<FLandscapeTargetListInfo>& InRHS)
				{
					// Put invalid layers at the end : 
					if (InLHS->LayerInfoObj == nullptr)
					{
						return (InRHS->LayerInfoObj == nullptr) ? InLHS->LayerName.LexicalLess(InRHS->LayerName) : false;
					}
					else if (InRHS->LayerInfoObj == nullptr)
					{
						return true;
					}

					if (InLHS->LayerInfoObj->GetBlendMethod() == InRHS->LayerInfoObj->GetBlendMethod())
					{
						// For a given blend method, sort by blend group 
						if (InLHS->LayerInfoObj->GetBlendGroup() != InRHS->LayerInfoObj->GetBlendGroup())
						{
							// But put empty group first (most frequent case)
							if (InLHS->LayerInfoObj->GetBlendGroup().IsNone())
							{
								return true;
							}
							else if (InRHS->LayerInfoObj->GetBlendGroup().IsNone())
							{
								return false;
							}
							
							return InLHS->LayerInfoObj->GetBlendGroup().LexicalLess(InRHS->LayerInfoObj->GetBlendGroup());
						}

						return InLHS->LayerName.LexicalLess(InRHS->LayerName);
					}
					else
					{
						static constexpr int32 BlendMethodSortOrder[static_cast<int32>(ELandscapeTargetLayerBlendMethod::Count)] = 
						{
							2, /* ELandscapeTargetLayerBlendMethod::None */
							1, /* ELandscapeTargetLayerBlendMethod::FinalWeightBlending */
							0, /* ELandscapeTargetLayerBlendMethod::PremultipliedAlphaBlending */
						};

						return (BlendMethodSortOrder[static_cast<int32>(InLHS->LayerInfoObj->GetBlendMethod())] < BlendMethodSortOrder[static_cast<int32>(InRHS->LayerInfoObj->GetBlendMethod())]);
					}
				});
			Algo::Transform(SortedTargetList, SavedTargetNameList, [](const TSharedRef<FLandscapeTargetListInfo>& InTargetListInfo) { return InTargetListInfo->GetLayerName(); });
	...
	if (!bInIsAscendingOrder)
	{
		Algo::Reverse(SavedTargetNameList);
	}
...
bool FEdModeLandscape::UsesTransformWidget() const
{
	...
	return (UISettings->CurrentGizmoActor.IsValid() && UISettings->CurrentGizmoActor->IsSelected() && (GLandscapeEditRenderMode & ELandscapeEditRenderMode::Gizmo));
}
...
FVector FEdModeLandscape::GetWidgetLocation() const
{
	...
	if (UISettings->CurrentGizmoActor.IsValid() && (GLandscapeEditRenderMode & ELandscapeEditRenderMode::Gizmo) && UISettings->CurrentGizmoActor->IsSelected())
	{
		if (UISettings->CurrentGizmoActor->TargetLandscapeInfo.IsValid() && UISettings->CurrentGizmoActor->TargetLandscapeInfo->GetLandscapeProxy())
		{
			// Apply Landscape transformation when it is available
			ULandscapeInfo* LandscapeInfo = UISettings->CurrentGizmoActor->TargetLandscapeInfo.Get();
			return UISettings->CurrentGizmoActor->GetActorLocation()
				+ FQuatRotationMatrix(LandscapeInfo->GetLandscapeProxy()->GetActorQuat()).TransformPosition(FVector(0, 0, UISettings->CurrentGizmoActor->GetLength()));
		}
		return UISettings->CurrentGizmoActor->GetActorLocation();
	}
...
{
	if (UISettings->CurrentGizmoActor.IsValid() && UISettings->CurrentGizmoActor->IsSelected() && (GEditor->GetSelectedActors()->CountSelections<AActor>() != 1))
	{
		GEditor->SelectNone(false, true);
		GEditor->SelectActor(UISettings->CurrentGizmoActor.Get(), true, false, true);
	}
}
...
void FEdModeLandscape::ImportWeightData(ULandscapeInfo* LandscapeInfo, const FGuid& LayerGuid, ULandscapeLayerInfoObject* LayerInfo, const FString& Filename, const FIntRect& ImportRegionVerts, ELandscapeImportTransformType TransformType, FIntPoint Offset, const ELandscapeLayerPaintingRestriction& PaintRestriction, bool bFlipYAxis)
{
	if (LayerInfo)
	{
		ImportDataInternal<uint8>(LandscapeInfo, Filename, UseSingleFileImport(), bFlipYAxis, ImportRegionVerts, TransformType, Offset, [LandscapeInfo, LayerGuid, LayerInfo, PaintRestriction](int32 MinX, int32 MinY, int32 MaxX, int32 MaxY, const TArray<uint8>& Data)
		{
			...
			TAlphamapAccessor</*bInUseInterp = */false> AlphamapAccessor(LandscapeInfo, LayerInfo);
...
void FEdModeLandscape::ToggleSplinesTool(const ULandscapeEditLayerBase* InEditLayer)
{
	const ALandscape* Landscape = GetLandscape();
	 
	if (Landscape == nullptr || InEditLayer == nullptr)
	{
		return;
	}
 
	// Enters spline mode or toggles to the last active landscape edit mode (LastActive defaults to Sculpt mode/tool)
	if (InEditLayer->IsA<ULandscapeEditLayerSplines>() && CurrentTool && CurrentTool->GetToolName() != FName("Splines"))
	{
		SetCurrentToolMode("ToolMode_Manage", false);
	...
	{
		SetCurrentToolMode(LastActiveToolModeName, false);
		SetCurrentTool(LastActiveToolName);
	}

	SetSelectedEditLayer(Landscape->GetLayerIndex(InEditLayer->GetGuid()));
}
...
bool FEdModeLandscape::CanEditSelectedTargetLayer(FText* Reason /*=nullptr*/, const ULandscapeEditLayerBase* InLayer /*= nullptr*/)
{
	return CanEditTargetLayer(Reason, CurrentToolTarget.LayerInfo.Get(), InLayer);
}

bool FEdModeLandscape::CanEditTargetLayer(FText* Reason /*=nullptr*/, const ULandscapeLayerInfoObject* InLayerInfo  /*=nullptr*/, const ULandscapeEditLayerBase* InLayer /*= nullptr*/)
{
...
bool LandscapeEditorUtils::SetWeightmapData(ALandscapeProxy* Landscape, ULandscapeLayerInfoObject* LayerObject, const TArray<uint8>& Data)
{
	...
	if (Data.Num() == (1 + ComponentsRect.Width())*(1 + ComponentsRect.Height()))
	{
		TAlphamapAccessor</*bInUseInterp = */false> AlphamapAccessor(Landscape->GetLandscapeInfo(), LayerObject);
...
{
	return LayerInfoObj.IsValid() ? LayerInfoObj->GetLayerName() : LayerName;
}
