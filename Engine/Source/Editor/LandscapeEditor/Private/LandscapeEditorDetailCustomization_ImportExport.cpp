...
void FLandscapeEditorDetailCustomization_ImportExport::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	auto AddHeightmapFileName = [&](TSharedRef<IPropertyHandle> PropertyHandle_HeightmapFilename, TAttribute<EVisibility> PropertyVisibility)
	{
		return ImportExportCategory.AddProperty(PropertyHandle_HeightmapFilename)
			...
			// Override the ResetToDefault so we can clear the file path and the check box on reset
			.OverrideResetToDefault(FResetToDefaultOverride::Create(
				FIsResetToDefaultVisible::CreateLambda([this, PropertyHandle_HeightmapFilename](TSharedPtr<IPropertyHandle> InProperty)
				{
					FString FilePathValue;
					PropertyHandle_HeightmapFilename->GetValue(FilePathValue);
					...
					if (LandscapeEdMode && LandscapeEdMode->UISettings)
					{
						...
						return !FilePathValue.IsEmpty() || LandscapeEdMode->UISettings->bHeightmapSelected;
					}

					return false;
				}),
				FResetToDefaultHandler::CreateLambda([this, PropertyHandle_HeightmapFilename](TSharedPtr<IPropertyHandle> InProperty)
				{
					// Reset the file path 
					PropertyHandle_HeightmapFilename->SetValue(FString());

					// Reset the selected check box
					FEdModeLandscape* LandscapeEdMode = GetEditorMode();
					if (LandscapeEdMode && LandscapeEdMode->UISettings)
					{
						LandscapeEdMode->UISettings->bHeightmapSelected = false;
					}
				})
			));
	};
	...
	if (NewCheckedState == ECheckBoxState::Checked)
	{
		...
		if (Value == EImportExportMode::Import)
		{
			if (ULandscapeInfo* LandscapeInfo = LandscapeEdMode->CurrentToolTarget.LandscapeInfo.Get())
			{
				...
				if (ALandscapeGizmoActiveActor* GizmoActor = LandscapeEdMode->UISettings->CurrentGizmoActor.Get())
				{
...
FReply FLandscapeEditorDetailCustomization_ImportExport::OnBrowseFilenameButtonClicked(TSharedRef<IPropertyHandle> PropertyHandle_Filename)
{
	...
	if (OptionalFilename.IsSet() && !OptionalFilename->IsEmpty())
	{
...
FReply FLandscapeEditorDetailCustomization_ImportExport::OnImportExportButtonClicked()
{
	...
	if (ULandscapeInfo* LandscapeInfo = LandscapeEdMode->CurrentToolTarget.LandscapeInfo.Get())
	{
		...
		if (IsImporting())
		{
			...
			FVector LocalGizmoPosition = LandscapeInfo->GetLandscapeProxy()->LandscapeActorToWorld().InverseTransformPosition(LandscapeEdMode->UISettings->CurrentGizmoActor->GetActorLocation());
