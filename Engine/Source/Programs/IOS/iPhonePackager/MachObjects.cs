...
namespace MachObjectHandling
{
	...
	class MachLoadCommandSegment : MachLoadCommand
	{
		...
		public void PatchFileLength(WritingContext SW, UInt32 NewLength)
		{
			...
			PatchOffset += 1 * Bits.Bytes(AddressSize); // VirtualAddress
			// Adjust VirtualSize to accomodate a larger FileSize
			if (FileSize > VirtualSize)
			{
				// VirtualSize is the page aligned memory size required to load FileSize
				ulong PageAlignmentMask = (AddressSize == Bits.Num._64 ? (ulong)0x4000 : (ulong)0x1000) - 1;
				VirtualSize = (FileSize + PageAlignmentMask) & ~PageAlignmentMask;
				SW.PushPositionAndJump(PatchOffset);
				SW.WriteUInt(VirtualSize, AddressSize);
				SW.PopPosition();
			}
			PatchOffset += 2 * Bits.Bytes(AddressSize); // VirtualSize, FileOffset
