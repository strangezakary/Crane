...
namespace UnrealBuildTool
{
	...
	class HotReloadState
	{
		...
		HotReloadState(BinaryArchiveReader Reader)
		{
			int ArchiveVersion = Reader.ReadInt();
			if (CurrentVersion == ArchiveVersion)
			{
				NextSuffix = Reader.ReadInt();
				OriginalFileToHotReloadFile = Reader.ReadDictionary(Reader.ReadFileReference, Reader.ReadFileReference) ?? [];
				TemporaryFiles = Reader.ReadHashSet(Reader.ReadFileReference) ?? [];
			}
		}
		...
		public static FileReference GetLocation(FileReference? ProjectFile, string TargetName, UnrealTargetPlatform Platform, UnrealTargetConfiguration Configuration, UnrealArchitectures Architectures)
		{
			...
			return FileReference.Combine(BaseDir, UEBuildTarget.GetPlatformIntermediateFolder(Platform, Architectures, false), TargetName, Configuration.ToString(), "HotReloadState.bin");
		}
		...
		{
			using BinaryArchiveReader Reader = new(Location);
			return new(Reader);
		}
		...
		public void Save(FileReference Location)
		{
			...
			using BinaryArchiveWriter Writer = new(Location);
			Writer.WriteInt(CurrentVersion);
			Writer.WriteInt(NextSuffix);
			Writer.WriteDictionary(OriginalFileToHotReloadFile, Writer.WriteFileReference, Writer.WriteFileReference);
			Writer.WriteHashSet(TemporaryFiles, Writer.WriteFileReference);
		}
	}
