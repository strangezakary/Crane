...
void UCSGMeshesTool::ConvertInputsAndSetPreviewMaterials(bool bSetPreviewMesh)
{
	...
	for (int ComponentIdx = 0; ComponentIdx < Targets.Num(); ComponentIdx++)
	{
		...
		*OriginalDynamicMeshes[ComponentIdx] = UE::ToolTarget::GetDynamicMeshCopy(Targets[ComponentIdx]);
		...
		int32 NumComponentMaterials = MaterialRemap[ComponentIdx].Num();
		// If component had valid materials, remap them. (If it had no materials, no IDs are valid, so just leave them as they were.)
		if (NumComponentMaterials > 0)
		{
			for (int TID : OriginalDynamicMeshes[ComponentIdx]->TriangleIndicesItr())
			{
				int32 MID = MaterialIDs->GetValue(TID);
				MaterialIDs->SetValue(TID, MaterialRemap[ComponentIdx][MID >= 0 && MID < NumComponentMaterials ? MID : 0]);
			}
		}
