...
public:
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float4>, RWColorOut)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float>, RWDepthOut)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<uint>, RWStencilOut)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<uint>, RWBlurStencilOut)
...
BEGIN_SHADER_PARAMETER_STRUCT(FMeshProjectionNormalsFilterParameters, )
	...
	SHADER_PARAMETER_RDG_TEXTURE_SRV(Texture2D, SceneStencil)
	SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float4>, RWColorIn)
	SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float4>, RWColorOut)
	SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float>, RWDepthIn)
	SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float>, RWDepthOut)
	SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<uint>, RWStencilIn)
	SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<uint>, RWStencilOut)
	SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<uint>, RWBlurStencilIn)
	SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<uint>, RWBlurStencilOut)
	...
	SHADER_PARAMETER(uint32, bCopyToBlurStencil)
END_SHADER_PARAMETER_STRUCT()
	...
	FRenderTargetBinding& OutputRenderTargetBinding)
{
	...
	FDepthStencilBinding DepthStencilBinding(DepthTexture, ERenderTargetLoadAction::EClear, ERenderTargetLoadAction::ENoAction, FExclusiveDepthStencil::DepthWrite_StencilWrite);
...
/** Structure that holds two textures, one for input and one for output */
struct FTextureUAVWithIntermediate
{
	FRDGTextureRef PrimaryTexture = nullptr;
	FRDGTextureUAVRef PrimaryUAV = nullptr;
	...
	{
		PrimaryUAV = GraphBuilder.CreateUAV(PrimaryTexture);
...
void AddSeparableFilterPass(FRDGBuilder& GraphBuilder,
	...
	const FTextureUAVWithIntermediate& InColorTextures,
	const FTextureUAVWithIntermediate& InDepthTextures,
	const FTextureUAVWithIntermediate& InStencilTextures,
	const FTextureUAVWithIntermediate& InFinalStencilTextures,
	bool bCopyToFinalStencil = false)
{
	...
	const FScreenPassTextureViewport InputViewport(InColorTextures.PrimaryUAV->Desc.Texture);
	...
	HorizontalPassParameters->RWColorIn = InColorTextures.PrimaryUAV;
	HorizontalPassParameters->RWColorOut = InColorTextures.IntermediateUAV;
	HorizontalPassParameters->RWDepthIn = InDepthTextures.PrimaryUAV;
	HorizontalPassParameters->RWDepthOut = InDepthTextures.IntermediateUAV;
	HorizontalPassParameters->RWStencilIn = InStencilTextures.PrimaryUAV;
	HorizontalPassParameters->RWStencilOut = InStencilTextures.IntermediateUAV;
	HorizontalPassParameters->RWBlurStencilIn = InFinalStencilTextures.PrimaryUAV;
	HorizontalPassParameters->RWBlurStencilOut = InFinalStencilTextures.IntermediateUAV;
	HorizontalPassParameters->SampleDirection = FVector2f(1.0f, 0.0f);
	HorizontalPassParameters->SampleOffsetScale = FVector2f(1.f);
	HorizontalPassParameters->bCopyToBlurStencil = bCopyToFinalStencil;
	...
	VerticalPassParameters->RWColorIn = InColorTextures.IntermediateUAV;
	VerticalPassParameters->RWColorOut = InColorTextures.PrimaryUAV;
	VerticalPassParameters->RWDepthIn = InDepthTextures.IntermediateUAV;
	VerticalPassParameters->RWDepthOut = InDepthTextures.PrimaryUAV;
	VerticalPassParameters->RWStencilIn = InStencilTextures.IntermediateUAV;
	VerticalPassParameters->RWStencilOut = InStencilTextures.PrimaryUAV;
	VerticalPassParameters->RWBlurStencilIn = InFinalStencilTextures.IntermediateUAV;
	VerticalPassParameters->RWBlurStencilOut = InFinalStencilTextures.PrimaryUAV;
	VerticalPassParameters->SampleDirection = FVector2f(0.0f, 1.0f);
	VerticalPassParameters->SampleOffsetScale = FVector2f(1.f);
	VerticalPassParameters->bCopyToBlurStencil = bCopyToFinalStencil;
	...
	const FMatrix44f& NormalCorrectionMatrix)
{
	...
	// Copy the scene color and depth to RW buffers
	{
		...
		PassParameters->RWColorOut = RWColor.PrimaryUAV;
		PassParameters->RWDepthOut = RWDepth.PrimaryUAV;
		PassParameters->RWStencilOut = RWStencil.PrimaryUAV;
		PassParameters->RWBlurStencilOut = RWBlurStencil.PrimaryUAV;
	...
	{
		AddSeparableFilterPass<EMeshProjectionFilterType::DepthDilate>(GraphBuilder, GlobalShaderMap, View, SceneDepth, RWColor, RWDepth, RWStencil, RWBlurStencil);
	}

	for (int32 Index = 0; Index < DilatePasses; ++Index)
	{
		// Copy the second iteration of the dilated stencil to the blur stencil so that the region using the smaller
		// interior blur kernel extends a little beyond the stage's geometry bounds
		const bool bCopyToBlurStencil = Index == 1;
		AddSeparableFilterPass<EMeshProjectionFilterType::Dilate>(GraphBuilder, GlobalShaderMap, View, SceneDepth, RWColor, RWDepth, RWStencil, RWBlurStencil, bCopyToBlurStencil);
	}

	for (int32 Index = 0; Index < BlurPasses; Index++)
	{
		AddSeparableFilterPass<EMeshProjectionFilterType::Blur>(GraphBuilder, GlobalShaderMap, View, SceneDepth, RWColor, RWDepth, RWStencil, RWBlurStencil);
	}

	// Copy the RW buffers to the output render target
	{
		const FScreenPassTextureViewport InputViewport(RWColor.PrimaryTexture);
		...
		ScreenPassParameters->RWColorIn = RWColor.PrimaryUAV;
		ScreenPassParameters->RWDepthIn = RWDepth.PrimaryUAV;
		ScreenPassParameters->RenderTargets[0] = OutputRenderTargetBinding;
