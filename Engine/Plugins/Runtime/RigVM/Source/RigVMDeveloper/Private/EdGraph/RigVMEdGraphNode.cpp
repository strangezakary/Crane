...
FText URigVMEdGraphNode::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	if(NodeTitle.IsEmpty())
	{
		...
		if(URigVMNode* ModelNode = GetModelNode())
		{
			if (bCanRenameNode)
			{
				FString ModelNodeTitle = ModelNode->GetNodeTitleRaw();
				if (ModelNodeTitle.IsEmpty())
				{
					ModelNodeTitle = ModelNode->GetName();
					ModelNodeTitle.RemoveFromStart(TEXT("DISPATCH_"));
					ModelNodeTitle.RemoveFromStart(TEXT("RigVMFunction_"));
					ModelNodeTitle.RemoveFromStart(TEXT("RigVMDispatch_"));
				}
				NodeTitle = FText::FromString(ModelNodeTitle);
				SubTitle = ModelNode->GetNodeSubTitle();
			}

			else if (const URigVMUnitNode* UnitNode = Cast<URigVMUnitNode>(ModelNode))
			{
				...
				if (ScriptStruct && ScriptStruct->IsChildOf(FRigVMStruct::StaticStruct()))
				{
					if (const TSharedPtr<FStructOnScope> StructOnScope = UnitNode->ConstructStructInstance())
					{
						...
						SubTitle = UnitNode->GetNodeSubTitle();
					}
				}
			}
			...
			else if(URigVMFunctionReferenceNode* FunctionReferenceNode = Cast<URigVMFunctionReferenceNode>(ModelNode))
			{
				const FRigVMGraphFunctionHeader& ReferencedHeader = FunctionReferenceNode->GetReferencedFunctionHeader();
				{
					...
					const FSoftObjectPath NodeSoftPath = ReferencedHeader.LibraryPointer.GetNodeSoftPath();
					const FString& PackagePath = NodeSoftPath.GetLongPackageName();
					if(PackagePath != RefNodePtr.GetLongPackageName())
					{
						const FString AssetName = NodeSoftPath.GetAssetName();
						SubTitle = FText::Format(LOCTEXT("AssetNameSubTitle", "From {0}"), FText::FromString(AssetName)).ToString();
					}
					else
					{
						SubTitle = LOCTEXT("LocalFunctionSubTitle", "Local Function").ToString();
					}
				}
			}
			...
			else if(URigVMCollapseNode* CollapseNode = Cast<URigVMCollapseNode>(ModelNode))
			{
				...
				{
					SubTitle = LOCTEXT("CollapsedGraphSubtitle", "Collapsed Graph").ToString();
				}
			}
			...
			else if(URigVMVariableNode* VariableNode = Cast<URigVMVariableNode>(ModelNode))
			{
				...
				if (SubTitle.Len() == 0)
				{
					if(FRigVMAssetInterfacePtr Blueprint = GetAsset())
					{
						const FName VariableName = VariableNode->GetVariableName();
						for(const FRigVMGraphVariableDescription& NewVariable : Blueprint->GetAssetVariables())
						{
							if(NewVariable.Name == VariableName)
							{
								FString DefaultValue = NewVariable.DefaultValue;
								if(DefaultValue.IsEmpty())
								{
									SubTitle = LOCTEXT("VariableSubtitle", "Variable").ToString();;
								}
								else
								{
									const FEdGraphPinType PinType = NewVariable.ToPinType();
									// Change the order of values in rotators so that they match the pin order
									if (!PinType.IsContainer() && PinType.PinSubCategoryObject == TBaseStructure<FRotator>::Get())
									{
									...
									SubTitle = FText::Format(LOCTEXT("DefaultValueSubTitle", "Default {0}"), FText::FromString(DefaultValue)).ToString();
								}
								break;
							}
						}
		...
		if(IsDeprecated())
		{
			NodeTitle = FText::Format(LOCTEXT("DeprecatedNodeTitle", "{0} (Deprecated)"), NodeTitle);
		}

		if(IsOutDated())
		{
			NodeTitle = FText::Format(LOCTEXT("OutDatedNodeTitle", "{0} (OutDated)"), NodeTitle);
		}
...
void URigVMEdGraphNode::ReconstructNode_Internal(bool bForce)
{
	...
	{
		FRigVMAssetInterfacePtr RigVMBlueprint = GetAsset();
		for (UEdGraphPin* NewPin : Pins)
		{
			...
			if (URigVMPin* ModelPin = Node->FindPin(Right))
			{
				...
				{
					RigVMBlueprint->AddPinWatch(NewPin);
				}
			}
		}
	}
...
void URigVMEdGraphNode::DestroyPinList(TArray<UEdGraphPin*>& InPins)
{
	...
	const FRigVMAssetInterfacePtr Blueprint = GetAsset();
	bool bNotify = false;
	if (Blueprint != nullptr)
	{
		bNotify = !Blueprint->IsRegeneratingOnLoad();
	}
...
void URigVMEdGraphNode::PostReconstructNode()
{
	...
	{
		if(const URigVMSchema* Schema = Controller->GetSchema())
		{
			bCanRenameNode = Schema->CanSetNodeTitle(Controller, GetModelNode());
		}
	}
...
bool URigVMEdGraphNode::IsExcludedFromPreview() const
{
	if(const URigVMNode* ModelNode = Cast<URigVMNode>(GetModelNode()))
	{
		return ModelNode->IsExcludedByEarlyExit();
	}
	return false;
}
...
bool URigVMEdGraphNode::IsFadedOut() const
{
	return GetFadedOutState() < 1.f - SMALL_NUMBER;
}

float URigVMEdGraphNode::GetFadedOutState() const
{
#if WITH_EDITOR
	...
	{
		if (GetDefault<URigVMEditorSettings>()->bFadeOutUnrelatedNodes)
    	{
			return 0.25f;
		}
	}
	...
	{
		bool bWasRunDuringLastEval = false;
		if (CachedModelNode.IsValid())
		{
			if(IRigVMAssetInterface* Blueprint = GetImplementingOuter<IRigVMAssetInterface>())
			{
				if(URigVMHost* DebuggedHost = Cast<URigVMHost>(Blueprint->GetObjectBeingDebugged()))
				{
					const int32 Count = CachedModelNode->GetInstructionVisitedCount(DebuggedHost->GetRigVMExtendedExecuteContext(), DebuggedHost->GetVM(), FRigVMASTProxy());
					bWasRunDuringLastEval = Count > 0;
				}
			}
		}

		if (!bWasRunDuringLastEval)
		{
			return 0.4f;
		}
	}
...
void URigVMEdGraphNode::OverrideFadeOutState(TOptional<float> InFadedOutState)
{
...
{
	if(const IRigVMAssetInterface* Blueprint = GetImplementingOuter<IRigVMAssetInterface>())
	{
		if (const UClass* GeneratedClass = Blueprint->GetRigVMGeneratedClass())
		{
			check(GeneratedClass->IsChildOf(URigVMHost::StaticClass()));
			return const_cast<UClass*>(GeneratedClass);
		}
	}
	return nullptr;
}
...
{
#if WITH_EDITOR
	if(ProfilingColor.IsSet())
	{
		return ProfilingColor.GetValue();
	}
	if (HighlightingColor.IsSet())
	{
		return HighlightingColor.GetValue();
	}
#endif
...
void URigVMEdGraphNode::GetNodeContextMenuActions(class UToolMenu* Menu, class UGraphNodeContextMenuContext* Context) const
{
	...
	if(const FRigVMAssetInterfacePtr Blueprint = GetAsset())
	{
...
void URigVMEdGraphNode::DestroyNode()
{
	...
	if(URigVMEdGraph* Graph = Cast<URigVMEdGraph>(GetOuter()))
	{
		...
		if(IRigVMAssetInterface* RigVMBlueprint = Graph->GetImplementingOuter<IRigVMAssetInterface>())
		{
			if(PropertyName_DEPRECATED.IsValid())
			{
				FRigVMBlueprintUtils::RemoveMemberVariableIfNotUsed(RigVMBlueprint->GetObject(), PropertyName_DEPRECATED);
			}
		}
	}
...
URigVMBlueprint* URigVMEdGraphNode::GetBlueprint() const
{
	...
	{
		return Cast<URigVMBlueprint>(Graph->GetAsset().GetObject());
	}
	return nullptr;
}

FRigVMAssetInterfacePtr URigVMEdGraphNode::GetAsset() const
{
	if(const URigVMEdGraph* Graph = Cast<URigVMEdGraph>(GetOuter()))
	{
		return Graph->GetAsset();
	}
	return nullptr;
}
...
void URigVMEdGraphNode::InvalidateNodeColor()
{
	if(const URigVMNode* ModelNode = GetModelNode())
	{
		if (ModelNode->HasEarlyExitMarker())
		{
			static constexpr FLinearColor PreviewColor = FLinearColor(0.f,0.f,1.f,1.f);
			SetColorFromModel(PreviewColor);
		}
		else if (ModelNode->IsOutDated())
		{
...
void URigVMEdGraphNode::UpdateVisualSettings()
{
	bIsHighlighted.Reset();
	ProfilingColor.Reset();
	HighlightingColor.Reset();
	bEnableProfiling.Reset();
	
	if(const URigVMEdGraph* RigGraph = Cast<URigVMEdGraph>(GetOuter()))
	{
		if(const FRigVMAssetInterfacePtr RigBlueprint = RigGraph->GetAsset())
		{
			bEnableProfiling = RigBlueprint->GetVMRuntimeSettings().bEnableProfiling;
			if (bEnableProfiling.Get(false))
			{
				if(IRigVMAssetInterface* Blueprint = GetImplementingOuter<IRigVMAssetInterface>())
				{
					if(URigVMHost* DebuggedHost = Cast<URigVMHost>(Blueprint->GetObjectBeingDebugged()))
					{
						if(const URigVMNode* ModelNode = GetModelNode())
						{
							...
							MicroSeconds = Blueprint->GetRigGraphDisplaySettings().AggregateAverage(MicroSecondsFrames, MicroSeconds.Get(0), CurrentMicroSeconds);
							if(MicroSeconds.GetValue() >= 0.0)
							{
								if(Blueprint->GetRigGraphDisplaySettings().bAutoDetermineRange)
								{
									if(CurrentMicroSeconds < Blueprint->GetRigGraphDisplaySettings().MinMicroSeconds)
									{
										Blueprint->GetRigGraphDisplaySettings().MinMicroSeconds = CurrentMicroSeconds;
									}
									if(CurrentMicroSeconds > Blueprint->GetRigGraphDisplaySettings().MaxMicroSeconds)
									{
										Blueprint->GetRigGraphDisplaySettings().MaxMicroSeconds = CurrentMicroSeconds;
									}
								}
							
								const double MinMicroSeconds = Blueprint->GetRigGraphDisplaySettings().LastMinMicroSeconds;
								const double MaxMicroSeconds = Blueprint->GetRigGraphDisplaySettings().LastMaxMicroSeconds;
								if(MaxMicroSeconds > MinMicroSeconds)
								{
									const FLinearColor& MinColor = Blueprint->GetRigGraphDisplaySettings().MinDurationColor;
									const FLinearColor& MaxColor = Blueprint->GetRigGraphDisplaySettings().MaxDurationColor;

									const double T = (MicroSeconds.GetValue() - MinMicroSeconds) / (MaxMicroSeconds - MinMicroSeconds);
									ProfilingColor = FMath::Lerp<FLinearColor>(MinColor, MaxColor, (float)T);
								}
							}
						}
					}
	...
	if (const URigVMNode* ModelNode = GetModelNode())
	{
		bIsHighlighted = ModelNode->IsHighlighted();
	}
	if (bIsHighlighted.Get(false))
	{
		HighlightingColor = FSlateColor(EStyleColor::Highlight).GetSpecifiedColor();
	}
}
...
TSharedPtr<INameValidatorInterface> URigVMEdGraphNode::MakeNameValidator() const
{
	const URigVMController* Controller = GetController();
	check(Controller);
	return MakeShareable(new FRigVMGraphNodeNameValidator(Controller));
}

void URigVMEdGraphNode::RequestRename(float InDelay)
{
	if(GetCanRenameNode())
	{
		if(RequestRenameDelegate.IsBound())
		{
			if(IsInGameThread())
			{
				RequestRenameDelegate.Execute();
			}
			else
			{
				TWeakObjectPtr<URigVMEdGraphNode> WeakThis = this;
				auto RequestRenameLambda = [WeakThis]()
				{
					if(WeakThis.IsValid())
					{
						(void)WeakThis->RequestRenameDelegate.ExecuteIfBound();
						WeakThis->bRenameIsPending = false;
					}
				};
				...
				{
					FTimerHandle RequestRenameHandle;
					GEditor->GetTimerManager()->SetTimer(RequestRenameHandle, FTimerDelegate::CreateLambda([RequestRenameLambda]()
					{
						RequestRenameLambda();
					}), 1, false, InDelay);
				}
...
FRigVMGraphNodeNameValidator::FRigVMGraphNodeNameValidator(const URigVMController* InController)
: Controller(InController)
{
}

EValidatorResult FRigVMGraphNodeNameValidator::IsValid(const FString& Name, bool bOriginal)
{
	if(Name.IsEmpty())
	{
	...
	const URigVMGraph* Graph = Controller->GetGraph();
	check(Graph);

	if(Graph->FindNode(Name))
	{
		return EValidatorResult::AlreadyInUse;
	}

	const URigVMSchema* Schema = Graph->GetSchema();
	check(Schema);

	const FString SanitizedName = Schema->GetValidNodeName(Controller->GetGraph(), Name);

	if(SanitizedName.Len() < Name.Len())
	{
		return EValidatorResult::TooLong;
	}
	
	if(!SanitizedName.Equals(Name, ESearchCase::CaseSensitive))
	{
...
EValidatorResult FRigVMGraphNodeNameValidator::IsValid(const FName& Name, bool bOriginal)
{
	if(Name.IsNone())
	{
		return EValidatorResult::EmptyName;
