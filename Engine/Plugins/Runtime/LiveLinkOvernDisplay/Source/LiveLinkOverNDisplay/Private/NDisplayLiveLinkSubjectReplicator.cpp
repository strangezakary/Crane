...
namespace UE::LiveLinkOverNDisplay::Private
{
	// This variable was introduced as part of a hotfix. To avoid any class layout changes,
	// it's defined here, not as an FNDisplayLiveLinkSubjectReplicator class member.
	// 
	// It's used to pass re-initialization request from a failover AnyThread callback
	// to the main thread. Once the request is processed, the flag will be reset.
	static std::atomic<bool> bBecamePrimary = false;
}
...
{
	// On the primary node (controller), we should skip this. The underlying logic
	// is not supposed to be called on the primary nodes (controllers).
...
{
	if (UE::LiveLinkOverNDisplay::Private::bBecamePrimary)
	{
		...
		UE::LiveLinkOverNDisplay::Private::bBecamePrimary = false;
	}
...
void FNDisplayLiveLinkSubjectReplicator::OnDisplayClusterPrimaryNodeChanged(const FString& NewPrimaryId)
{
	...
	{
		UE::LiveLinkOverNDisplay::Private::bBecamePrimary = true;
	}
}
