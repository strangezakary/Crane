...
static FRDGBufferRef UpdateBoneData(
	FRDGBuilder& GraphBuilder,
	...
	TConstArrayView<FBoneIndexType> BoneMap)
{
	...
	TArray<FMatrix3x4> ChunkMatrices;
	ChunkMatrices.SetNum(NumBones);
	...
	const uint32 BytesPerElement = sizeof(FMatrix3x4);
	FRDGBufferRef BoneMatricesBuffer = CreateVertexBuffer(GraphBuilder, TEXT("Hair.SkinnedBoneMatrices"), FRDGBufferDesc::CreateBufferDesc(BytesPerElement, ChunkMatrices.Num()), ChunkMatrices.GetData(), BytesPerElement * ChunkMatrices.Num());
	return BoneMatricesBuffer;
}

static FRDGBufferRef CreateBoneBuffer(FRDGBuilder& GraphBuilder, const FSkeletalMeshObject* InMeshObject, int32 LODIndex, int32 SectionIndex)
{
	const TArray<FSkelMeshRenderSection>& Sections = InMeshObject->GetRenderSections(LODIndex);
	if (Sections.IsValidIndex(SectionIndex))
	{
		const FSkelMeshRenderSection& Section = Sections[SectionIndex];
		if (Section.IsValid())
		{
			return UpdateBoneData(GraphBuilder, InMeshObject, InMeshObject->GetReferenceToLocalMatrices(), Section.BoneMap);
		}
	}
...
struct FHairGeometryCache
{
	...
	struct FData
	{
		...
		TArray<TRefCountPtr<FRDGPooledBuffer>> BoneBuffers;
		...
		FRDGBufferSRVRef& OutTangent)
	{
		...
		if (!HashTable.IsValid(Index))
		{
			...
			Data.BoneBuffers.Init(nullptr, Data.TotalSectionCount);
		...
		for (uint32 SectionIndex : UniqueSections)
		{
			if (!Data.RequestedSectionBits[SectionIndex])
			{
				...
				if (UpdateSection.BoneBuffer == nullptr)
				{
					check(InMeshObject->IsCPUSkinned());
					if (Data.BoneBuffers[SectionIndex] == nullptr)
					{
						FRDGBufferRef BoneBuffer = CreateBoneBuffer(GraphBuilder, InMeshObject, InLODIndex, SectionIndex);
						Data.BoneBuffers[SectionIndex] = GraphBuilder.ConvertToExternalBuffer(BoneBuffer);
					}
					UpdateSection.BoneBuffer = Data.BoneBuffers[SectionIndex]->GetOrCreateSRV(GraphBuilder.RHICmdList, FRHIBufferSRVCreateInfo(PF_A32B32G32R32F));
				}
				check(UpdateSection.BoneBuffer);
		...
		if (Instance->Debug.GroomBindingType == EGroomBindingMeshType::SkeletalMesh)
		{
			if (const FSkeletalMeshSceneProxy* SceneProxy = static_cast<const FSkeletalMeshSceneProxy*>(PrimitiveSceneInfo->Proxy))
			{
				...
				else if (IsHairManualSkinCacheEnabled())
				{
					...
					FSkeletalMeshUpdater::WaitForStage(GraphBuilder, ESkeletalMeshUpdateStage::Inline);
