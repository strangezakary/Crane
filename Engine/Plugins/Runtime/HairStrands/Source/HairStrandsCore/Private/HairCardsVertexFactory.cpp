	...
	bool bSupportsManualVertexFetch)
{
	...
	if (GeometryType == EHairGeometryType::Cards)
	{
		...
		UniformParameters.NumUVs = LOD.RestResource->BulkData.Header.NumUVs;
...
void FHairCardsVertexFactory::GetPSOPrecacheVertexFetchElements(EVertexInputStreamType VertexInputStreamType, FVertexDeclarationElementList& Elements)
{
	...
	if (!bManualFetch)
	{
		...
		Elements.Add(FVertexElement(0+NumStreams, 0, FHairCardsPositionFormat::VertexElementType, 0, FHairCardsPositionFormat::SizeInByte, false)); // Current Position
		Elements.Add(FVertexElement(0+NumStreams, 0, FHairCardsPositionFormat::VertexElementType, 1, FHairCardsPositionFormat::SizeInByte, false)); // Previous Position

		uint8 NormalOffset0 = 0u;
		uint8 NormalOffset1 = FHairCardsNormalFormat::SizeInByte;
		Elements.Add(FVertexElement(1+NumStreams, NormalOffset0, FHairCardsNormalFormat::VertexElementType, 2, FHairCardsNormalFormat::SizeInByte * FHairCardsNormalFormat::ComponentCount, false)); // Normal
		Elements.Add(FVertexElement(1+NumStreams, NormalOffset1, FHairCardsNormalFormat::VertexElementType, 3, FHairCardsNormalFormat::SizeInByte * FHairCardsNormalFormat::ComponentCount, false)); // Normal
...
void FHairCardsVertexFactory::InitResources(FRHICommandListBase& RHICmdList)
{
	...
	if (!bManualFetch)
	{
		auto AddUVChannels = [this, &Elements](const FHairCardsVertexBuffer* UVsBuffer, uint32 NumUVs, uint32 StartStreamIndex)
		{
			// Add additional TexCoord
			const uint32 UVSize = sizeof(FVector2f);
			const uint32 NumUVComponentCount = FMath::DivideAndRoundUp(NumUVs, 2u); // Two UVs are packed per Float4
			const EVertexElementType UVComponentVertexElementType = VET_Float2;

			// UV[0] is rootUV which is not part of the regular UVs, hence UV starts at index 1
			for (uint32 UVIt = 1; UVIt < NumUVs; UVIt +=2)
			{
				Elements.Add(this->AccessStreamComponent(FVertexStreamComponent(UVsBuffer, 0, UVIt * UVSize, FHairCardsUVFormat::SizeInByte * NumUVComponentCount, UVComponentVertexElementType, EVertexStreamUsage::Default), StartStreamIndex++));
			}
		};
		...
		if (Data.GeometryType == EHairGeometryType::Cards)
		{
			...
			const uint32 NumUVs = LOD.RestResource->BulkData.GetNumUVs();
			const uint32 NumUVComponentCount = FMath::DivideAndRoundUp(NumUVs, 2u); // Two UVs are packed per Float4
			const bool bHasVertexColor = LOD.RestResource->BulkData.Header.bVertexColor;
			const bool bDynamic = LOD.DeformedResource != nullptr;
			const FHairCardsVertexBuffer* VertexColorsBuffer = bHasVertexColor ? &LOD.RestResource->VertexColorsBuffer : &LOD.RestResource->NormalsBuffer;
			if (bDynamic)
			{
				...
				Elements.Add(AccessStreamComponent(FVertexStreamComponent(&(DeformedPositionVertexBuffer[1]),		0, 0,									FHairCardsPositionFormat::SizeInByte,												FHairCardsPositionFormat::VertexElementType,	EVertexStreamUsage::Default), 1));

				Elements.Add(AccessStreamComponent(FVertexStreamComponent(&DeformedNormalVertexBuffer,				0, 0,									FHairCardsNormalFormat::SizeInByte * FHairCardsNormalFormat::ComponentCount,		FHairCardsNormalFormat::VertexElementType,		EVertexStreamUsage::Default), 2));
				Elements.Add(AccessStreamComponent(FVertexStreamComponent(&DeformedNormalVertexBuffer,				0, FHairCardsNormalFormat::SizeInByte,	FHairCardsNormalFormat::SizeInByte * FHairCardsNormalFormat::ComponentCount,		FHairCardsNormalFormat::VertexElementType,		EVertexStreamUsage::Default), 3));
			}
			else
			{
				...
				Elements.Add(AccessStreamComponent(FVertexStreamComponent(&LOD.RestResource->RestPositionBuffer,	0, 0,									FHairCardsPositionFormat::SizeInByte,												FHairCardsPositionFormat::VertexElementType,	EVertexStreamUsage::Default), 1));

				Elements.Add(AccessStreamComponent(FVertexStreamComponent(&LOD.RestResource->NormalsBuffer,			0, 0,									FHairCardsNormalFormat::SizeInByte * FHairCardsNormalFormat::ComponentCount,		FHairCardsNormalFormat::VertexElementType,		EVertexStreamUsage::Default), 2));
				Elements.Add(AccessStreamComponent(FVertexStreamComponent(&LOD.RestResource->NormalsBuffer,			0, FHairCardsNormalFormat::SizeInByte,	FHairCardsNormalFormat::SizeInByte * FHairCardsNormalFormat::ComponentCount,		FHairCardsNormalFormat::VertexElementType,		EVertexStreamUsage::Default), 3));
			}
			...
			Elements.Add(AccessStreamComponent(FVertexStreamComponent(VertexColorsBuffer,							0, 0,									FHairCardsMaterialFormat::SizeInByte,												FHairCardsMaterialFormat::VertexElementType,	EVertexStreamUsage::Default), 5));
			Elements.Add(AccessStreamComponent(FVertexStreamComponent(&LOD.RestResource->UVsBuffer,					0, 0,									FHairCardsUVFormat::SizeInByte * NumUVComponentCount,														FHairCardsUVFormat::VertexElementType,			EVertexStreamUsage::Default), 6));

			// Add additional UVs
			AddUVChannels(&LOD.RestResource->UVsBuffer, LOD.RestResource->BulkData.GetNumUVs(), 7);
		...
		else if (Data.GeometryType == EHairGeometryType::Meshes)
		{
			...
			Elements.Add(AccessStreamComponent(FVertexStreamComponent(&LOD.RestResource->VertexColorsBuffer,		0, 0,									FHairCardsMaterialFormat::SizeInByte,												FHairCardsMaterialFormat::VertexElementType,	EVertexStreamUsage::Default), 5));
			Elements.Add(AccessStreamComponent(FVertexStreamComponent(&LOD.RestResource->UVsBuffer,					0, 0,									FHairCardsUVFormat::SizeInByte * NumUVComponentCount,								FHairCardsUVFormat::VertexElementType,			EVertexStreamUsage::Default), 6));
