...
static bool AddHairStrandUpdateMeshTrianglesPass(
	...
	const TArray<uint32>& UniqueMeshSectionIndices,
	...
	FRDGBufferUAVRef OutputPrevUAV)
{
	...
	for (uint32 MeshSectionIndex : UniqueMeshSectionIndices)
	{
		// MeshLODData.Sections can contains only a sparse set of sections (vs. all sections). 
		// Thus, the section's index needs to be remapped
		const int32 RemappedSectionIndex = MeshLODData.Sections.FindLastByPredicate(
			[MeshSectionIndex](const FCachedGeometry::Section& In) 
			{ 
...
{	
	...
	{
		OutputPrevBuffer = Register(GraphBuilder, DeformedResources->GetLOD(MeshLODIndex)->GetDeformedUniqueTrianglePositionBuffer(FHairStrandsLODDeformedRootResource::Previous), ERDGImportedBufferFlags::CreateUAV, ERDGUnorderedAccessViewFlags::SkipBarrier);
	}
	...
	if (AddHairStrandUpdateMeshTrianglesPass(
			...
			RestLODData.GetBulkData().Header.UniqueSectionIndices,
			MeshLODData,
