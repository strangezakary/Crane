...
{
	IRemoteControlProtocolModule::Get().ApplyProtocolBindings(GetPreset());
}

void SRemoteControlPanel::UnapplyProtocolBindings()
{
	IRemoteControlProtocolModule::Get().UnapplyProtocolBindings(GetPreset());
}
...
void SRemoteControlPanel::Construct(const FArguments& InArgs, URemoteControlPreset* InPreset, TSharedPtr<IToolkitHost> InToolkitHost)
{
	...
	EntityList = SNew(SRCPanelExposedEntitiesList, InPreset, WidgetRegistry)
...
{
	const URemoteControlPreset* const Preset = GetPreset();
	if (!Preset)
	{
		return false;
	}

	TArray<TSharedPtr<const FRemoteControlProperty>, TInlineAllocator<1>> PotentialMatches;
	for (const TWeakPtr<const FRemoteControlProperty>& WeakProperty : Preset->GetExposedEntities<FRemoteControlProperty>())
	{
		if (TSharedPtr<const FRemoteControlProperty> Property = WeakProperty.Pin())
		{
			...
			const bool bIsBound = bUsingDuplicatesInPath ? Property->CheckIsBoundToPropertyPath(InPath) : Property->CheckIsBoundToString(InPath);
			if (bIsBound)
			{
	...
	for (UObject* OuterObject : InOuterObjects)
	{
		...
		for (const TSharedPtr<const FRemoteControlProperty>& Property : PotentialMatches)
		{
	...
	MenuBuilder.BeginSection(NAME_None, LOCTEXT("ExposeHeader", "Expose"));
	{
		...
		else
		{
			MenuBuilder.AddSubMenu(
				...
				FNewMenuDelegate::CreateSPLambda(this,
					[This=this](FMenuBuilder& SubMenuBuilder)
					{
						FSceneOutlinerModule& SceneOutlinerModule = FModuleManager::LoadModuleChecked<FSceneOutlinerModule>("SceneOutliner");
						FSceneOutlinerInitializationOptions InitOptions;
						constexpr bool bAllowPIE = false;
						UWorld* PresetWorld = URemoteControlPreset::GetWorld(This->GetPreset(), bAllowPIE);
...
TSharedRef<SWidget> SRemoteControlPanel::CreateExposeByClassWidget()
{
	...
	TSharedRef<SWidget> Widget = FModuleManager::LoadModuleChecked<FClassViewerModule>("ClassViewer").CreateClassViewer(Options, FOnClassPicked::CreateSPLambda(this,
		[This=this](UClass* ChosenClass)
		{
			constexpr bool bAllowPIE = false;

			if (UWorld* World = URemoteControlPreset::GetWorld(This->GetPreset(), bAllowPIE))
			{
			...
			if (This->ExposeActorsComboButton)
			{
				This->ExposeActorsComboButton->SetIsOpen(false);
			}
		}));
...
void SRemoteControlPanel::CacheLevelClasses()
{
	...
	if (UWorld* World = URemoteControlPreset::GetWorld(GetPreset(), bAllowPIE))
	{
...
void SRemoteControlPanel::OnMapChange(uint32)
{
	...
	if (GWorld && (!Preset || !Preset->SelectedWorld.IsValid()))
	{
...
{
	URemoteControlPreset* const Preset = GetPreset();
	if (!WidgetRegistry.IsValid() || !Preset || !Preset->SelectedWorld.IsValid())
	{
		return;
	}
...
void SRemoteControlPanel::PostLoadMapWithWorld(UWorld* InWorld)
{
	...
	const UWorld* const SelectedWorld = Preset ? Preset->SelectedWorld.Get() : nullptr;
	if (SelectedWorld && SelectedWorld->WorldType == EWorldType::PIE)
	{
		UpdatePanelForWorld(InWorld);
	}
}
...
void SRemoteControlPanel::OnEndPIE(const bool bInIsSimulating)
{
	...
	if (URemoteControlPreset* const Preset = GetPreset())
	{
...
void SRemoteControlPanel::RegisterEvents()
{
	...
	FCoreUObjectDelegates::PostLoadMapWithWorld.AddSP(this, &SRemoteControlPanel::PostLoadMapWithWorld);
...
void SRemoteControlPanel::Unexpose(const FRCExposesPropertyArgs& InPropertyArgs)
{
	...
	auto CheckAndUnexpose = [Preset](TArray<UObject*> InOuterObjects, const FString& InPath, bool bInUsingDuplicatesInPath)
	{
...
void SRemoteControlPanel::UpdateEntityDetailsView(const TSharedPtr<SRCPanelTreeNode>& SelectedNode)
{
	...
	if(LastSelectedEntity.IsValid() && FModuleManager::Get().IsModuleLoaded(ProtocolWidgetsModuleName))
	{
		if (const TSharedPtr<FRemoteControlEntity> RCEntity = Preset->GetExposedEntity(LastSelectedEntity->GetRCId()).Pin())
		{
			if(RCEntity->IsBound())
			{
				...
				EntityProtocolDetails->SetContent(ProtocolWidgetsModule.GenerateDetailsForEntity(Preset, RCEntity->GetId()));
			}
			else
			{
...
void SRemoteControlPanel::UpdateRebindButtonVisibility()
{
	if (URemoteControlPreset* Preset = GetPreset())
	{
		for (TWeakPtr<FRemoteControlEntity> WeakEntity : Preset->GetExposedEntities<FRemoteControlEntity>())
		{
...
void SRemoteControlPanel::SaveAsset() const
{
	if (const URemoteControlPreset* const Preset = GetPreset())
	{
		...
		if (!Preset->IsAsset())
		{
			...
			UE_LOG(LogRemoteControl, Log, TEXT("Invalid object to save: %s. Preset is not an asset"), *Preset->GetFullName());
		}
		else
		{
...
void SRemoteControlPanel::FindInContentBrowser() const
{
	...
	{
		GEditor->SyncBrowserToObject(Preset);
	}
}
...
bool SRemoteControlPanel::CanDeleteEntity() const
{
	...
	const URemoteControlPreset* const Preset = GetPreset();
	if (LastSelectedEntity.IsValid() && Preset)
	{
...
{
	URemoteControlPreset* const Preset = GetPreset();
	if (!IsValid(InWorld) || (Preset && InWorld == Preset->SelectedWorld.Get()))
	{
		return;
	}
...
{
	UAssetEditorSubsystem* const AssetSubsystem = GEditor ? GEditor->GetEditorSubsystem<UAssetEditorSubsystem>() : nullptr;
	if (!AssetSubsystem)
	{
		return;
	}
	...
	URemoteControlPreset* SelectedEmbeddedPreset = nullptr;

	const ULevel* const SelectedLevel = InWorld ? InWorld->GetCurrentLevel() : nullptr;

	if (SelectedLevel)
	{
		for (TWeakObjectPtr<URemoteControlPreset> EmbeddedPresetWeak : EmbeddedPresets)
		{
			URemoteControlPreset* const EmbeddedPreset = EmbeddedPresetWeak.Get();
			if (EmbeddedPreset && EmbeddedPreset->GetTypedOuter<ULevel>() == SelectedLevel)
			{
				SelectedEmbeddedPreset = EmbeddedPreset;
				break;
			}
		}
	}

	AssetSubsystem->OpenEditorForAsset(SelectedEmbeddedPreset);
}
