...
#include "Tests/RemoteControlInterceptionForwardInterceptor.h"
...
void FRemoteControlModule::StartupModule()
{
	...
	ConsoleObjects.Add(IConsoleManager::Get().RegisterConsoleCommand(
		TEXT("RemoteControl.EnableLocalForwardingInterceptor"),
		TEXT("Enables/Disables the local forwarding interceptor debugging mode.\n")
		TEXT("This allows to emulate remote control interception and processing locally.\n")
		TEXT("Usage: \"RemoteControl.EnableLocalForwardingInterceptor [true/false] or [0/1]\""),
		FConsoleCommandWithArgsDelegate::CreateLambda([this](const TArray<FString>& InArgs)
		{
			const bool bEnable = InArgs.Num() > 0 ? InArgs[0].ToBool() : true;
			if (bEnable)
			{
				RegisterLocalForwardingInterceptor();
				UE_LOG(LogRemoteControl, Log, TEXT("Local Forwarding Interception is enabled."));
			}
			else
			{
				UnregisterLocalForwardingInterceptor();
				UE_LOG(LogRemoteControl, Log, TEXT("Local Forwarding Interception is disabled."));
			}
		}),
		ECVF_Default
		));
...
bool FRemoteControlModule::SetPresetController(const FName PresetName, class URCVirtualPropertyBase* VirtualProperty, IStructDeserializerBackend& Backend, const TArray<uint8>& InPayload, const bool bAllowIntercept)
{
	...
	if (bSuccess)
	{
		Preset->OnControllerModified().Broadcast(Preset, {VirtualProperty->Id});
	}
	return bSuccess;
}
...
bool FRemoteControlModule::CanBeAccessedRemotely(UObject* Object) const
{
	...
	if (!GetDefault<URemoteControlSettings>()->bEnableRemotePythonExecution)
	{
		...
		static const FName PythonScriptSettingsName = "PythonScriptPluginSettings";

		const FName ObjectClassName = Object->GetClass()->GetFName();
		if (ObjectClassName == PythonScriptName || ObjectClassName == PythonScriptSettingsName)
		{
			return false;
		}
	}
...
void FRemoteControlModule::UnregisterEditorDelegates()
{
	...
	{
		FTSTicker::RemoveTicker(FallbackOngoingChangeTimer);
	}
}
...
void FRemoteControlModule::RegisterLocalForwardingInterceptor()
{
	if (!RCILocalForwardingInterceptor)
	{
		RCILocalForwardingInterceptor =  MakeUnique<FRemoteControlInterceptionForwardInterceptor>();
		IModularFeatures::Get().RegisterModularFeature(IRemoteControlInterceptionFeatureInterceptor::GetName(), RCILocalForwardingInterceptor.Get());
	}
}

void FRemoteControlModule::UnregisterLocalForwardingInterceptor()
{
	if (RCILocalForwardingInterceptor)
	{
		IModularFeatures::Get().UnregisterModularFeature(IRemoteControlInterceptionFeatureInterceptor::GetName(), RCILocalForwardingInterceptor.Get());
		RCILocalForwardingInterceptor.Reset();
	}
}

void FRemoteControlModule::UnregisterConsoleObjects()
{
