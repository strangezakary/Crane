...
void UAvaMask2DBaseModifier::SavePreState()
{
	...
	ScannedPrimitiveComponents.Empty();
	TSet<AActor*> KeepActors;

	ForEachActor<AActor>(
		[this, &KeepActors](AActor* InActor)
		{
			...
			{
				SaveActorPreState(InActor, ActorData.FindOrAdd(InActor));
				KeepActors.Add(InActor);
			}
	...
	// Cleanup
	for (TMap<TWeakObjectPtr<AActor>, FAvaMask2DActorData>::TIterator It(ActorData); It; ++It)
	{
		if (!KeepActors.Contains(It->Key.Get()))
		{
			It.RemoveCurrent();
		}
	}
...
void UAvaMask2DBaseModifier::SaveActorPreState(AActor* InActor, FAvaMask2DActorData& InActorData)
{
	...
	Algo::Transform(PrimitiveComponents, ScannedPrimitiveComponents, [](UPrimitiveComponent* InComponent)
	{
		return InComponent;
	});
}
...
void UAvaMask2DBaseModifier::OnRenderStateUpdated(AActor* InActor, UActorComponent* InComponent)
{
	...
	if (MaterialCount != 0)
	{
		if (const FAvaMask2DActorData* MaskData = ActorData.Find(InActor))
		{
			if (CanvasWeak.IsValid()
				&& MaskData->CanvasTextureWeak.IsValid()
				&& ScannedPrimitiveComponents.Contains(PrimitiveComponent))
			{
				return;
			}
		}
...
void UAvaMask2DBaseModifier::OnMaterialsChanged(UObject* InMaterialOwner, const TArray<TSharedPtr<IAvaMaskMaterialHandle>>& InMaterialHandles)
{
	if (!InMaterialHandles.IsEmpty())
	{
		if (AActor* OwningActor = InMaterialOwner->GetTypedOuter<AActor>())
		{
			...
			// Only trigger update if first modifier found in the hierarchy above us is this modifier
			if (MaskModifier == this && ActorData.Contains(OwningActor))
			{
				FInstancedStruct* HandleData = MaterialCollectionHandleData.Find(OwningActor);
...
void UAvaMask2DBaseModifier::RestoreActors()
{
	...
	for (TPair<TWeakObjectPtr<AActor>, FAvaMask2DActorData>& ActorDataPair : ActorDataCopy)
	{
		if (AActor* Actor = ActorDataPair.Key.Get())
		{
			...
			// Do not remove ActorData, only handles
			MaterialCollectionHandleData.Remove(Actor);
			MaterialCollectionHandles.Remove(Actor);
		}
	}
