...
static UTickableConstraint* CreateConstraintIfNeeded(UWorld* InWorld, const FConstraintAndActiveValue& InConstraintValue, UMovieSceneControlRigParameterSection* InSection)
{
	...
	if (InWorld)
	{
		...
		{
			FConstraintsManagerController::Get(InWorld).AddConstraint(InConstraintValue.Constraint.Get());
			...
			if (ControlRig)
			{
				...
				if (BoundObject)
				{
					...
					for (FConstraintAndActiveValue& ConstraintValue : ConstraintsValues)
					{
						...
						{
							UTickableTransformConstraint* TransformConstraint = Cast<UTickableTransformConstraint>(ConstraintValue.Constraint);
							...
							if (TransformConstraint && ConstraintValue.Value && TransformableHandleUtils::SkipTicking())
							{
