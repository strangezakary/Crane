...
void FControlRigSchematicModel::Tick(float InDeltaTime)
{
	...
	if(ControlRigBlueprint.IsValid())
	{
		...
		if (UControlRig* DebuggedRig = ControlRigBlueprint->GetDebuggedControlRig())
		{
			...
			check(Hierarchy);
			const FModularRigConnections& Connections = ControlRigBlueprint->GetModularRigModel().Connections;
					...
					if(const URigHierarchy* Hierarchy = ControlRigBeingDebuggedPtr->GetHierarchy())
					{
						if(const FRigConnectorElement* Connector = Hierarchy->Find<FRigConnectorElement>(InElementKey))
						{
							if(Connector->IsPrimary())
							{
								...
								if(ControlRigBlueprint->GetModularRigModel().Connections.FindConnectorsFromTarget(ResolvedKey).Num() == 1)
								{
	...
	if (AssetDragDropOp.IsValid())
	{
		for (const FAssetData& AssetData : AssetDragDropOp->GetAssets())
		{
			...
			if (UControlRigBlueprint* AssetBlueprint = Cast<UControlRigBlueprint>(AssetData.GetAsset()))
			{
				...
				ModuleSettings = AssetBlueprint->RigModuleSettings;
			}
			...
			if(ControlRigClass && ModuleSettings.IsValidModule())
			{
				ExecuteOnGameThread(UE_SOURCE_LOCATION, [this, ControlRigClass, ModuleSettings, TargetKeys]()
				{
					if (UModularRigController* Controller = ControlRigBlueprint->GetModularRigController())
					{
						...
						{
							const FName DesiredModuleName = Controller->GetSafeNewName(FRigName(ModuleSettings.Identifier.Name));
...
bool FControlRigSchematicModel::IsConnectorResolved(const FRigElementKey& InConnectorKey, FRigElementKey* OutKey) const
{
	...
	{
		const FRigElementKey TargetKey = ControlRigBlueprint->GetModularRigModel().Connections.FindTargetFromConnector(InConnectorKey);
