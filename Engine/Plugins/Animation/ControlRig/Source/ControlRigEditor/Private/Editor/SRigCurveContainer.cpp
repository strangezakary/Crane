...
void SRigCurveContainer::Construct(const FArguments& InArgs, TSharedRef<IControlRigBaseEditor> InControlRigEditor)
{
	...
	ControlRigBlueprint = InControlRigEditor.Get().GetControlRigAssetInterface();
	bIsChangingRigHierarchy = false;

	ControlRigBlueprint->GetHierarchy()->OnModified().AddRaw(this, &SRigCurveContainer::OnHierarchyModified_AnyThread);
	ControlRigBlueprint->GetRigVMAssetInterface()->OnSetObjectBeingDebugged().AddRaw(this, &SRigCurveContainer::HandleSetObjectBeingDebugged);
	ControlRigBlueprint->GetRigVMAssetInterface()->OnRefreshEditor().AddRaw(this, &SRigCurveContainer::HandleRefreshEditorFromBlueprint);
...
SRigCurveContainer::~SRigCurveContainer()
{
	...
	OnEditorClose(Editor, ControlRigBlueprint.ToScriptInterface());
}
...
void SRigCurveContainer::OnEditorClose(IControlRigBaseEditor* InEditor, FControlRigAssetInterfacePtr InBlueprint)
{
	if (InBlueprint)
	{
...
void SRigCurveContainer::RefreshCurveList_AnyThread()
{
	if(IsInGameThread())
	{
		RefreshCurveList();
...
{
	// For actions that involve topological changes, the code uses the default hierarchy
	// But here, we use the instance, in order to be able to see the curve values while they change in the debugged instance
...
void SRigCurveContainer::OnHierarchyModified_AnyThread(ERigHierarchyNotification InNotif, URigHierarchy* InHierarchy, const FRigNotificationSubject& InSubject)
{
	if(bIsChangingRigHierarchy)
	{
		return;
	...
	{
		FRigElementKey Key;
		if(const FRigBaseElement* InElement = InSubject.Element)
		{
...
void SRigCurveContainer::OnPostConstruction(UControlRig* InControlRig, const FName& InName)
{
	RefreshCurveList_AnyThread();
}

void SRigCurveContainer::HandleSetObjectBeingDebugged(UObject* InObject)
{
	...
	if(UControlRig* ControlRig = Cast<UControlRig>(InObject))
	{
		...
		if(URigHierarchy* Hierarchy = ControlRig->GetHierarchy())
		{
			...
			Hierarchy->OnModified().AddSP(this, &SRigCurveContainer::OnHierarchyModified_AnyThread);
		}
		ControlRig->OnPostConstruction_AnyThread().RemoveAll(this);
		ControlRig->OnPostConstruction_AnyThread().AddSP(this, &SRigCurveContainer::OnPostConstruction);
	}

	RefreshCurveList_AnyThread();
}

void SRigCurveContainer::HandleRefreshEditorFromBlueprint(FRigVMAssetInterfacePtr InBlueprint)
{
	RefreshCurveList_AnyThread();
}
...
void SRigCurveContainer::ImportCurve(const FAssetData& InAssetData)
{
	...
	if (Hierarchy)
	{
		...
		if (Mesh)
		{
			...
			ControlRigBlueprint->GetSourceCurveImport() = Skeleton;
		}
		else 
		{
...
URigHierarchy* SRigCurveContainer::GetDefaultHierarchy() const
{
	if (ControlRigBlueprint.IsValid())
	{
