...
void SRigHierarchy::Construct(const FArguments& InArgs, TSharedRef<IControlRigBaseEditor> InControlRigEditor)
{
	...
	ControlRigBlueprint->GetHierarchy()->OnModified().AddRaw(this, &SRigHierarchy::OnHierarchyModified);
	ControlRigBlueprint->GetRigVMAssetInterface()->OnRefreshEditor().AddRaw(this, &SRigHierarchy::HandleRefreshEditorFromBlueprint);
	ControlRigBlueprint->GetRigVMAssetInterface()->OnSetObjectBeingDebugged().AddRaw(this, &SRigHierarchy::HandleSetObjectBeingDebugged);
...
void SRigHierarchy::OnEditorClose(IControlRigBaseEditor* InEditor, FControlRigAssetInterfacePtr InBlueprint)
{
	if (InEditor)
	{
	...
	if (InBlueprint)
	{
		InBlueprint->GetHierarchy()->OnModified().RemoveAll(this);
...
void SRigHierarchy::BindCommands()
{
	...
	CommandList->MapAction(
		Commands.ZeroControls,
		FExecuteAction::CreateSP(this, &SRigHierarchy::HandleZeroControls),
		FCanExecuteAction::CreateSP(this, &SRigHierarchy::CanHandleZeroControls));

	CommandList->MapAction(
		Commands.ZeroControlsFromClosestBone,
		FExecuteAction::CreateSP(this, &SRigHierarchy::HandleZeroControlsFromClosestBone),
		FCanExecuteAction::CreateSP(this, &SRigHierarchy::CanHandleZeroControlsFromClosestBone));

	CommandList->MapAction(
		Commands.ZeroControlShape,
		FExecuteAction::CreateSP(this, &SRigHierarchy::HandleZeroControlShape),
		FCanExecuteAction::CreateSP(this, &SRigHierarchy::CanHandleZeroControlShape));

	CommandList->MapAction(
		Commands.SetInitialTransformFromCurrent,
		FExecuteAction::CreateSP(this, &SRigHierarchy::HandleSetInitialTransformsAsCurrent),
		FCanExecuteAction::CreateSP(this, &SRigHierarchy::CanHandleSetInitialTransformsAsCurrent));
...
void SRigHierarchy::OnHierarchyModified(ERigHierarchyNotification InNotif, URigHierarchy* InHierarchy, const FRigNotificationSubject& InSubject)
{
	...
	if (ControlRigBlueprint->GetRigVMAssetInterface()->bSuspendAllNotifications)
	{
		return;
	}
...
void SRigHierarchy::HandleRefreshEditorFromBlueprint(FRigVMAssetInterfacePtr InBlueprint)
{
	if (bIsChangingRigHierarchy)
	{
		return;
		...
		MenuBuilder.AddWidget(
			...
			[
				...
				.CurrentValue_Lambda([this]() -> int32
				{
					...
					if (FControlRigAssetInterfacePtr Blueprint = StrongEditor->GetControlRigAssetInterface())
					{
						return static_cast<int32>(Blueprint->GetHierarchySettings().ElementNameDisplayMode);
					}
				...
				.OnEnumSelectionChanged_Lambda([this](int32 InEnumValue, ESelectInfo::Type)
				{
					...
					if (FControlRigAssetInterfacePtr Blueprint = StrongEditor->GetControlRigAssetInterface())
					{
						...
						Blueprint->Modify();
						Blueprint->GetHierarchySettings().ElementNameDisplayMode = static_cast<EElementNameDisplayMode>(InEnumValue);
		...
		Menu->AddDynamicSection(NAME_None, FNewToolMenuDelegate::CreateLambda([](UToolMenu* InMenu)
			{
				...
				if (SRigHierarchy* RigHierarchyPanel = MainContext->GetRigHierarchyPanel())
				{
										...
										*FString::Printf(TEXT("Add%s"), *ComponentName),
										...
										FUIAction(
											FExecuteAction::CreateLambda([RigHierarchyPanel, RigComponentStruct, SelectedElements, ComponentLabel]() {
												...
												FControlRigAssetInterfacePtr CurrentControlRigBlueprint = RigHierarchyPanel->ControlRigBlueprint.GetObject();
					...
					if(RigHierarchyPanel->IsProceduralSelected() && RigHierarchyPanel->ControlRigBlueprint.IsValid())
					{
						ElementsSection.AddMenuEntry(
							...
							FUIAction(FExecuteAction::CreateLambda([RigHierarchyPanel]() {
								...
								const FControlRigAssetInterfacePtr CurrentControlRigBlueprint = RigHierarchyPanel->ControlRigBlueprint.GetObject();
									...
									if(Element->IsProcedural())
									{
										...
										if(const UControlRig* ControlRig = Cast<UControlRig>(CurrentControlRigBlueprint->GetObjectBeingDebugged()))
										{
											if(ControlRig->VM)
											{
												...
												{
													if(URigVMController* Controller = CurrentControlRigBlueprint->GetRigVMAssetInterface()->GetController(Node->GetGraph()))
													{
...

					{
						...
						const FText FromCurrentLabel = FText::Format(LOCTEXT("SetTransformFromCurrentTransform", "Zero {0}"), FText::FromString(*Keyword));
						const FText FromClosestBoneLabel = FText::Format(LOCTEXT("SetTransformFromClosestBone", "Zero {0} from Closest Bone"), FText::FromString(*Keyword));
						TransformsSection.AddMenuEntry(Commands.ZeroControls, FromCurrentLabel);
						TransformsSection.AddMenuEntry(Commands.ZeroControlsFromClosestBone, FromClosestBoneLabel);
					}
					
					TransformsSection.AddMenuEntry(Commands.ZeroControlShape);
					TransformsSection.AddMenuEntry(Commands.SetInitialTransformFromCurrent);
...
void SRigHierarchy::RefreshHierarchy(const FAssetData& InAssetData, bool bOnlyResetCurves, bool bRestrictToMeshBones)
{
	...
	if (Mesh && Hierarchy)
	{
		TGuardValue<bool> SuspendBlueprintNotifs(ControlRigBlueprint->GetRigVMAssetInterface()->bSuspendAllNotifications, true);
...
{
	if (const FControlRigAssetInterfacePtr Blueprint = ControlRigEditor.Pin()->GetControlRigAssetInterface())
	{
		const FRigElementKey ResolvedKey = Blueprint->GetModularRigModel().Connections.FindTargetFromConnector(InKey.GetElement());
...
void SRigHierarchy::ImportHierarchy(const FAssetData& InAssetData)
{
	...
	if (URigHierarchy* Hierarchy = GetDefaultHierarchy())
	{
		...
		ControlRigBlueprint->GetSourceHierarchyImport() = Mesh->GetSkeleton();
		ControlRigBlueprint->GetSourceCurveImport() = Mesh->GetSkeleton();
			...
			const FName NewElementName = CreateUniqueName(*NewNameTemplate, InElementType);
			{
				...
				switch (InElementType)
				{
					...
					case ERigElementType::Connector:
					{
						...
						{
							if(ControlRigBlueprint->GetHierarchy()->Num(ERigElementType::Connector) == 0)
							{
								if(!ControlRigBlueprint->IsControlRigModule())
								{
...
URigHierarchy* SRigHierarchy::GetDefaultHierarchy() const
{
	if (ControlRigBlueprint.IsValid())
	{
		return ControlRigBlueprint->GetHierarchy();
	}
	return nullptr;
}
		...
		for (const FAssetData& AssetData : AssetDragDropOp->GetAssets())
		{
			...
			if(FControlRigAssetInterfacePtr AssetBlueprint = AssetData.GetAsset())
			{
				if (UModularRigController* Controller = ControlRigBlueprint->GetModularRigController())
				{
					...
					for (FRigModuleConnector& Connector : AssetBlueprint->GetRigModuleSettings().ExposedConnectors)
					{
						if (Connector.IsPrimary())
						{
	...
	if (AssetDragDropOp.IsValid())
	{
		for (const FAssetData& AssetData : AssetDragDropOp->GetAssets())
		{
			...
			if(FControlRigAssetInterfacePtr AssetBlueprint = AssetData.GetAsset())
			{
				...
				ModuleSettings = AssetBlueprint->GetRigModuleSettings();
			}
			else if (UControlRigBlueprintGeneratedClass* GeneratedClass = Cast<UControlRigBlueprintGeneratedClass>(AssetData.GetAsset()))
			{
				...
				ModuleSettings = GeneratedClass->RigModuleSettings;
			}
			...
			if(ControlRigClass)
			{
				if (UModularRigController* Controller = ControlRigBlueprint->GetModularRigController())
				{
					const FRigName DesiredModuleName = Controller->GetSafeNewName(FRigName(ModuleSettings.Identifier.Name));
					const FName ModuleName = Controller->AddModule(DesiredModuleName.GetFName(), ControlRigClass, NAME_None);
...
void SRigHierarchy::HandleResetTransform(bool bSelectionOnly)
{
	...
	{
		if (FControlRigAssetInterfacePtr Blueprint = ControlRigEditor.Pin()->GetControlRigAssetInterface())
		{
			if (URigHierarchy* DebuggedHierarchy = GetHierarchy())
			{
...
void SRigHierarchy::HandleZeroControls()
{
	const TSharedPtr<IControlRigBaseEditor> PinnedControlRigEditor = ControlRigEditor.IsValid() ? ControlRigEditor.Pin() : nullptr;
	const TScriptInterface<IControlRigAssetInterface> Blueprint = PinnedControlRigEditor.IsValid() ? PinnedControlRigEditor->GetControlRigAssetInterface() : nullptr;
	URigHierarchy* DefaultHierarchy = GetDefaultHierarchy();
	URigHierarchy* DebuggedHierarchy = GetHierarchy();

	if (CanHandleZeroControls() &&
		PinnedControlRigEditor.IsValid() &&
		Blueprint &&
		DefaultHierarchy &&
		DebuggedHierarchy)
	{
		const FScopedTransaction Transaction(LOCTEXT("HandleZeroControlsTransaction", "Zero Controls"));
		...
		for (const FRigElementKey& SelectedKey : SelectedKeys)
		{
			...
			const FTransform GlobalTransform = GlobalTransforms[SelectedKey];
			const FTransform LocalTransform = GlobalTransform.GetRelativeTransform(ParentGlobalTransforms[SelectedKey]);
			...
			if (FRigControlElement* DebuggedControlElement = DebuggedHierarchy->Find<FRigControlElement>(SelectedKey))
			{
				DebuggedHierarchy->SetControlOffsetTransform(DebuggedControlElement, LocalTransform, ERigTransformType::InitialLocal, bAffectChildren, bSetupUndo, bForce, bPrintPythonCommand);
				DebuggedHierarchy->SetControlOffsetTransform(DebuggedControlElement, LocalTransform, ERigTransformType::CurrentLocal, bAffectChildren, bSetupUndo, bForce, bPrintPythonCommand);
				DebuggedHierarchy->SetTransform(DebuggedControlElement, FTransform::Identity, ERigTransformType::InitialLocal, bAffectChildren, bSetupUndo, bForce, bPrintPythonCommand);
				DebuggedHierarchy->SetTransform(DebuggedControlElement, FTransform::Identity, ERigTransformType::CurrentLocal, bAffectChildren, bSetupUndo, bForce, bPrintPythonCommand);
			}

			if (FRigControlElement* DefaultControlElement = DefaultHierarchy->Find<FRigControlElement>(SelectedKey))
			{
				DefaultHierarchy->SetControlOffsetTransform(DefaultControlElement, LocalTransform, ERigTransformType::InitialLocal, bAffectChildren, bSetupUndo);
				DefaultHierarchy->SetControlOffsetTransform(DefaultControlElement, LocalTransform, ERigTransformType::CurrentLocal, bAffectChildren, bSetupUndo);
				DefaultHierarchy->SetTransform(DefaultControlElement, FTransform::Identity, ERigTransformType::InitialLocal, bAffectChildren, bSetupUndo);
				DefaultHierarchy->SetTransform(DefaultControlElement, FTransform::Identity, ERigTransformType::CurrentLocal, bAffectChildren, bSetupUndo);
			}
		}
	}
}

bool SRigHierarchy::CanHandleZeroControls() const
{
	if (IsProceduralSelected())
	{
		return false;
	}

	return GetSelectedElementKeys().ContainsByPredicate(
		[](const FRigElementKey& SelectedKey)
		{
			return SelectedKey.Type == ERigElementType::Control;
		});
}

void SRigHierarchy::HandleZeroControlsFromClosestBone()
{
	const TSharedPtr<IControlRigBaseEditor> PinnedControlRigEditor = ControlRigEditor.IsValid() ? ControlRigEditor.Pin() : nullptr;
	const TScriptInterface<IControlRigAssetInterface> Blueprint = PinnedControlRigEditor.IsValid() ? PinnedControlRigEditor->GetControlRigAssetInterface() : nullptr;
	URigHierarchy* DefaultHierarchy = GetDefaultHierarchy();
	...
	if (CanHandleZeroControlsFromClosestBone() &&
		PinnedControlRigEditor.IsValid() &&
		Blueprint &&
		DefaultHierarchy &&
		DebuggedHierarchy)
	{
		const FScopedTransaction Transaction(LOCTEXT("ZeroControlsFromClosestBoneTransaction", "Zero Controls from Closest Bone"));

		const TArray<FRigElementKey> SelectedKeys = GetSelectedElementKeys();

		TMap<FRigElementKey, FTransform> ClosestTransforms;
		TMap<FRigElementKey, FTransform> ParentGlobalTransforms;
		for (const FRigElementKey& SelectedKey : GetSelectedElementKeys())
		{
		...
		for (const FRigElementKey& SelectedKey : SelectedKeys)
		{
			...
			const FTransform GlobalTransform = ClosestTransforms[SelectedKey];
			const FTransform LocalTransform = GlobalTransform.GetRelativeTransform(ParentGlobalTransforms[SelectedKey]);
...
bool SRigHierarchy::CanHandleZeroControlsFromClosestBone() const
{
	if (IsProceduralSelected())
	{
		return false;
...
void SRigHierarchy::HandleZeroControlShape()
{
	const TSharedPtr<IControlRigBaseEditor> PinnedControlRigEditor = ControlRigEditor.IsValid() ? ControlRigEditor.Pin() : nullptr;
	const TScriptInterface<IControlRigAssetInterface> Blueprint = PinnedControlRigEditor.IsValid() ? PinnedControlRigEditor->GetControlRigAssetInterface() : nullptr;
	URigHierarchy* DebuggedHierarchy = GetHierarchy();

	if (CanHandleZeroControlShape() &&
		PinnedControlRigEditor.IsValid() &&
		Blueprint &&
		DebuggedHierarchy)
	{
		const FScopedTransaction Transaction(LOCTEXT("ZeroControlShapeTransaction", "Zero Control Shape"));

		const FRigHierarchyInteractionBracket DebuggedInteractionBracket(DebuggedHierarchy);
		...
		for (const TSharedPtr<FRigTreeElement>& SelectedItem : SelectedItems)
		{
			...
			if (FRigControlElement* ControlElement = DebuggedHierarchy->Find<FRigControlElement>(SelectedItem->Key.GetElement()))
			{
				...
				constexpr bool bInitial = true;
				constexpr bool bNotInitial = false;
				constexpr bool bSetupUndo = true;
				constexpr bool bAffectChildren = true;
				constexpr bool bPrintPythonCommand = true;

				DebuggedHierarchy->SetControlShapeTransform(Key, ShapeLocalTransform, bInitial, bSetupUndo);
				DebuggedHierarchy->SetControlShapeTransform(Key, ShapeLocalTransform, bNotInitial, bSetupUndo);
				GetHierarchy()->SetControlShapeTransform(Key, ShapeLocalTransform, bInitial, bSetupUndo);
				GetHierarchy()->SetControlShapeTransform(Key, ShapeLocalTransform, bNotInitial, bSetupUndo);

				DebuggedHierarchy->SetLocalTransform(Key, FTransform::Identity, bNotInitial, bAffectChildren, bSetupUndo);
				DebuggedHierarchy->SetLocalTransform(Key, FTransform::Identity, bInitial, bAffectChildren, bSetupUndo);
				GetHierarchy()->SetLocalTransform(Key, FTransform::Identity, bNotInitial, bAffectChildren, bSetupUndo, bPrintPythonCommand);
				GetHierarchy()->SetLocalTransform(Key, FTransform::Identity, bInitial, bAffectChildren, bSetupUndo, bPrintPythonCommand);

				if (FControlRigEditorEditMode* EditMode = PinnedControlRigEditor->GetEditMode())
				{
...
bool SRigHierarchy::CanHandleZeroControlShape() const
{
	if (IsProceduralSelected())
	{
		return false;
...
void SRigHierarchy::HandleSetInitialTransformsAsCurrent()
{
	const TScriptInterface<IControlRigAssetInterface> Blueprint = ControlRigEditor.Pin()->GetControlRigAssetInterface();
	URigHierarchy* DefaultHierarchy = GetDefaultHierarchy();
	URigHierarchy* DebuggedHierarchy = GetHierarchy();

	if (CanHandleSetInitialTransformsAsCurrent() &&
		Blueprint &&
		DefaultHierarchy &&
		DebuggedHierarchy)
	{
		const FScopedTransaction Transaction(LOCTEXT("SetInitialTransformsAsCurrentTransaction", "Set Initial Transforms as Current"));
		...
		for (const FRigElementKey& SelectedKey : SelectedKeys)
		{
				...
				SelectedKey.Type == ERigElementType::Connector)
			{
				...
				if (FRigTransformElement* DebuggedTransformElement = DebuggedHierarchy->Find<FRigTransformElement>(SelectedKey))
				{
					DebuggedHierarchy->SetTransform(DebuggedTransformElement, LocalTransform, ERigTransformType::InitialLocal, bAffectChildren, bSetupUndo, bForce, bPrintPythonCommand);
					DebuggedHierarchy->SetTransform(DebuggedTransformElement, LocalTransform, ERigTransformType::CurrentLocal, bAffectChildren, bSetupUndo, bForce, bPrintPythonCommand);
				}
				
				if (FRigTransformElement* DefaultTransformElement = DefaultHierarchy->Find<FRigTransformElement>(SelectedKey))
				{
					DefaultHierarchy->SetTransform(DefaultTransformElement, LocalTransform, ERigTransformType::InitialLocal, bAffectChildren, bSetupUndo);
					DefaultHierarchy->SetTransform(DefaultTransformElement, LocalTransform, ERigTransformType::CurrentLocal, bAffectChildren, bSetupUndo);
				}
			}
		}
	}
...
bool SRigHierarchy::CanHandleSetInitialTransformsAsCurrent() const
{
	if (IsProceduralSelected())
	{
		return false;
...
{
	FControlRigAssetInterfacePtr Blueprint = ControlRigEditor.Pin()->GetControlRigAssetInterface();
	if (Blueprint == nullptr)
	{
		return;
	}

	UControlRig* DebuggedControlRig = Cast<UControlRig>(Blueprint->GetRigVMAssetInterface()->GetObjectBeingDebugged());
...
FReply SRigHierarchy::ReparentOrMatchTransform(const TArray<FRigHierarchyKey>& DraggedKeys, FRigHierarchyKey TargetKey, bool bReparentItems, int32 LocalIndex)
{
	...
	if (Hierarchy && ControlRigBlueprint.IsValid())
	{
		...
		bool bBlueprintSuspensionFlag = ControlRigBlueprint->GetRigVMAssetInterface()->bSuspendAllNotifications;
		...
		TGuardValue<bool> GuardRigHierarchyChanges(bIsChangingRigHierarchy, true);
		TGuardValue<bool> SuspendBlueprintNotifs(ControlRigBlueprint->GetRigVMAssetInterface()->bSuspendAllNotifications, bBlueprintSuspensionFlag);
