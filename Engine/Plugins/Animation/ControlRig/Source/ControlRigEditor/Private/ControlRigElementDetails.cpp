...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ControlRigElementDetails)
...
void RigElementKeyDetails_GetCustomizedInfo(TSharedRef<IPropertyHandle> InStructPropertyHandle, FControlRigAssetInterfacePtr& OutBlueprint)
{
	...
	if (OutBlueprint == nullptr)
	{
		...
		for (UPackage* Package : Packages)
		{
			...
			for (UObject* SubObject : SubObjects)
			{
				...
				{
					FControlRigAssetInterfacePtr Blueprint(Rig->GetClass()->ClassGeneratedBy);
					if (Blueprint)
					{
						if(Blueprint.GetObject()->GetOutermost() == Package)
						{
	...
	if(OutBlueprint)
	{
		for (UEdGraph* Graph : OutBlueprint->GetRigVMAssetInterface()->GetUberGraphs())
		{
...
FControlRigAssetInterfacePtr RigElementDetails_GetBlueprintFromHierarchy(URigHierarchy* InHierarchy)
{
	...
	FControlRigAssetInterfacePtr Blueprint = IControlRigAssetInterface::GetInterfaceOuter(InHierarchy);
	if(Blueprint == nullptr)
	{
		UControlRig* Rig = InHierarchy->GetTypedOuter<UControlRig>();
		if(Rig)
		{
			Blueprint = FControlRigAssetInterfacePtr(Rig->GetClass()->ClassGeneratedBy);
        }
	}
	return Blueprint;
}
...
{
	BlueprintBeingCustomized = InArgs._Asset;
...
void SRigElementKeyWidget::UpdateElementNameList()
{
	...
	{
		for (UEdGraph* Graph : BlueprintBeingCustomized->GetRigVMAssetInterface()->GetUberGraphs())
		{
...
FReply FRigElementKeyDetails::OnGetSelectedClicked()
{
	if (BlueprintBeingCustomized)
	{
		const TArray<FRigElementKey>& Selected = BlueprintBeingCustomized->GetHierarchy()->GetSelectedKeys();
		if (Selected.Num() > 0)
		{
			if (TypeHandle.IsValid())
			{
...
void FRigComponentKeyDetails::UpdateComponentNameList()
{
	...
	URigHierarchy* Hierarchy = BlueprintBeingCustomized->GetHierarchy();
...
void FRigComputedTransformDetails::OnTransformChanged(FEditPropertyChain* InPropertyChain)
{
	if(BlueprintBeingCustomized && InPropertyChain)
	{
		if(InPropertyChain->Num() > 1)
		{
			...
			BlueprintBeingCustomized->GetRigVMAssetInterface()->BroadcastPostEditChangeChainProperty(ChainEvent);
		}
	}
}
...
void FRigBaseElementDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	for(TWeakObjectPtr<UObject> DetailObject : DetailObjects)
	{
		...
		if(const FControlRigAssetInterfacePtr Blueprint = Info.GetAsset())
		{
			Info.DefaultElement = Blueprint->GetHierarchy()->GetHandle(Key);
		}
	...
	GeneralCategory.AddCustomRow(FText::FromString(TEXT("Spawn Index")), true /* advanced */)
	.IsEnabled(!bIsProcedural)
	.NameContent()
	[
		SNew(STextBlock)
		.Text(FText::FromString(TEXT("Spawn Index")))
		.Font(IDetailLayoutBuilder::GetDetailFont())
	]
	.ValueContent()
	[
		SNew(STextBlock)
		.Text(this, &FRigBaseElementDetails::GetSpawnIndex)
		.Font(IDetailLayoutBuilder::GetDetailFont())
	];
...
FText FRigBaseElementDetails::GetSpawnIndex() const
{
	if(PerElementInfos.Num() > 1)
	{
		return ControlRigDetailsMultipleValues;
	}
	return FText::FromString(FString::FromInt(PerElementInfos[0].GetElement<FRigBaseElement>()->GetSpawnIndex()));
}
...
bool FRigBaseElementDetails::IsConstructionModeEnabled() const
{
	...
	if(const FControlRigAssetInterfacePtr Blueprint = PerElementInfos[0].GetAsset())
	{
...
void FRigBaseElementDetails::CustomizeMetadata(IDetailLayoutBuilder& DetailBuilder)
{
	...
	if (!MetadataHandle.IsValid())
	{
		...
		if (FControlRigAssetInterfacePtr Blueprint = PerElementInfos[0].GetAsset())
		{
			UAssetEditorSubsystem* AssetEditorSubsystem = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>();
			if (IAssetEditorInstance* EditorInstance = AssetEditorSubsystem->FindEditorForAsset(Blueprint.GetObject(), false))
			{
	...
	for(FName MetadataName: MetadataNames)
	{
		...
		TSharedPtr<FStructOnScope> StructOnScope = MakeShareable(new FStructOnScope(Metadata->GetMetadataStruct()));
		Metadata->GetMetadataStruct()->CopyScriptStruct(StructOnScope->GetStructMemory(), Metadata, 1);
	...
	TSharedPtr<SWidget> NameContent)
{
		...
		bool bSetupUndoRedo)
	{
		...
		if (UControlRig* DebuggedRig = Cast<UControlRig>(StrongThisPtr->PerElementInfos[0].GetAsset()->GetObjectBeingDebugged()))
		{
	...
	TransformWidgetArgs.OnBeginSliderMovement_Lambda(
			...
			ESlateTransformSubComponent::Type SubComponent)
		{
			...
			if (UControlRig* DebuggedRig = Cast<UControlRig>(StrongThisPtr->PerElementInfos[0].GetAsset()->GetObjectBeingDebugged()))
			{
				...
				StrongThisPtr->InteractionScopes.Emplace(MakeUnique<FControlRigInteractionScope>(DebuggedRig));
			}
		});
...
{
	PropertyUtilities->ForceRefresh();
	UEnum* ControlEnum = FindObject<UEnum>(nullptr, **InItem.Get(), EFindObjectFlags::None);
	...
	for(int32 ControlIndex = 0; ControlIndex < PerElementInfos.Num(); ControlIndex++)
	{
		...
		if (ControlEnum != nullptr)
		{
			...
			if (UControlRig* DebuggedRig = Cast<UControlRig>(Info.GetAsset()->GetObjectBeingDebugged()))
			{
		...
		[
			...
			[
				...
				.OnTextCommitted_Lambda([this, AvailableSpace](const FText& InText, ETextCommit::Type InCommitType)
				{
					...
					FControlRigAssetInterfacePtr Blueprint = PerElementInfos[0].GetAsset();
...
void FRigControlElementDetails::HandleControlTypeChanged(ERigControlType ControlType, TArray<FRigElementKey> ControlKeys, const TSharedRef<IPropertyUtilities> PropertyUtilities)
{
	...
	for(const FRigElementKey& ControlKey : ControlKeys)
	{
		...
		{
			PerElementInfos[0].GetAsset()->PropagateHierarchyFromBPToInstances();
		}
	}
...
void FRigControlElementDetails::CustomizeShape(IDetailLayoutBuilder& DetailBuilder)
{
	...
	if (FControlRigAssetInterfacePtr Blueprint = PerElementInfos[0].GetAsset())
	{
		if(UEdGraph* RootEdGraph = Blueprint->GetRigVMAssetInterface()->GetEdGraph(Blueprint->GetRigVMAssetInterface()->GetModel()))
		{
			if(UControlRigGraph* RigGraph = Cast<UControlRigGraph>(RootEdGraph))
			{
				...
				const TArray<TSoftObjectPtr<UControlRigShapeLibrary>>* ShapeLibraries = &Blueprint->GetShapeLibraries();
				...
				RigGraph->CacheNameLists(Hierarchy, &Blueprint->GetDrawContainer(), *ShapeLibraries);
		...
		{
			const bool bUseNameSpace = Blueprint->GetShapeLibraries().Num() > 1;
			for(TSoftObjectPtr<UControlRigShapeLibrary>& ShapeLibrary : Blueprint->GetShapeLibraries())
			{
	...
	.ValueContent()
	[
		SAssignNew(ShapeNameListWidget, SControlRigShapeNameList, ControlElements, PerElementInfos[0].GetAsset())
...
void FRigConnectorElementDetails::CustomizeSettings(IDetailLayoutBuilder& DetailBuilder)
{
	...
	IsPostConstructionHandle = SettingsHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FRigConnectorSettings, bPostConstruction));
	IDetailPropertyRow& IsPostConstructionPropertyRow = SettingsCategory
		.AddProperty(IsPostConstructionHandle)
		.IsEnabled(IsArrayEnabled());

	if(!IsArrayEnabled())
	{
		IsPostConstructionPropertyRow.ToolTip(LOCTEXT("PrimaryConnectorsDontAllowPostConstructionToolTip", "Primary Connectors cannot be used during post construction."));
	}
	else
	{
		IsPostConstructionPropertyRow.ToolTip(LOCTEXT("PostConstructionConnectorToolTip", "This connector will be available only during the Post Construction Event.\nYou can connect any element spawned during Construction from any module to it."));
	}
...
bool FRigConnectorElementDetails::OnTargetsChanged(TArray<FRigElementKey> InTargets)
{
	if(PerElementInfos.Num() == 1)
	{
		if(FControlRigAssetInterfacePtr ControlRigBlueprint = PerElementInfos[0].GetAsset())
		{
