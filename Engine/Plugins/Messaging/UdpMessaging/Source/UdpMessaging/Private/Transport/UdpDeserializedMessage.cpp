...
TWeakObjectPtr<UScriptStruct> DeserializeSafeLookupForFindObject_InThread(FStringView TypeName)
{
	UUdpMessagingSettings const* CurrentSettings = GetDefault<UUdpMessagingSettings>();
	const bool bRunningInTaskThread = (CurrentSettings->MaxConcurrentDeserializationTasks > 1);
	if (bRunningInTaskThread)
	{
		FGCScopeGuard Guard;
		return FindFirstObjectSafe<UScriptStruct>(TypeName, EFindFirstObjectOptions::EnsureIfAmbiguous);
	}
	else
	{
		// If we're not in a task, we want to avoid potentially stalling the processor thread.
		FGCScopeTryGuard MaybeGuard;
		if (MaybeGuard.LockSucceeded())
		{
			return FindFirstObjectSafe<UScriptStruct>(TypeName, EFindFirstObjectOptions::EnsureIfAmbiguous);
		}
	}
...
bool FUdpDeserializedMessageDetails::DeserializeV10(FUdpDeserializedMessage& DeserializedMessage, FMemoryReader& MessageReader)
{
	// message type info
	{
		...
		{
			DeserializedMessage.TypeInfo = DeserializeSafeLookupForFindObject_InThread(*MessageType.ToString());
		}
