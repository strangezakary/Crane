...
bool FSceneRenderState::SetupRayTracingScene(FRDGBuilder& GraphBuilder, FSceneUniformBuffer& SceneUniforms, int32 LODIndex)
{
...

	{
			...
			for (int32 LandscapeIndex = 0; LandscapeIndex < LandscapeRenderStates.Elements.Num(); LandscapeIndex++)
			{
				...
				for (int32 SubY = 0; SubY < Landscape.NumSubsections; SubY++)
				{
					for (int32 SubX = 0; SubX < Landscape.NumSubsections; SubX++)
					{
						...
						const uint32 SegmentCount = Landscape.SectionRayTracingStates[SubSectionIdx]->Geometry.Initializer.Segments.Num();
						...
						{
							FDynamicRayTracingMeshCommandContext CommandContext(DynamicRayTracingMeshCommandStorage, DynamicRTShaderBindings, RayTracingInstance.GeometryRHI, MeshBatch.SegmentIndex, SBTAllocation);
							FLightmapRayTracingMeshProcessor RayTracingMeshProcessor(&CommandContext);

							RayTracingMeshProcessor.AddMeshBatch(MeshBatch, 1, nullptr);
						}
		...
		if (IsRayTracingEnabled())
		{
			...
			{
				HitGroupContributionsBuffer.Initialize(RHICmdList, TEXT("LightmassRayTracingHitGroupBuffer"), 4, SceneInitializer.MaxNumInstances);
			}
