...
void FPCGGraphExecutor::CombineParams(FPCGGraphTask& Task)
{
	...
	if (AllParamsData.Num() > 1)
	{
		...
		TArray<FPCGCrc> TempDataCrcs{};

		TempTaggedData.Reserve(NewNumberOfInputs);

		bool bForwardCrcs = true;

		if (!ensure(Task.TaskInput.TaggedData.Num() == Task.TaskInput.DataCrcs.Num()))
		{
			bForwardCrcs = false;
			TempDataCrcs.SetNumZeroed(NewNumberOfInputs);
		}
		else
		{
			TempDataCrcs.Reserve(NewNumberOfInputs);
		}

		for (int32 i = 0; i < Task.TaskInput.TaggedData.Num(); ++i)
		{
			FPCGTaggedData& TaggedData = Task.TaskInput.TaggedData[i];

			if (TaggedData.Pin != PCGPinConstants::DefaultParamsLabel)
			{
				TempTaggedData.Add(MoveTemp(TaggedData));

				if (bForwardCrcs)
				{
					TempDataCrcs.Add(MoveTemp(Task.TaskInput.DataCrcs[i]));
				}
			}
		}
		...
		if (CombinedParamData)
		{
			...
			if (bForwardCrcs)
			{
				TempDataCrcs.Add(CombineParams.ComputeCrc(/*bFullDataCrc=*/true));
			}
		}

		Task.TaskInput.TaggedData = MoveTemp(TempTaggedData);
		Task.TaskInput.DataCrcs = MoveTemp(TempDataCrcs);
	}
}
