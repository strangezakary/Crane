...
static TAutoConsoleVariable<bool> CVarPCGFullDataCrcInCommandlets(
	TEXT("pcg.FullDataCrcInCommandlets"),
	true,
	TEXT("Always compute full data Crcs in Commandlets (to improve reusability)."));
...
{
#if WITH_EDITOR
	const bool bComputeFullDataCrc = (bFullDataCrc || (CVarPCGFullDataCrcInCommandlets.GetValueOnAnyThread() && IsRunningCommandlet())) && SupportsFullDataCrc();
#else
...
TArray<FPCGTaggedData> FPCGDataCollection::GetAllParams() const
{
	return TaggedData.FilterByPredicate([](const FPCGTaggedData& Data)
	{
		return Cast<UPCGParamData>(Data.Data) || (Data.Data && Data.Data->GetUnderlyingDataType() == EPCGDataType::Param);
	});
}
...
TArray<FPCGTaggedData> FPCGDataCollection::GetParamsByPin(const FName& InPinLabel) const
{
	return TaggedData.FilterByPredicate([&InPinLabel](const FPCGTaggedData& Data)
	{
		return Data.Pin == InPinLabel && (Cast<UPCGParamData>(Data.Data) || (Data.Data && Data.Data->GetUnderlyingDataType() == EPCGDataType::Param));
	});
}
...
TArray<FPCGTaggedData> FPCGDataCollection::GetTaggedParams(const FString& InTag) const
{
	...
	{
		return Data.Tags.Contains(InTag) && (Cast<UPCGParamData>(Data.Data) || (Data.Data && Data.Data->GetUnderlyingDataType() == EPCGDataType::Param));
	});
}
