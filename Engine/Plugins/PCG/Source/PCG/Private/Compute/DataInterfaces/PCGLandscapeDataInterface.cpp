...
void UPCGLandscapeDataProvider::InitializeFromLandscapeCollision(UPCGDataBinding* InBinding, ALandscape* InLandscape, const FBox& Bounds)
{
	...
	// Expand max bounds by the LS grid size so that we don't go off the height texture when computing normals for points at the edges of the bounds.
	SystemWorldBounds.Max += LWCTileOffset + FVector(LandscapeGridSize.X, LandscapeGridSize.Y, 0.0);
