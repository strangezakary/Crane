...
PCG_DEFINE_TYPE_INFO(FPCGDataTypeInfoLandscapeSpline, UPCGLandscapeSplineData)
...
FTransform UPCGLandscapeSplineData::GetTransformAtDistance(int SegmentIndex, FVector::FReal Distance, bool bWorldSpace, FBox* OutBounds) const
{
	...
	const int32 SecondToLastPointIndex = InterpPoints.Num() - 2;

	if (SecondToLastPointIndex < 0)
	{
		// Not enough points in the segment to interpolate a transform.
		return GetTransform();
	}
	...
	if (ClampedSegmentIndex == SegmentIndex)
	{
		...
		// If our RightPoint ends up being on the next segment, clamp it back to the current segment.
		PointIndex = FMath::Clamp(PointIndex, 0, SecondToLastPointIndex);
	}
	else if(ClampedSegmentIndex < SegmentIndex)
	{
		// If we've landed beyond the last segment, just clamp to the end of the last segment.
		Alpha = 1.0f;
		PointIndex = SecondToLastPointIndex;
	}
