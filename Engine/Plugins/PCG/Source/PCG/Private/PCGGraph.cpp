...
{
	return GetGraph() && GetGraph()->bIsStandaloneGraph;
}
...
bool UPCGGraphInterface::IsStandaloneGraphAsset(const FAssetData& InAssetData)
{
	...
	{
		return GraphInterface->IsStandaloneGraph();
	}
	else if (InAssetData.IsInstanceOf<UPCGGraph>())
	{
		bool bTagValue = false;
		return InAssetData.GetTagValue(GET_MEMBER_NAME_CHECKED(UPCGGraph, bIsStandaloneGraph), bTagValue) && bTagValue;
	}
	else if (InAssetData.IsInstanceOf<UPCGGraphInstance>())
	{
		...
		{
			return IsStandaloneGraphAsset(GraphAssetData);
		}
	}
...
void UPCGGraph::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedEvent)
{
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(UPCGGraph, bLandscapeUsesMetadata) ||
		PropertyName == GET_MEMBER_NAME_CHECKED(UPCGGraph, bHasDefaultConstructedInputs))
	{
		...
		|| PropertyName == GET_MEMBER_NAME_CHECKED(UPCGGraph, HiGenExponential))
	{
		...
		|| PropertyName == GET_MEMBER_NAME_CHECKED(UPCGGraph, bIsStandaloneGraph))
	{
