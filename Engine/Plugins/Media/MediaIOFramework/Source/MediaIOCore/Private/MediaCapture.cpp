...
static TAutoConsoleVariable<int32> CVarMediaIOPrerollFrames(
	TEXT("MediaIO.PrerollFrames"), 0,
	TEXT("Whether to preroll frames during media capture. Enabling this may reduce latency by initializing GPU resources during the preroll phase"),
	ECVF_RenderThreadSafe);


static bool bSimulateError = false;

FAutoConsoleCommand FMediaIOSetErrorState
(
	TEXT("MediaIO.SimulateError"),
	TEXT("Causes the next call to ProcessCapture_RenderThread to fail."),
...
FString FMediaCaptureOptions::GetColorConversionSettingsString() const
{
	return ColorConversionSettings.ToString();
}
...
bool UMediaCapture::StartSourceCapture(TSharedPtr<UE::MediaCapture::Private::FCaptureSource> InSource)
{
	...
	if (bInitialized)
	{
		...
		if (DesiredCaptureOptions.CapturePhase != EMediaCapturePhase::EndFrame)
		{
			...
			if (CaptureSource->GetSceneViewport())
			{
				RenderTarget = CaptureSource->GetSceneViewport()->GetViewport();
			}
			else if (CaptureSource->GetRenderTarget())
			{
				RenderTarget = CaptureSource->GetRenderTarget()->GameThread_GetRenderTargetResource();
			}

			if (RenderTarget) 
			{
				ViewExtension = FSceneViewExtensions::NewExtension<FMediaCaptureSceneViewExtension>(this, DesiredCaptureOptions.CapturePhase, Priority, RenderTarget);
			}
			else
			{
				UE_LOG(LogMediaIOCore, Warning, TEXT("Could not acquire a render target to initialize the scene view extension. Consider changing the capture phase to End Frame."));
			}
		}
		...
		if (CaptureSource->GetCaptureType() == UE::MediaCapture::Private::ECaptureType::Immediate)
		{
			...
			// Add frame info for current frame in case OnBeginFrame delegate has already fired
			AddFrameInfoForCaptureImmediate(GFrameCounter);
		}
		else
		{
...
{
	StopCapture_Callback(bAllowPendingFrameToBeProcess, FSimpleDelegate{});
}

void UMediaCapture::StopCapture_Callback(bool bAllowPendingFrameToBeProcess, FSimpleDelegate Callback)
{
	if (bWaitingForStopCapture)
	{
		// This capture is already in the process of stopping.
		return;
	}
	...
	if (!IsInGameThread())
	{
		...
		AsyncTask(ENamedThreads::GameThread, [Self, bAllowPendingFrameToBeProcess, Callback]()
			{
				UMediaCapture* MediaCapture = Self.Get();
				if (UObjectInitialized() && MediaCapture)
				{
					MediaCapture->StopCapture_Callback(bAllowPendingFrameToBeProcess, Callback);
				}
			});
	...
	else
	{
		...
		{
			if (SupportsStopCapture_Future())
			{
				SetState(EMediaCaptureState::StopRequested);
				// Prevent reentry and subsequent calls to StopCapture with bAllowPendingFrameToBeProcess = false to set the state to Stopped
				// while the TFuture isn't ready.
				bWaitingForStopCapture = true;
			}
			else
			{
			...
			if (SupportsStopCapture_Future())
			{
				...
				TRACE_BOOKMARK(TEXT("UMediaCapture::InitiateStopCapture"));
				StopCapture_Future().Then([Self, Callback](TFuture<void>) mutable
				{
					AsyncTask(ENamedThreads::GameThread, [Self, Callback]
					{
						// Setting bEvenifPendingKill because this might be called as part of BeginDestroy,
						// but we still want to cleanup card connections.
						if (TStrongObjectPtr<UMediaCapture> Capture = Self.Pin(/*bEvenIfPendingKill=*/true)) 
						{
							Capture->SetState(EMediaCaptureState::Stopped);
							Capture->PostCaptureCleanup();
							Callback.ExecuteIfBound();
							TRACE_BOOKMARK(TEXT("UMediaCapture::CompleteStopCapture"));
						}
					});
				});
			}
...
bool UMediaCapture::CaptureImmediate_RenderThread(const UE::MediaCaptureData::FCaptureFrameArgs& Args)
{
	...
	{
		UE_LOG(LogMediaIOCore, Warning, TEXT("Can't capture frame. Could not find the matching game frame %llu."), GFrameCounterRenderThread);
		return false;
	}
...
void UMediaCapture::PrepareAndDispatchCapture_GameThread(const TSharedPtr<UE::MediaCaptureData::FCaptureFrame>& CapturingFrame)
{
	...
	//The Lock only synchronize while we are copying the value to the enqueue. The viewport and the rendertarget may change while we are in the enqueue command.
	{
		...
		if (CaptureSource.IsValid())
		{
			...
			TWeakObjectPtr<UMediaCapture> WeakThis = this;

			UE_LOG(LogMediaIOCore, Verbose, TEXT("[%s] - Queuing frame to capture %d"), *MediaOutputName, CapturingFrame ? CapturingFrame->FrameId : -1);

			// RenderCommand to be executed on the RenderThread
			ENQUEUE_RENDER_COMMAND(FMediaOutputCaptureFrameCreateTexture)(
				[WeakThis, CaptureSource_RT = CaptureSource, CapturingFrame, InDesiredSize = DesiredSize, SourceGPUMask](FRHICommandListImmediate& RHICmdList)
			{
				SCOPED_GPU_MASK(RHICmdList, SourceGPUMask);

				TStrongObjectPtr<UMediaCapture> This = WeakThis.Pin();
				if (!This.IsValid())
				{
					return;
				}

				TSharedPtr<FCaptureFrame> NextPending = This->FrameManager->PeekNextPending<FCaptureFrame>();
				...
				// Capture frame
				{
					...
					FTextureRHIRef SourceTexture = CaptureSource_RT->GetSourceTextureForInput_RenderThread();
					
					FCaptureFrameArgs CaptureArgs{ GraphBuilder };
					CaptureArgs.MediaCapture = This.Get();
					CaptureArgs.DesiredSize = InDesiredSize;
					CaptureArgs.ResourceToCapture = MoveTemp(SourceTexture);

					This->ProcessCapture_RenderThread(CapturingFrame, CaptureArgs);
					UE_LOG(LogMediaIOCore, Verbose, TEXT("[%s] - Captured frame %d"), *This->MediaOutputName, CapturingFrame ? CapturingFrame->FrameId : -1);
					
					GraphBuilder.Execute();
				}
				
				if (!This->UseExperimentalScheduling())
				{
					//Process the next pending frame
					UE_LOG(LogMediaIOCore, Verbose, TEXT("[%s] - Processing pending frame %d"), *This->MediaOutputName, NextPending ? NextPending->FrameId : -1);
					if (NextPending)
					{
						This->ProcessReadyFrame_RenderThread(RHICmdList, This.Get(), NextPending);
					}
				}

				// Whatever happens, we want to decrement our counter to track enqueued commands
				--This->WaitingForRenderCommandExecutionCounter;
			});
...
void UMediaCapture::InitializeOutputResources(int32 InNumberOfBuffers)
{
	...
	[WeakThis](FRHICommandListImmediate& RHICmdList)
		{
			TStrongObjectPtr<UMediaCapture> This = WeakThis.Pin();
			if (!This.IsValid())
			{
			...
			for (int32 Index = 0; Index < This->NumberOfCaptureFrame; ++Index)
			{
				if (This->DesiredOutputResourceType == EMediaCaptureResourceType::Texture)
				{
					...
					if (This->GetDesiredCaptureOptions().bAutostopOnCapture || !CVarMediaIOPrerollFrames.GetValueOnRenderThread())
					{
...
void UMediaCapture::AddFrameInfoForCaptureImmediate(const uint64 FrameNumber)
{
	...
	CaptureData.BaseData.SourceFrameNumberRenderThread = FrameNumber;
	...
	// Insert it into the queue
	{
		...
		// Don't add if we already cached this frame number or newer.
		// Note: Only checks last inserted since frames are expected to increase monotonically and inserted at position 0.
		if (CaptureDataQueue.Num() > 0 && CaptureDataQueue[0].BaseData.SourceFrameNumberRenderThread >= FrameNumber)
		{
			return;
		}

		TRACE_BOOKMARK(TEXT("MediaCapture GT push frame %llu"), FrameNumber);
...
bool UMediaCapture::ProcessCapture_RenderThread(const TSharedPtr<UE::MediaCaptureData::FCaptureFrame>& CapturingFrame, UE::MediaCaptureData::FCaptureFrameArgs Args)
{
	...
	if (CapturingFrame)
	{
		...
		if (Args.RDGResourceToCapture || Args.ResourceToCapture)
		{
			...
			if (CapturingFrame->bPrerollFrame || GetDesiredCaptureOptions().bAutostopOnCapture || !CVarMediaIOPrerollFrames.GetValueOnRenderThread())
			{
			...
			bool bIsReadyToStartOutput = Args.MediaCapture->IsReadyToOutput();
			...
			{
				UE_LOG(LogMediaIOCore, Verbose, TEXT("Capturing %s frame (%d)"), FApp::GetTimecode().Frames % 2 == 0 ? TEXT("Even") : TEXT("Odd"), FApp::GetTimecode().Frames);

				if (Args.MediaCapture->ShouldCaptureThisFrame(CapturingFrame->CaptureBaseData))
				{
				...
				{
					// Return the frame to the available pool if we're not gonna capture it.
					bHasCaptureSuceeded = true;
					FrameManager->MarkAvailable(*CapturingFrame);

					UE_LOG(LogMediaIOCore, Verbose, TEXT("=============================================================== Skipping frame %d"), FApp::GetTimecode().Frames);
				}
			}
			else
			{
		...
		if (bHasCaptureSuceeded == false)
		{
			if (Args.MediaCapture->bIsAutoRestartRequired)
			{
				...
				AsyncTask(ENamedThreads::GameThread, [Self]
				{
					TStrongObjectPtr<UMediaCapture> MediaCapture = Self.Pin();
					if (UObjectInitialized() && MediaCapture.IsValid())
					{
...
void UMediaCapture::PostCaptureCleanup()
{
	// Clean up implementation resources.
	PostCaptureCleanupImpl();
